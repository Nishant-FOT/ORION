...
const PANEL_CLUSTER: Record<string, PanelChunkName> = {
  ...
  AaiiSentiment: 'panels-markets', AAIISentiment: 'panels-markets',
  CotPositioning: 'panels-markets',
  EtfFlows: 'panels-markets', ETFFlows: 'panels-markets', EarningsCalendar: 'panels-markets',
  ...
  MarketOverview: 'panels-markets',
  Positioning: 'panels-markets', Stablecoin: 'panels-markets', Stablecoins: 'panels-markets',
  StockAnalysis: 'panels-markets', StockBacktest: 'panels-markets',
  WsbTickerScanner: 'panels-markets', WsbTickers: 'panels-markets',
  YieldCurve: 'panels-markets', BigMacIndex: 'panels-markets',
  // Energy / commodities / supply infra
  ChokepointStrip: 'panels-energy', Chokepoints: 'panels-energy',
  ChokepointMonitoring: 'panels-energy',
  EnergyComplex: 'panels-energy', EnergyCrisis: 'panels-energy',
  EnergyDisruptions: 'panels-energy', EnergyRisk: 'panels-energy',
  EnergyRiskOverview: 'panels-energy', EnergySupply: 'panels-energy',
  FuelPrices: 'panels-energy', FuelShortage: 'panels-energy',
  FuelShortages: 'panels-energy', Hormuz: 'panels-energy',
  HormuzTracker: 'panels-energy',
  OilInventories: 'panels-energy', PipelineStatus: 'panels-energy',
  StorageFacilityMap: 'panels-energy', StorageFacilities: 'panels-energy',
  RenewableEnergy: 'panels-energy', AlternativeRoutes: 'panels-energy',
  CorridorRiskMonitor: 'panels-energy', LiveDisruptionProbability: 'panels-energy',
  ProcurementActionCenter: 'panels-energy', ProcurementAdvisor: 'panels-energy',
  RefineryCompatibility: 'panels-energy', ReserveOptimization: 'panels-energy',
  AisShipping: 'panels-energy',
  IndiaEnergyHub: 'panels-energy',
  ...
  UcdpEvents: 'panels-defense', IndiaSprTimeline: 'panels-defense',
  // News / feeds / briefs
  BreakingNews: 'panels-news', Breakthroughs: 'panels-news',
  ...
  LiveNews: 'panels-news', LiveIntelligence: 'panels-news',
  News: 'panels-news',
  PositiveNewsFeed: 'panels-news', PositiveNews: 'panels-news',
  TelegramIntel: 'panels-news',
  // Macro / prices / trade
  BigMac: 'panels-economy', ConsumerPrices: 'panels-economy',
  Economic: 'panels-economy', EconomicIndicators: 'panels-economy',
  ExecutiveAction: 'panels-economy', ExecutiveReports: 'panels-economy',
  FaoFoodPriceIndex: 'panels-economy', FinancialStress: 'panels-economy',
  FSI: 'panels-economy',
  GroceryBasket: 'panels-economy', GulfEconomies: 'panels-economy',
  Investments: 'panels-economy', MacroTiles: 'panels-economy',
  NationalDebt: 'panels-economy', Sanctions: 'panels-economy',
  SanctionsPressure: 'panels-economy',
  SupplyChain: 'panels-economy', TradePolicy: 'panels-economy',
  QuantitativeRisk: 'panels-economy',
  ...
  Cascade: 'panels-intel', CascadeAnalysis: 'panels-intel',
  Correlation: 'panels-intel',
  CountryBrief: 'panels-intel', CountryBriefPage: 'panels-intel',
  CountryDeepDive: 'panels-intel', CountryInstabilityIndex: 'panels-intel',
  CountryTimeline: 'panels-intel',
  ...
  GeopoliticalHubs: 'panels-intel', GeopoliticalRisk: 'panels-intel',
  HeroSpotlight: 'panels-intel',
  HistoricalIntelligence: 'panels-intel', HistoricalIntel: 'panels-intel',
  ...
  RegionalIntelligenceBoard: 'panels-intel', RegionalIntel: 'panels-intel',
  Regulation: 'panels-intel',
  OrionDecisionDesk: 'panels-intel',
  AiScenarioSimulator: 'panels-intel', ScenarioSimulator: 'panels-intel',
  Webcams: 'panels-intel',
  // Disasters / climate / connectivity / society
  ClimateAnomaly: 'panels-risk', Counters: 'panels-risk',
  CyberThreats: 'panels-risk',
  DiseaseOutbreaks: 'panels-risk', Earthquakes: 'panels-risk',
  Displacement: 'panels-risk', GeoHubs: 'panels-risk',
  Giving: 'panels-risk', InternetDisruptions: 'panels-risk',
  InternetOutages: 'panels-risk',
  PopulationExposure: 'panels-risk', Radiation: 'panels-risk',
  ...
  SocialVelocity: 'panels-risk', Species: 'panels-risk',
  ...
  WeatherAlerts: 'panels-risk',
  AlertCenter: 'panels-risk',
};
...
function htmlVariantPlugin(activeMeta: VariantMeta, activeVariant: string): Plugin {
  return {
    name: 'html-variant',
    transformIndexHtml(html) {
      let result = html
        ...
        .replace(/"name": "ORION"/, `"name": "${activeMeta.siteName}"`)
        .replace(/"alternateName": "ORION"/, `"alternateName": "${activeMeta.siteName.replace(' ', '')}"`)
        .replace(/"url": "https:\/\/orion\.app\/"/, `"url": "${activeMeta.url}"`)
...
function dashboardHtmlOutputPlugin(): Plugin {
  return {
    name: 'orion-dashboard-html-output',
    apply: 'build',
    enforce: 'post',
...
// deferred form (media="print" + data-orion-deferred-style="dashboard") plus a
...
function deferDashboardStylesheetLinks(html: string, bundle: OutputBundle): string {
  return html.replace(/<link\b(?=[^>]*\brel=["']stylesheet["'])(?=[^>]*\bhref=["'][^"']+\.css["'])[^>]*>/gi, (tag) => {
    if (/\bdata-orion-deferred-style=/.test(tag) || /\bmedia=/.test(tag)) return tag;
    if (!shouldDeferDashboardStylesheet(tag, bundle)) return tag;
    const deferredTag = tag.replace(/\s*\/?>$/, ' media="print" data-orion-deferred-style="dashboard">');
  ...
  async function buildRouter() {
    const [
      ...
      webcamServerMod, webcamHandlerMod,
        ...
        import('./src/generated/server/orion/seismology/v1/service_server'),
        import('./server/orion/seismology/v1/handler'),
        import('./src/generated/server/orion/wildfire/v1/service_server'),
        import('./server/orion/wildfire/v1/handler'),
        import('./src/generated/server/orion/climate/v1/service_server'),
        import('./server/orion/climate/v1/handler'),
        import('./src/generated/server/orion/prediction/v1/service_server'),
        import('./server/orion/prediction/v1/handler'),
        import('./src/generated/server/orion/displacement/v1/service_server'),
        import('./server/orion/displacement/v1/handler'),
        import('./src/generated/server/orion/aviation/v1/service_server'),
        import('./server/orion/aviation/v1/handler'),
        import('./src/generated/server/orion/research/v1/service_server'),
        import('./server/orion/research/v1/handler'),
        import('./src/generated/server/orion/unrest/v1/service_server'),
        import('./server/orion/unrest/v1/handler'),
        import('./src/generated/server/orion/conflict/v1/service_server'),
        import('./server/orion/conflict/v1/handler'),
        import('./src/generated/server/orion/maritime/v1/service_server'),
        import('./server/orion/maritime/v1/handler'),
        import('./src/generated/server/orion/cyber/v1/service_server'),
        import('./server/orion/cyber/v1/handler'),
        import('./src/generated/server/orion/economic/v1/service_server'),
        import('./server/orion/economic/v1/handler'),
        import('./src/generated/server/orion/infrastructure/v1/service_server'),
        import('./server/orion/infrastructure/v1/handler'),
        import('./src/generated/server/orion/market/v1/service_server'),
        import('./server/orion/market/v1/handler'),
        import('./src/generated/server/orion/news/v1/service_server'),
        import('./server/orion/news/v1/handler'),
        import('./src/generated/server/orion/intelligence/v1/service_server'),
        import('./server/orion/intelligence/v1/handler'),
        import('./src/generated/server/orion/military/v1/service_server'),
        import('./server/orion/military/v1/handler'),
        import('./src/generated/server/orion/positive_events/v1/service_server'),
        import('./server/orion/positive-events/v1/handler'),
        import('./src/generated/server/orion/giving/v1/service_server'),
        import('./server/orion/giving/v1/handler'),
        import('./src/generated/server/orion/trade/v1/service_server'),
        import('./server/orion/trade/v1/handler'),
        import('./src/generated/server/orion/supply_chain/v1/service_server'),
        import('./server/orion/supply-chain/v1/handler'),
        import('./src/generated/server/orion/natural/v1/service_server'),
        import('./server/orion/natural/v1/handler'),
        import('./src/generated/server/orion/resilience/v1/service_server'),
        import('./server/orion/resilience/v1/handler'),
        import('./src/generated/server/orion/leads/v1/service_server'),
        import('./server/orion/leads/v1/handler'),
        import('./src/generated/server/orion/scenario/v1/service_server'),
        import('./server/orion/scenario/v1/handler'),
        import('./src/generated/server/orion/shipping/v2/service_server'),
        import('./server/orion/shipping/v2/handler'),
        import('./src/generated/server/orion/forecast/v1/service_server'),
        import('./server/orion/forecast/v1/handler'),
        import('./src/generated/server/orion/consumer_prices/v1/service_server'),
        import('./server/orion/consumer-prices/v1/handler'),
        import('./src/generated/server/orion/thermal/v1/service_server'),
        import('./server/orion/thermal/v1/handler'),
        import('./src/generated/server/orion/health/v1/service_server'),
        import('./server/orion/health/v1/handler'),
        import('./src/generated/server/orion/radiation/v1/service_server'),
        import('./server/orion/radiation/v1/handler'),
        import('./src/generated/server/orion/sanctions/v1/service_server'),
        import('./server/orion/sanctions/v1/handler'),
        import('./src/generated/server/orion/imagery/v1/service_server'),
        import('./server/orion/imagery/v1/handler'),
        import('./src/generated/server/orion/webcam/v1/service_server'),
        import('./server/orion/webcam/v1/handler'),
      ]);
...
const RSS_PROXY_ALLOWED_DOMAINS = new Set([
  ...
  'abcnews.go.com', 'abcnews.com', 'feeds.abcnews.com', 'www.corriere.it', 'www.rt.com', 'www.alarabiya.net', 'tuoitrenews.vn',
  'www.pbs.org', 'feeds.news24.com',
...
function chatAnalystDevPlugin(): Plugin {
  return {
    name: 'chat-analyst-dev',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (req.url !== '/api/chat-analyst' && !req.url?.startsWith('/api/chat-analyst?')) {
          return next();
        }
        if (req.method === 'OPTIONS') {
          res.statusCode = 204;
        ...
        try {
          ...
          const geoContext = typeof body.geoContext === 'string'
            ? body.geoContext.trim().toUpperCase().slice(0, 2)
            : undefined;
          ...
          const prevUserTurn = history.filter((m: any) => m.role === 'user').slice(-1)[0]?.content ?? '';
          const retrievalQuery = prevUserTurn ? `${rawQuery} ${prevUserTurn}` : rawQuery;

          // Lazy import server-side modules
          const { assembleAnalystContext } = await import('./server/orion/intelligence/v1/chat-analyst-context');
          const { buildAnalystSystemPrompt } = await import('./server/orion/intelligence/v1/chat-analyst-prompt');
          const { buildActionEvents } = await import('./server/orion/intelligence/v1/chat-analyst-actions');
          const { callLlmReasoningStream } = await import('./server/_shared/llm');
          ...
          // Prepend meta + action events
          const enc = new TextEncoder();
          const metaEvent = enc.encode(`data: ${JSON.stringify({ meta: { sources: context.activeSources, degraded: context.degraded } })}\n\n`);
          const actionEvents = buildActionEvents(rawQuery).map((a: any) =>
            enc.encode(`data: ${JSON.stringify({ action: a })}\n\n`)
          );

          const prefixedStream = new ReadableStream<Uint8Array>({
            async start(controller) {
              controller.enqueue(metaEvent);
              for (const ev of actionEvents) controller.enqueue(ev);
          ...
          const reader = prefixedStream.getReader();
          ...
          console.error('[chat-analyst-dev]', err);
          if (!res.headersSent) {
            res.statusCode = 503;
            res.setHeader('Content-Type', 'application/json');
          }
...
function hormuzTrackerDevPlugin(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (req.url !== '/api/supply-chain/hormuz-tracker' && !req.url?.startsWith('/api/supply-chain/hormuz-tracker?')) {
          return next();
        }
        const now = Date.now();
        ...
        try {
          ...
          if (!body.result) throw new Error('No data');
          const raw = JSON.parse(body.result);
          cached = raw?._seed && raw?.data !== undefined ? raw.data : raw;
          cachedAt = now;
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Cache-Control', 'no-cache');
          res.end(JSON.stringify(cached));
          ...
          res.statusCode = 503;
          res.setHeader('Content-Type', 'application/json');
          res.end(JSON.stringify({ error: 'Hormuz tracker data unavailable. Run: node scripts/seed-hormuz.mjs' }));
        }
      });
    },
  };
...
function bootstrapDevPlugin(): Plugin {
  const BOOTSTRAP_KEYS: Record<string, string> = {
  ...
  const NEG_SENTINEL = '__ORION_NEG__';

  async function batchGet(keys: string[]): Promise<Map<string, unknown>> {
    const url = process.env.UPSTASH_REDIS_REST_URL;
    const token = process.env.UPSTASH_REDIS_REST_TOKEN;
    const result = new Map<string, unknown>();
    if (!url || !token || keys.length === 0) return result;
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (raw) {
        try {
          ...
          const unwrapped = parsed?._seed && parsed?.data !== undefined ? parsed.data : parsed;
          result.set(keys[i], unwrapped);
  ...
  const bootstrapCache = new Map<string, { data: Record<string, unknown>; missing: string[]; ts: number }>();
  const CACHE_TIER_TTL: Record<string, number> = { fast: 30_000, slow: 60_000, default: 10_000 };

  return {
    name: 'bootstrap-dev',
    configureServer(server) {
      // Pre-warm cache on startup so first browser request is instant
      const warmCache = async (tier: 'fast' | 'slow') => {
        const registry = Object.fromEntries(
          Object.entries(BOOTSTRAP_KEYS).filter(([k]) =>
            tier === 'fast' ? FAST_TIER_KEYS.has(k) : !FAST_TIER_KEYS.has(k)
          )
        );
        const redisKeys = Object.values(registry);
        const names = Object.keys(registry);
        const cacheKey = `tier:${tier}`;
        try {
          const result = await batchGet(redisKeys);
          const data: Record<string, unknown> = {};
          const missing: string[] = [];
          for (let i = 0; i < names.length; i++) {
            const val = result.get(redisKeys[i]);
            if (val !== undefined) data[names[i]] = val;
            else missing.push(names[i]);
          }
          bootstrapCache.set(cacheKey, { data, missing, ts: Date.now() });
          console.log(`[bootstrap-dev] Warmed ${tier} tier: ${Object.keys(data).length} keys, ${missing.length} missing`);
        } catch (e) {
          console.warn(`[bootstrap-dev] Warm ${tier} failed:`, (e as Error).message);
        }
      };
      // Fire and forget — don't block server start
      Promise.all([warmCache('fast'), warmCache('slow')]).catch(() => {});
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        const cacheKey = tier ? `tier:${tier}` : `keys:${requested?.join(',') || 'all'}`;

        const cached = bootstrapCache.get(cacheKey);
        const ttl = tier ? (CACHE_TIER_TTL[tier] ?? CACHE_TIER_TTL.default) : CACHE_TIER_TTL.default;
        if (cached && Date.now() - cached.ts < ttl) {
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Cache-Control', 'no-cache');
          res.end(JSON.stringify({ data: cached.data, missing: cached.missing }));
          return;
        }
        ...
        if (tier === 'slow' || tier === 'fast') {
          ...
          registry = requested
            ? Object.fromEntries(Object.entries(BOOTSTRAP_KEYS).filter(([k]) => requested.includes(k)))
            : BOOTSTRAP_KEYS;
        }
        ...
        try {
          ...
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Cache-Control', 'no-cache');
          res.end(JSON.stringify({ data, missing }));
        } catch (e) {
          console.error('[bootstrap-dev] error:', e);
          if (cached) {
            res.setHeader('Content-Type', 'application/json');
            res.setHeader('Cache-Control', 'no-cache');
            res.end(JSON.stringify({ data: cached.data, missing: cached.missing }));
            ...
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ data: {}, missing: names }));
          }
        }
      });
    },
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      htmlVariantPlugin(activeMeta, activeVariant),
      dashboardHtmlOutputPlugin(),
      polymarketPlugin(),
      bootstrapDevPlugin(),
      rssProxyPlugin(),
      youtubeLivePlugin(),
    ...
    server: {
      ...
      proxy: {
        ...
        '/api/fred-proxy': {
          target: 'https://api.stlouisfed.org',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/fred-proxy/, ''),
        },
        // EIA - Energy Information Administration proxy
        '/api/eia-data': {
          target: 'https://api.eia.gov',
          changeOrigin: true,
          rewrite: (path) => `/v2${path.replace(/^\/api\/eia-data/, '')}`,
        },
        // Alternative.me - Fear & Greed Index proxy
        '/api/alternative-me': {
          target: 'https://api.alternative.me',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/alternative-me/, ''),
        },
        ...
        // IUCN Red List proxy
        '/api/iucn': {
          target: 'https://apiv3.iucnredlist.org',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/iucn/, ''),
        },
        // USGS Earthquake API proxy
        '/api/usgs': {
          target: 'https://earthquake.usgs.gov',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/usgs/, ''),
        },
        // NWS Weather API proxy
        '/api/weather-gov': {
          target: 'https://api.weather.gov',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/weather-gov/, ''),
        },
        // Groq API proxy (for AI scenario simulation)
        '/api/groq-api': {
          target: 'https://api.groq.com',
          changeOrigin: true,
          rewrite: (path) => `/openai/v1${path.replace(/^\/api\/groq-api/, '')}`,
        },
        // GitHub Raw content proxy (for OWID energy data)
        '/api/github-raw': {
          target: 'https://raw.githubusercontent.com',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/github-raw/, ''),
        },
        // CFTC data proxy (for COT positioning)
        '/api/cftc': {
          target: 'https://www.cftc.gov',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/cftc/, ''),
        },
        // CoinGecko proxy (for stablecoins fallback)
        '/api/coingecko': {
