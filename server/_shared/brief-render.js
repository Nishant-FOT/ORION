// Deterministic renderer for the ORION Brief magazine.
//
...
//   - Brainstorm: docs/brainstorms/2026-04-17-orion-brief-magazine-requirements.md
//   - Plan: docs/plans/2026-04-17-003-feat-orion-brief-magazine-plan.md
 ...
 * <svg><defs><symbol id="orion-logo-core"> block. Every placement then
...
const LOGO_SYMBOL = (
  ...
  '<defs>' +
  '<symbol id="orion-logo-core" viewBox="0 0 64 64">' +
  ...
  '<path class="orion-ekg" d="M 6 32 L 20 32 L 24 24 L 30 40 L 36 22 L 42 38 L 46 32 L 56 32"/>' +
  '<circle class="orion-ekg-dot" cx="57" cy="32" r="1.8"/>' +
  '</symbol>' +
  '</defs>' +
  '</svg>'
);
...
function logoRef({ size, color }) {
  ...
  return (
    `<svg class="orion-logo" width="${size}" height="${size}" viewBox="0 0 64 64" ` +
    `aria-label="ORION"${styleAttr}>` +
    '<use href="#orion-logo-core"/>' +
    '</svg>'
  );
}
...
function digestRunningHead(dateShort, label) {
  return (
    ...
    logoRef({ size: 22 }) +
    ` · ORION Brief · ${escapeHtml(dateShort)} ·` +
    '</span>' +
...
function buildTrackedSourceUrl(raw, issueDate, rank) {
  try {
    const u = new URL(raw);
    if (!u.searchParams.has('utm_source')) u.searchParams.set('utm_source', 'orion');
...
function renderStoryPage({ story, rank, palette, pageIndex, totalPages, issueDate, followedSet }) {
  ...
  return (
    ...
    logoRef({ size: 28 }) +
    '<span class="mono">ORION Brief</span>' +
    '</div>' +
    `<div class="page-number mono">${pad2(pageIndex)} / ${pad2(totalPages)}</div>` +
    '</section>'
  );
...
function renderBackCover({ tz, pageIndex, totalPages, publicMode, refCode }) {
  const ctaHref = publicMode
    ? `https://orion.app/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`
    : 'https://orion.app';
  ...
  const metaLeft = publicMode
    ...
    : '<span class="mono">orion.app</span>';
...
const STYLE_BLOCK = `<style>
  :root {
    /* ORION brand palette — aligned with /pro landing + dashboard.
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .orion-logo { display: block; fill: none; stroke: currentColor; stroke-width: 2; stroke-linecap: round; }
  .orion-logo .orion-ekg { stroke-width: 2.4; }
  .orion-logo .orion-ekg-dot { fill: currentColor; stroke: none; }
  .logo-chrome {
  ...
  .cover .orion-logo .orion-ekg-dot,
  .story.dark .orion-logo .orion-ekg-dot { fill: var(--mint); }
  .digest .orion-logo .orion-ekg-dot,
  .story.light .orion-logo .orion-ekg-dot { fill: var(--sienna); }
  ...
  .orion-share:hover { background: rgba(20, 20, 20, 0.85); transform: translateY(-1px); }
  .orion-share[data-state="sharing"] { opacity: 0.6; cursor: progress; }
  .orion-share[data-state="copied"]::after { content: ' \u00b7 copied'; opacity: 0.75; }
  .orion-share[data-state="error"]::after { content: ' \u00b7 error'; opacity: 0.75; color: #ff9b9b; }
  ...
  .orion-public-strip a {
    color: var(--mint, #4ade80);
    text-decoration: none;
    border-bottom: 1px solid currentColor;
  }
  @media (max-width: 640px) {
    .orion-public-strip { font-size: 11px; padding: 0.7em 1em; gap: 0.6em; flex-wrap: wrap; }
  }
</style>`;
...
(function() {
  var btn = document.querySelector('.orion-share');
  if (!btn) return;
  ...
  btn.addEventListener('click', async function() {
    ...
    try {
      var shareTitle = 'ORION Brief';
      var shareText = 'My ORION Brief for today:';
      if (navigator.share) {
        try {
...
// index.html. Hosted magazine pages are served from orion.app
...
const UMAMI_LOADER = '<script async src="https://abacus.orion.app/script.js" data-website-id="e8800335-c853-46a8-8497-c993ed2f58bc" data-domains="orion.app,tech.orion.app,finance.orion.app,commodity.orion.app,happy.orion.app"></script>';
...
function redactForPublic(data) {
  ...
  return {
    ...
    stories: data.stories.map((s) => ({
      ...s,
      whyMatters: 'Subscribe to ORION Brief to see the full editorial on this story.',
    })),
  };
}
...
export function renderBriefMagazine(envelope, options = {}) {
  ...
  const title = `ORION Brief · ${escapeHtml(dateLong)}`;
  ...
  const publicStripHref = `https://orion.app/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`;
  const publicStripHtml = publicMode
    ? '<div class="orion-public-strip">'
      + '<span>ORION Brief \u00b7 shared issue</span>'
  ...
  const shareButtonHtml = shareUrl
    ? `<button class="orion-share" type="button" data-share-url="${escapeHtml(shareUrl)}" data-issue-date="${escapeHtml(date)}" aria-label="Share this brief">Share</button>`
    : '';
