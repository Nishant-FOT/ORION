...
export const INTERNAL_MCP_SIG_HEADER = 'X-ORION-MCP-Internal';

/** Header carrying the userId the signature claims to represent. */
export const INTERNAL_MCP_USER_ID_HEADER = 'X-ORION-MCP-User-Id';
...
export const INTERNAL_MCP_VERIFIED_HEADER = 'x-orion-mcp-internal-verified';
 ...
 * `x-orion-mcp-internal-verified` on requests rebuilt by the gateway after
...
export interface SignedInternalMcpHeaders {
  /** Header value: `<ts>.<base64url-sig>`. Sent as `X-ORION-MCP-Internal`. */
  signature: string;
  /** UserId the signature claims; sent as `X-ORION-MCP-User-Id`. */
  userId: string;
...
export interface VerifiedInternalMcpRequest {
  /** UserId carried by the verified `X-ORION-MCP-User-Id` header. */
  userId: string;
}
...
/**
 * Parse the `X-ORION-MCP-Internal` header value into `{ts, sigB64u}`.
 ...
 *
 * Reads `X-ORION-MCP-Internal` (`<ts>.<base64url-sig>`) and `X-ORION-MCP-User-Id`
...
export async function verifyInternalMcpRequest(
  ...
  // (ORION-R1 / ORION-T8 — deterministic since U7 shipped,
