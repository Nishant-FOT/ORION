...
} from '../../../../src/generated/server/orion/resilience/v1/service_server';
...
async function isSeedRefreshAuthorized(ctx: ServerContext): Promise<boolean> {
  ...
  const candidate = ctx.request.headers.get('X-ORION-Key') ?? '';
...
export const getResilienceRanking: ResilienceServiceHandler['getResilienceRanking'] = async (
  ...
  // (Pro bearer, ORION_VALID_KEYS, ORION_API_KEY) must keep the
