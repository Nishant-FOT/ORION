...
<Info>
**All users can connect via OAuth — no API key required.** Click _"Sign in with ORION"_ on the consent page. You may also paste a `wm_…` key directly.
</Info>

All users share the same MCP server and the same 39 tools. MCP clients are protected by the **60 requests/minute/key** limiter in this handler.

Users can connect Claude Desktop / Cursor / claude.ai without ever pasting an API key — see [Sign-in flow](#sign-in-flow) below. You may also paste a `wm_…` key on the consent page.

## Endpoints

| Endpoint | Purpose |
|----------|---------|
| `https://orion.app/mcp` | JSON-RPC server (Streamable HTTP transport; JSON responses by default, SSE responses when clients advertise `text/event-stream`; `initialize` defaults to protocol `2025-03-26`) |
| `https://api.orion.app/api/oauth/register` | Dynamic Client Registration (RFC 7591) |
| `https://api.orion.app/api/oauth/authorize` | OAuth 2.1 authorization endpoint (PKCE required) |
| `https://api.orion.app/api/oauth/token` | Token endpoint (authorization_code + refresh_token) |
| `https://api.orion.app/.well-known/oauth-authorization-server` | AS metadata (RFC 8414) |
| `https://orion.app/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |

Server identifier: `orion` v1.10.0.

### Protocol negotiation

ORION's static server card at `/.well-known/mcp/server-card.json` advertises protocol version `2025-06-18`, matching the current published capability card. The live `initialize` handshake is intentionally more conservative during the rollout:
...
ORION supports the Streamable HTTP POST flow with either JSON or SSE responses:
...
2. **Direct API key** — `X-ORION-Key: wm_0123456789abcdef0123456789abcdef01234567` for user-issued keys, or an opaque operator-issued enterprise key. Intended for server-side scripts, `curl`, and custom integrations. Do **not** send an API key as a `Bearer` token — it will fail OAuth resolution and return `401 invalid_token`.

OAuth bearer requests re-check the resolved MCP token and user binding before dispatch. Direct `X-ORION-Key` requests validate the configured API key and then use the per-key limiter.
...
Users can authorize MCP clients via their existing ORION account — no API key needed.

1. **Add the server URL** in your MCP client. The canonical entrypoint is:

   ```
   https://api.orion.app/mcp
   ```

   (`https://orion.app/mcp` works too — it proxies the same handler.)

2. **Click "Sign in with ORION"** on the consent page. This is the default CTA. You'll bounce through `orion.app/mcp-grant` (sign in if needed), then back to `api.orion.app/oauth/authorize-pro`, then to your client's redirect.
...
If you'd rather paste an API key, expand "Use API key instead" on the consent page and submit your `wm_` user key or operator-issued enterprise key — that path is unchanged.

### Rate limits

- **60 requests/minute** per API key or per user.
- `describe_tool`, `tools/list`, `prompts/list`, `prompts/get`, `resources/list`, `logging/setLevel`, `initialize`, `notifications/initialized`, and `ping` are not rate-limited beyond the per-minute cap.

Need higher-volume scripted access? `wm_…` MCP calls are protected by the 60/minute/key MCP throttle. For batch or high-volume workflows, prefer the REST/API endpoints or see [Plans & limits](#plans--limits).

### Connected MCP clients

Each authorization mints a separate row, so revoking Claude Desktop does not affect Cursor.

- Manage connected clients in **[Settings → Connected MCP clients](https://orion.app/settings)**.
...
| Plan | MCP access | Auth modes | Rate limit | Notes |
|------|------------|------------|------------|-------|
| **Free** | ✅ Yes | OAuth or `wm_…` key | 60 requests/minute | All users have full access. |
| **Pro** | ✅ Yes | OAuth or `wm_…` key | 60 requests/minute | Higher REST/API throughput. |
| **API Starter** | ✅ Yes | OAuth **or** `wm_…` key | 60 requests/minute | Standard developer-tier REST/API access. |
| **API Business** | ✅ Yes | OAuth **or** `wm_…` key | 60 requests/minute | Higher REST/API throughput. |
| **Enterprise** | ✅ Yes | OAuth **or** `wm_…` key | Custom | Custom SLA available. |

Per-minute throttling of 60 calls/minute/key (API-key clients) or 60/minute/user (OAuth contexts) protects against burst storms across all tiers. **The per-minute limiter runs before method dispatch, so every method (including `initialize`, `tools/list`, `prompts/list`, `resources/list`, `describe_tool`, etc.) counts toward 60/min.**
...
Hitting the per-minute limit returns JSON-RPC error `-32029` with a short `Retry-After`.
...
```bash
npx @modelcontextprotocol/inspector https://orion.app/mcp
```
...
In addition to the 39 tools, ORION exposes MCP prompts and resources so clients can discover common workflows and address stable data slices without inventing tool plans from scratch.
...
|--------------------|--------------|
| `orion://countries/{iso2}/risk` | Country risk score, component breakdown, travel advisory, and sanctions exposure. `{iso2}` is lowercase alpha-2, for example `de` or `us`. |
| `orion://chokepoints/{slug}/status` | Chokepoint transit summary and risk narrative. `{slug}` is one of the published kebab-case chokepoint slugs, such as `suez`, `strait-of-hormuz`, or `bab-el-mandeb`. |
| `orion://seed-meta/freshness` | Market-data bootstrap freshness envelope only: `cached_at` and `stale`. |
| `orion://markets/{symbol}/quote` | Single-symbol market quote slice. `{symbol}` is uppercase, for example `AAPL`, `GC=F`, or `BTC-USD`. |
...
Server-side with a direct API key — send it as `X-ORION-Key`, **not** as a bearer token.

```bash
WM_KEY="wm_0123456789abcdef0123456789abcdef01234567"

# 1. List tools
curl -s https://orion.app/mcp \
  -H "X-ORION-Key: $WM_KEY" \
...
Seed-level health per key: [status.orion.app](https://status.orion.app/).
...
- [Pro features](https://www.orion.app/pro)
