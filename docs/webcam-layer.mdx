...
| **Native client** | Keychain via Settings > API Keys | On-demand image fetches via sidecar |
...
| **Client — pinned store** | localStorage (permanent) | None (user-managed) | `orion-pinned-webcams` |
...
Pinned webcam data is stored in `localStorage` under the key `orion-pinned-webcams`. Each entry contains:
