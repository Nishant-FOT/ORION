...
ORION has four authentication modes. Which one applies depends on how you're calling.
...
| **API key** | `X-ORION-Key: wm_0123456789abcdef0123456789abcdef01234567` | Server-to-server, scripts, SDKs | User API keys cover entitled API access; operator-issued enterprise keys cover internal/partner access. |
...
- A valid `X-ORION-Key` (env-allowlisted from `ORION_VALID_KEYS`, or a user-owned `wm_`-prefixed key whose Convex record has the `apiAccess` entitlement), **or**
- A Clerk `Authorization: Bearer …` token whose user has role `pro` or Dodo entitlement tier ≥ 1.

From the browser, `premiumFetch()` (`src/services/premium-fetch.ts`) handles this by injecting one of those credentials on every request. Desktop app uses `ORION_API_KEY` from the runtime config. Server-to-server callers must send the header explicitly.
