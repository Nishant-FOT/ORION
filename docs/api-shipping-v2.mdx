...
The v2 shipping API is a **PRO-gated** read + webhook-subscription surface on top of ORION's chokepoint registry and AIS tracking data.

<Info>
All v2 shipping endpoints require `X-ORION-Key` (server-to-server). Browser origins are **not** trusted here — `validateApiKey` runs with `forceKey: true`.
</Info>
...
Auth: `X-ORION-Key` (forceKey: true) + PRO. Returns `401` / `403` otherwise.
...
X-ORION-Signature: sha256=<HMAC-SHA256(body, secret)>
X-ORION-Delivery-Id: <ulid>
X-ORION-Event: chokepoint.disruption
...
The delivery worker re-resolves `callbackUrl` before each send and re-checks against `PRIVATE_HOSTNAME_PATTERNS` to mitigate DNS rebinding. Delivery is at-least-once — consumers must handle duplicates via `X-ORION-Delivery-Id`.
