...
description: "Cloud API access is gated on a ORION_API_KEY. Without a valid key, the native client operates local-only (sidecar). A registration form collects emails via Convex DB for future key distribution."
---
## Overview

Cloud API access is gated on a `ORION_API_KEY`. Without a valid key, the native client operates local-only (sidecar). A registration form collects emails via Convex DB for future key distribution.
...
|----------|-------------|---------|
| `ORION_VALID_KEYS` | Comma-separated list of valid API keys | `wm_abc123def456,wm_xyz789` |
...
Add to `ORION_VALID_KEYS` in Vercel dashboard (comma-separated, no spaces).
...
| `appVersion` | string? | Native client version |

Indexed by `normalizedEmail` for duplicate detection.

## Security Model

### Client-side (native client)

- `installRuntimeFetchPatch()` checks `ORION_API_KEY` before allowing cloud fallback
...
- `api/_api-key.js` validates `X-ORION-Key` header on sebuf routes
- **Origin-aware**: native client origins (`tauri.localhost`, `tauri://`, `asset://`) require a key
- Web origins (`orion.app`) pass through without a key
- Non-native origin with key header: key is still validated
- Invalid key returns `401 { error: "Invalid API key" }`

### CORS

`X-ORION-Key` is allowed in both `server/cors.ts` and `api/_cors.js`.
...
- [ ] Set `ORION_VALID_KEYS` in Vercel
...
| `src/services/runtime-config.ts` | `ORION_API_KEY` type, validation, `secretsReady` |
...
| `server/cors.ts` / `api/_cors.js` | CORS headers with `X-ORION-Key` |
| `src/components/ORIONTab.ts` | Settings UI for key + registration |
