...
description: "Installation instructions, tech stack overview, project structure, API dependencies, and setup guide for ORION."
---
This guide covers everything you need to set up ORION locally, from prerequisites and installation to understanding the project structure and API dependencies.
...
ORION has three setup tracks depending on what you're doing.
...
```bash
git clone https://github.com/orion-energy-resilience/orion.git
cd orion
npm install
...
├── main.ts                   # Entry point
├── components/
│   ├── DeckGLMap.ts          # WebGL map with deck.gl + MapLibre
...
├── create-checkout.ts        # Checkout endpoint (hand-written operational)
├── customer-portal.ts        # Customer portal endpoint (hand-written operational)
...
├── orion/<domain>/    # 34 proto-backed domain handler implementations
...
Thanks to **[@fai9al](https://github.com/fai9al)** for the inspiration and original PR that led to the Tech Monitor configuration.
