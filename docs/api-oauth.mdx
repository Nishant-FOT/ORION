...
description: "Dynamic client registration, authorization, and token exchange endpoints that back the ORION MCP server."
---

ORION runs a minimal OAuth 2.1 authorization server whose only client-facing purpose today is **granting access to the MCP server** at `/api/mcp`. It implements:
...
`/.well-known/oauth-protected-resource` currently advertises the public resource scope `mcp`. Authorization-code grants return the internal scope value `mcp`; legacy API-key grants and `client_credentials` return `mcp`.
...
Starts the OAuth flow. Renders a consent page that redirects to the sign-in page, then issues an authorization code.
...
Tokens are bound to the user's account and re-check authorization on every call.
