---
title: "Quickstart"
description: "Make your first ORION API call in under a minute — curl, JavaScript, and Python."
---

## 1. Get an API key

PRO subscribers get an API key from [orion.app/pro](https://www.orion.app/pro). User-issued keys are exactly `wm_` followed by 40 lowercase hex characters.
...
```bash
curl -s 'https://api.orion.app/api/conflict/v1/list-conflict-events?limit=5' \
  -H 'X-ORION-Key: wm_0123456789abcdef0123456789abcdef01234567'
```
...
const res = await fetch(
  'https://api.orion.app/api/market/v1/get-quotes?symbols=AAPL,TSLA',
  { headers: { 'X-ORION-Key': process.env.WM_KEY } }
);
...
r = httpx.get(
    "https://api.orion.app/api/economic/v1/get-fred-series",
    params={"series_id": "FEDFUNDS"},
    headers={"X-ORION-Key": os.environ["WM_KEY"]},
)
r.raise_for_status()
print(r.json())
```
...
```bash
curl -s 'https://api.orion.app/api/bootstrap' \
  -H 'X-ORION-Key: wm_0123456789abcdef0123456789abcdef01234567'
```
