...
description: "Design principles, intelligence tradecraft, algorithmic decisions, and implementation patterns behind ORION."
---

> **Looking for the system reference?** See [`ARCHITECTURE.md`](https://github.com/orion-energy-resilience/orion/blob/main/ARCHITECTURE.md) for deployment topology, directory layout, caching layers, CI/CD, and contributor how-to guides.
...
| **Defense in depth**                | CORS origin allowlist, domain-allowlisted RSS proxy, server-side API key isolation, token-authenticated sidecar, input sanitization with output encoding, IP rate limiting on AI endpoints.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **Run anywhere**                    | Same codebase produces six specialized configurations (geopolitical, tech, finance, commodity, happy, energy) from a single Vercel deployment. Service worker caches map tiles for offline use while keeping intelligence data always-fresh (NetworkOnly).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
ORION is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
- **WebView compatibility** — native builds run in WKWebView (macOS) and WebKitGTK (Linux), which have idiosyncratic behavior around drag-and-drop, clipboard, autoplay, and memory management. Direct DOM manipulation makes it possible to work around these platform quirks without fighting framework abstractions
...
**Negative sentinel caching** — when a Redis key contains no data, the bootstrap endpoint stores a `__ORION_NEG__` sentinel in the response rather than omitting the key. This allows consumers to distinguish between "data not yet loaded" (key absent from hydration) and "data source has no content" (negative sentinel), preventing unnecessary RPC fallback calls for empty data sources.
...
All six dashboard configurations (ORION, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor, Energy Monitor) serve from a **single Vercel deployment**. The active configuration is stored in `localStorage['orion-variant']` and can be switched without rebuilding. The variant selector in the header bar toggles between configurations.

This architecture provides several advantages:

- **Instant switching** — users toggle configurations in the header bar without a full page navigation
- **Shared CDN cache** — the static SPA assets are identical across configurations; only runtime configuration differs. CDN cache hit rates are 4× higher than with separate deployments
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `orion.app`, preview on `*.vercel.app`, disabled on localhost and native builds).
...
Transactions are sampled at 10% to balance observability with cost. Release tracking (`orion@{version}`) enables regression detection across deployments.
...
The Service Status panel tracks the operational health of external services that ORION users may depend on.
