...
All six dashboard variants (ORION, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor, Energy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
...
| `orion.app` (default) | `full` |

On the desktop app, the variant is stored in `localStorage['orion-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `orion.app`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
