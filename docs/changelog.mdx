...
- **CDN cache bypass closed**: `CDN-Cache-Control` header now only emitted for trusted origins (orion.app, Vercel previews, Tauri). No-origin server-side requests always invoke the edge function so `validateApiKey` runs, preventing a cached 200 from being served to external scrapers.
...
- **Finance variant**: Added a dedicated market-first variant (`finance.orion.app`) with finance/trading-focused feeds, panels, and map defaults
