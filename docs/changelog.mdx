---
title: "Changelog"
description: "All notable changes to ORION, organized by release."
rss: true
---

All notable changes to ORION are documented here. Subscribe via [RSS](/changelog/rss.xml) to stay updated.
...
- **Sebuf API migration (#3207)** — scenario + supply-chain endpoints moved to the typed sebuf contract. RPC URLs now derive from method names; five renamed v1 URLs remain live as thin aliases so existing integrations keep working:
  ...
  Aliases retire at the next v1→v2 break (tracked in [#3282](https://github.com/orion-energy-resilience/orion/issues/3282)).
...
- **CDN cache bypass closed**: `CDN-Cache-Control` header now only emitted for trusted origins (orion.app, Vercel previews, native clients). No-origin server-side requests always invoke the edge function so `validateApiKey` runs, preventing a cached 200 from being served to external scrapers.
...
- Trade Policy panel WTO gate changed from panel-wide to per-tab, so Revenue tab works without WTO API key (#1663)
...
- Bootstrap timeouts restored (5s/8s) while keeping aggressive web timeouts (1.2s/1.8s) (#1653)
...
- **Map**: prevent ghost layers rendering without a toggle (#1264), DeckGL layer toggles getting stuck (#1248), auto-fallback to OpenFreeMap on basemap failure (#1109), CORS fallback for Carto basemap (#1142), use CORS-enabled R2 URL for PMTiles in native builds (#1119), CII Instability layer disabled in 3D mode (#1292)
- **Layout**: reconcile ultrawide zones when map is hidden (#1246), keep settings button visible on scaled widths (#1249), exit fullscreen before switching variants (#1253), apply map-hidden layout class on initial load (#1087), preserve panel column position across refresh (#1170, #1108, #1112)
...
- Story header uses full domain ORION.APP (#799)
...
- AVIATIONSTACK_API key in settings (#553)
...
- **Edge caching**: Cloudflare edge caching for proxy.orion.app (#478) and api.orion.app (#471)
...
- **Windows CORS for Tauri** — allow `http://` origin from `tauri.localhost` for Windows builds (#262)
...
**Local LLM Support (Ollama / LM Studio)** — Run AI summarization entirely on your own hardware with zero cloud dependency. The native client auto-discovers models from any OpenAI-compatible local inference server (Ollama, LM Studio, llama.cpp, vLLM) and populates a selection dropdown. A 4-tier fallback chain ensures summaries always generate: Local LLM → Groq → OpenRouter → browser-side T5. Combined with the native client, this enables fully air-gapped intelligence analysis where no data leaves your machine.
...
- **Consolidated keychain vault** — all secrets stored as a single JSON blob in one OS keychain entry (`secrets-vault`), reducing macOS Keychain authorization prompts from 20+ to exactly 1 on app startup. One-time auto-migration from individual entries with cleanup
...
- **Privacy &amp; Offline Architecture** documentation — README now details the three privacy levels: full cloud, native client with cloud APIs, and air-gapped local with Ollama
...
- Native client architecture uses consolidated vault instead of per-key keychain entries
- README expanded with ~85 lines of new content covering local LLM support, privacy architecture, summarization chain internals, and native client readiness framework
...
- **Mobile detection**: use viewport width only for mobile detection; touch-capable notebooks (e.g. ROG Flow X13) now get expanded layout (#113)
...
- **Finance variant**: Added a dedicated market-first configuration with finance/trading-focused feeds, panels, and map defaults
- **Finance profile**: Added finance-specific config and build profile for Tauri packaging
...
- **Finance map parity**: Finance configuration now runs first-class Deck.GL map/layer behavior
...
- **Mobile UX**: Improved mobile experience
...
- **Node.js bundled in installer**: Sidecar no longer requires system Node.js
...
- **CORS for native builds**: Fixed CORS issues for native client requests
...
- **Windows console window**: Hide the `node.exe` console window that appeared alongside the app on Windows
...
- **Windows sidecar crash**: Strip `\\?\` UNC extended-length prefix from paths before passing to Node.js — `resource_dir()` on Windows returns UNC-prefixed paths that cause `EISDIR: lstat 'C:'` in Node.js module resolution
...
- **Resilient keychain reads**: Use `Promise.allSettled` in `loadSecrets` so a single key failure doesn't discard all loaded secrets
- **Settings window capabilities**: Add `"settings"` to capabilities window list for core plugin permissions
...
- **CORS hardening**: Tighten Vercel preview deployment regex to block origin spoofing (`orionEVIL.vercel.app`)
...
- Download app slide-in banner for web visitors
...
- Hide config panel on web, fix irrelevant prediction markets
...
Initial v2.2 release with multi-variant support (World + Tech) and comprehensive geopolitical intelligence features.
