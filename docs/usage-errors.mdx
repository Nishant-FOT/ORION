...
| `403` | Authenticated but not entitled | No — check credentials |
...
| `502` | Upstream / Convex failure | Yes — exponential backoff |
...
- Every edge response includes `x-vercel-id` and `x-orion-deploy` headers — include those when reporting issues.
- Sentry alerts forward to [status.orion.app](https://status.orion.app/).
