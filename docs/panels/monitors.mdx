...
- **Availability**: registered and enabled by default in the **full/geopolitical** (`priority: 2`), **tech** (`priority: 2`), **finance** (`priority: 2`), and **commodity** (`priority: 2`) configurations. Not present in the happy configuration. Source: `monitors` entries in `FULL_PANELS` (`:62`), `TECH_PANELS` (`:282`), `FINANCE_PANELS` (`:473`), `COMMODITY_PANELS` (`:775`) of `src/config/panels.ts`.
...
**Free.** No `premium` flag in any registration.
