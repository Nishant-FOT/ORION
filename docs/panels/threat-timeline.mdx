...
- **Availability**: registered and enabled by default in the **full/geopolitical** configuration only. It is not present in the tech, finance, commodity, or happy configurations. Source: `'threat-timeline'` in `FULL_PANELS` of `src/config/panels.ts`.
...
Threat Timeline is **free** on the web. It has no `premium` flag in the full configuration registration.
