...
All free. Source curation lives in `src/config/variants/full.ts` (and equivalents).
...
## Tech / startup streams (tech configuration)

The tech configuration ships a large family of content streams covering the startup ecosystem, infrastructure trends, and tech-policy signals. They appear in the tech configuration's panel grid and are reachable from Cmd+K.
...
## Finance streams (finance configuration)

The finance configuration's content-stream panels are tighter and market-focused.
...
## Energy streams (energy configuration)

The energy configuration ships a focused set of content streams for live energy
...
- **Free by default** — all news-feed panels are freely accessible.
