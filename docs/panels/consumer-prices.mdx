...
- **Market bar** — a button per supported market (default `AE`; the full list comes from `MARKETS` in `src/services/consumer-prices`). Selecting a market implicitly sets the basket to that market's `essentials-<code>` configuration (or the default basket when the `all` aggregate view is active) — there is no separate basket picker.
...
Settings persist across sessions in `localStorage` under `orion-consumer-prices-v1`.
...
- **Availability**: enabled by default in the **finance** and **commodity** configurations. Registered but disabled (opt-in) in the **full/geopolitical** configuration. Not registered in the tech or happy configurations. Source: `FINANCE_PANELS`, `COMMODITY_PANELS`, `FULL_PANELS` in `src/config/panels.ts`.
...
Consumer Prices is **free**. The panel is registered without a `premium` flag in `src/config/panels.ts`, so anyone — including unauthenticated readers — can see it on configurations where it is enabled.
