...
- **Availability**: registered and enabled by default in the **full/geopolitical** (`priority: 2`), **tech** (`priority: 2`), **finance** (`priority: 2`), and **commodity** (`priority: 2`) configurations. Not present in the happy configuration. Source: `'airline-intel'` entries in `FULL_PANELS` (`:104`), `TECH_PANELS` (`:280`), `FINANCE_PANELS` (`:471`), and `COMMODITY_PANELS` in `src/config/panels.ts`.
