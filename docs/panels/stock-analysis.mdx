...
- **Availability**: registered and enabled by default in the **full/geopolitical** and **finance** configurations — locked in both. Not present in the tech, commodity, or happy configurations. Source: `'stock-analysis'` entries in `FULL_PANELS` and `FINANCE_PANELS` at `src/config/panels.ts:49` and `:422`.
