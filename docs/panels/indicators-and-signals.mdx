...
ORION ships many **tile-style** indicator, correlation, and signal panels — compact surfaces that show one or two readings each and are designed to be composed alongside the deep-dive panels. This page is the collective reference so each tile is findable without writing a dedicated page per panel.
...
- All panels above are **free** unless noted otherwise.
- Compact tiles are registered across multiple configurations; the per-configuration registration determines whether a tile is enabled by default. Check `src/config/panels.ts` directly when confirming availability.
