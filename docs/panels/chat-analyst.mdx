...
**PRO (hard-locked).** `premium: 'locked'` in `src/config/panels.ts:52`. `isPanelEntitled` unlocks it when **either** a valid `ORION_API_KEY` is present **or** `isPro` is true (`src/config/panels.ts:973` — `apiKeyPanels` allowlist includes `chat-analyst`).
