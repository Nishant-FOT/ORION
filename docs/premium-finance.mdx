...
description: "Premium finance provides stock-analysis capabilities within ORION, originally inspired by a standalone daily_stock_analysis project but now fully integrated."
---
Premium finance provides stock-analysis capabilities within ORION. These features were originally inspired by a standalone `daily_stock_analysis` project but are now fully integrated into the ORION codebase.
...
3. Finance premium panels are called through `MarketService`.
...
- [analyze-stock.ts](https://github.com/orion-energy-resilience/orion/blob/main/server/orion/market/v1/analyze-stock.ts)
- [get-stock-analysis-history.ts](https://github.com/orion-energy-resilience/orion/blob/main/server/orion/market/v1/get-stock-analysis-history.ts)
- [backtest-stock.ts](https://github.com/orion-energy-resilience/orion/blob/main/server/orion/market/v1/backtest-stock.ts)
- [list-stored-stock-backtests.ts](https://github.com/orion-energy-resilience/orion/blob/main/server/orion/market/v1/list-stored-stock-backtests.ts)
- [premium-stock-store.ts](https://github.com/orion-energy-resilience/orion/blob/main/server/orion/market/v1/premium-stock-store.ts)

Primary contracts:

- [analyze_stock.proto](https://github.com/orion-energy-resilience/orion/blob/main/proto/orion/market/v1/analyze_stock.proto)
- [get_stock_analysis_history.proto](https://github.com/orion-energy-resilience/orion/blob/main/proto/orion/market/v1/get_stock_analysis_history.proto)
- [backtest_stock.proto](https://github.com/orion-energy-resilience/orion/blob/main/proto/orion/market/v1/backtest_stock.proto)
- [list_stored_stock_backtests.proto](https://github.com/orion-energy-resilience/orion/blob/main/proto/orion/market/v1/list_stored_stock_backtests.proto)
- [service.proto](https://github.com/orion-energy-resilience/orion/blob/main/proto/orion/market/v1/service.proto)

### Frontend Surfaces

Panels:

- [StockAnalysisPanel.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/components/StockAnalysisPanel.ts)
- [StockBacktestPanel.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/components/StockBacktestPanel.ts)
- [DailyMarketBriefPanel.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/components/DailyMarketBriefPanel.ts)

Services and loading:

- [stock-analysis.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/services/stock-analysis.ts)
- [stock-analysis-history.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/services/stock-analysis-history.ts)
- [stock-backtest.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/services/stock-backtest.ts)
- [daily-market-brief.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/services/daily-market-brief.ts)
- [data-loader.ts](https://github.com/orion-energy-resilience/orion/blob/main/src/app/data-loader.ts)

---

## Stock Analysis

The premium stock-analysis engine was originally a TypeScript port of core logic from the standalone project, now fully native to ORION.
...
ORION still lacks a general-purpose relational backend, so premium finance currently uses Redis as the backend-owned source of truth.
...
- browser or native client cache is no longer the canonical history
...
Backtesting in ORION is intentionally tied to stored analysis records, not just a raw signal replay.
...
This is a ORION native feature, not a direct port of the original project's scheduler/automation system.
...
- [gateway.ts](https://github.com/orion-energy-resilience/orion/blob/main/server/gateway.ts)
- [api/_api-key.js](https://github.com/orion-energy-resilience/orion/blob/main/api/_api-key.js)
...
- Finnhub for broader market data already used elsewhere in ORION
- Google News RSS as the baseline stock-news fallback
- the shared LLM provider chain in [llm.ts](https://github.com/orion-energy-resilience/orion/blob/main/server/_shared/llm.ts)
...
This feature is valid and production-usable within ORION's current architecture, but some boundaries remain explicit:
...
These boundaries keep the feature aligned with ORION's architecture.
