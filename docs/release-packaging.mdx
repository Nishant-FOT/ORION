---
title: "Release Packaging Guide (Local, Reproducible)"
description: "This guide provides reproducible local packaging steps for ORION."
---
This guide provides reproducible local packaging steps for ORION.
...
Install dependencies (this also installs the pinned Tauri CLI used by build scripts):

```bash
npm ci
```

All build scripts call the local `tauri` binary from `node_modules/.bin`; no runtime `npx` package download is required after `npm ci`.
...
Before running packaging in CI or managed networks, verify connectivity and proxy config:
...
- `src-tauri/tauri.conf.json` → `ORION` / `orion`
...
5. Validate identity:
   - Window title and product name match expected configuration.
