---
title: "Rate Limits"
description: "Per-endpoint, per-key, and per-IP rate limits across the ORION API surface."
---
...
| `POST /api/leads/v1/register-interest` | 5 | 60 min | Per IP + Turnstile (native client sources require signed HMAC bypass) |
| `POST /api/leads/v1/submit-contact` | 3 | 60 min | Per IP + Turnstile |

Other write endpoints (`/api/brief/share-url`, `/api/notification-channels`, etc.) fall back to the default per-IP limit above.
