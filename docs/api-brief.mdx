...
ORION composes a per-user intelligence brief on Railway, stores each
...
<Info>
All read routes require a valid session, except the public share route (`/api/brief/public/{hash}`).
</Info>
...
Single-shot completion endpoint used by embedded widget iframes. Auth via `X-ORION-Key` (partner keys). Rate-limited per key.
