...
- **Pro** — sign in with your ORION account, no key to manage. 50 quota-consuming calls per UTC day.
...
Upgrade or generate a key at [orion.app/pro](https://www.orion.app/pro). The rest of this guide assumes Claude Desktop + OAuth (the simplest path); if you'd rather paste a `wm_…` key into a `curl` script, jump to [Server-side curl](#server-side-curl) at the bottom.
