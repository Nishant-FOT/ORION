...
| Pattern | Matches |
| --- | --- |
| `(*.)?orion.app` | Production + subdomains |
| `orion-*-elie-*.vercel.app` | Vercel preview deploys |
| `localhost:*` / `127.0.0.1:*` | Local development only when `NODE_ENV !== "production"` |
| `tauri.localhost:*` / `*.tauri.localhost:*` | Native client (Tauri v2) |
| `tauri://localhost` / `asset://localhost` | Native client (Tauri v2 asset protocol) |
...
2. Update the test in `tests/api/_cors.test.mjs`.
...
- `Content-Type`
- `Authorization`
- `X-ORION-Key` (API key for native client/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
