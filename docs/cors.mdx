...
| Pattern | Matches |
| --- | --- |
| `(*.)?orion.app` | Production + subdomains (`tech.`, `finance.`, etc.) |
...
- `Content-Type`
- `Authorization`
- `X-ORION-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
