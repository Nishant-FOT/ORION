...
description: "How ORION collects RSS/Atom feeds, classifies news, scores importance, tracks stories, filters briefs, uses LLMs, and controls bias."
---

## Scope

This page is the canonical public methodology for the news-feed digest,
daily digest notifications, and ORION Brief. It covers the automated
...
Feed inventory lives in `server/orion/news/v1/_feeds.ts`.
...
`energy` is a client-feed configuration, but it is not a separate server digest
variant yet: `listFeedDigest` accepts only the variants above and normalises
any other request, including `variant=energy`, to `full`. Energy panels still
have client-side feed inventory for energy headlines, energy markets, and
chokepoints/routes; categories absent from the server digest use the direct
per-feed fallback path instead of a dedicated
...
`server/orion/news/v1/_classifier.ts`.
...
ORION Brief uses two LLM surfaces:
...
- Feed inventory and digest build: `server/orion/news/v1/_feeds.ts`,
  `server/orion/news/v1/list-feed-digest.ts`
- Keyword classifier: `server/orion/news/v1/_classifier.ts`
- API contract: `proto/orion/news/v1/list_feed_digest.proto`
