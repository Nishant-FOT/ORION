...
(proto `orion.intelligence.v1.GetRiskScoresResponse`).

These weights are **editorial** — authored by the ORION intelligence
...
> version ([#4200](https://github.com/orion-energy-resilience/orion/issues/4200)), and
...
> [#3789](https://github.com/orion-energy-resilience/orion/issues/3789) by keeping the
...
> `server/orion/intelligence/v1/_risk-config.ts`, an update to this
...
`server/orion/intelligence/v1/get-risk-scores.ts`. Each `CiiScore`
therefore exposes:
...
are all defined in
[`server/orion/intelligence/v1/_risk-config.ts`](../../server/orion/intelligence/v1/_risk-config.ts).
Their **rationale**:
