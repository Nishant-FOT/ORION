...
description: "Bootstrap, health, versioning, cache-purge, and user preferences — the plumbing endpoints every ORION client talks to."
---
...
- **Auth**: browser `orion-session` cookie OR `X-ORION-Key`
...
Returns the latest **GitHub Release** of `orion-energy-resilience/orion`. Used by native clients to detect a newer published release and prompt the user to update. It is **not** the currently-deployed Vercel commit.

```json
{
  "version": "2.6.7",
  "tag": "v2.6.7",
  "url": "https://github.com/orion-energy-resilience/orion/releases/tag/v2.6.7",
  "prerelease": false
}
```
...
Per-user dashboard preferences (layout, toggles, filters). Session required. Backed by Convex.
...
Invalidates a user's API key cache. Internal — requires `RELAY_SHARED_SECRET`.
...
Redirects to the matching asset on the latest GitHub release of `orion-energy-resilience/orion`. Returns `302` to the asset URL on success, or `302` to [releases/latest](https://github.com/orion-energy-resilience/orion/releases/latest) on any failure (unknown platform, no match, GitHub error).
...
Browser callers must pass Turnstile. Native clients using `source: "desktop-settings"` bypass Turnstile only when the request is authenticated with the shared secret:

- `X-ORION-Desktop-Timestamp`: Unix epoch milliseconds, within 5 minutes of server time.
- `X-ORION-Desktop-Signature`: `sha256=<hex HMAC-SHA256>`.

The HMAC input is `<timestamp>\n<canonical JSON>`, where canonical JSON contains `email`, `source`, `appVersion`, `referredBy`, `website`, and `turnstileToken` in that order. Configure `ORION_DESKTOP_SHARED_SECRET` on both the native client and the cloud API. During rollout, `WM_DESKTOP_AUTH_ALLOW_LEGACY=true` only accepts unsigned legacy requests while the cloud API has no `ORION_DESKTOP_SHARED_SECRET` configured. Once the cloud secret is set, native requests fail closed unless they include a valid signature, still subject to the tighter rate limit.
