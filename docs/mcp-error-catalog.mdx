...
description: "Every error shape ORION's MCP server can emit — JSON-RPC codes, HTTP statuses, and soft-behavior envelopes — with triggers, payloads, and what the client should do."
---
...
- **Both 401 paths set `WWW-Authenticate`** with `realm="orion"` and a `resource_metadata` pointer at `/.well-known/oauth-protected-resource`. RFC 9728-aware clients (Claude Desktop, MCP Inspector) bounce through the OAuth flow on this header without further intervention.

## JSON-RPC error codes

| Code      | Meaning (this server)                                                  | Paired HTTP status        | Recovery                                                              |
|-----------|------------------------------------------------------------------------|---------------------------|-----------------------------------------------------------------------|
| `-32001`  | Unauthenticated or invalid token                                       | **401**                   | Re-authenticate via OAuth or fix the `X-ORION-Key` header      |
...
Fires from six `api/mcp/auth.ts` emission sites, always paired with HTTP **401** and a `WWW-Authenticate` header. Those sites collapse to four user-visible triggers, in order of how clients hit them:

1. **No `Authorization` bearer AND no `X-ORION-Key`** — the client called `/mcp` with no credentials.
...
3. **`X-ORION-Key: <key>` but `<key>` isn't in the valid set** — the API key is wrong.
4. **OAuth token resolves but the MCP token row is missing or cross-bound** — the `mcpTokenId` no longer maps to the userId. Typically a revoke from Settings → Connected MCP clients.
...
```json
{
  ...
  "error": {
    "code": -32001,
    "message": "Authentication required. Use OAuth (/oauth/token) or pass your API key via X-ORION-Key header."
  }
}
```
...
|------------------------|------------------------------------------------------------------|-----------------------|
| `X-ORION-Key`   | `Rate limit exceeded. Max 60 requests per minute per API key.`   | `api/mcp/auth.ts:249` |
...
| **401** | JSON-RPC envelope         | `-32001`         | Missing/invalid/expired credentials. `WWW-Authenticate` header set. |
