...
const installPaintObservers = async (page: Page): Promise<void> => {
  await page.addInitScript(() => {
    localStorage.setItem('orion-layer-warning-dismissed', 'true');
    localStorage.setItem('orion-pro-banner-launched-dismissed', String(Date.now()));
    localStorage.setItem('orion-mission-preset-dismissed-v1', '1');
...
test.describe('pre-hydration dashboard shell', () => {
  ...
  test('paints contentful HTML before the dashboard bundle hydrates', async ({ page }) => {
    ...
    try {
      ...
      await expect.poll(async () => page.evaluate(() => {
        if (document.documentElement.classList.contains('orion-layout-hydrated')) return -1;
      ...
      const preHydration = await page.evaluate(() => {
        ...
        return {
          ...
          hydrated: document.documentElement.classList.contains('orion-layout-hydrated'),
      ...
      expect(preHydration.appHeadingText).toContain('ORION');
      expect(preHydration.badgeAriaLabel).toBeNull();
      expect(preHydration.focusableCount).toBe(0);
      expect(preHydration.shellText).toContain('ORION');
      ...
      await expect(page.locator('body > h1.app-heading')).toContainText('ORION');
      await expect.poll(async () => page.evaluate(() => (
        document.documentElement.classList.contains('orion-layout-hydrated')
      ))).toBe(true);
...
test.describe('dashboard shell without JavaScript', () => {
  ...
  test('hides the JS-only shell and keeps the no-JS content scrollable', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText('ORION Pro');
