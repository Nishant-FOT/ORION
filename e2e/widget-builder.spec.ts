...
test.describe('AI widget builder', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript((key) => {
      if (!sessionStorage.getItem('__widget_e2e_init__')) {
        ...
        localStorage.setItem('orion-variant', 'happy');
        localStorage.setItem('orion-widget-key', key);
        sessionStorage.setItem('__widget_e2e_init__', '1');
        return;
      }

      if (!localStorage.getItem('orion-widget-key')) {
        localStorage.setItem('orion-widget-key', key);
      }
    }, widgetKey);
  });
  ...
  test('creates a widget through the live modal flow and persists it after reload', async ({ page }) => {
    ...
    await installWidgetAgentMocks(
      ...
      [
        {
          delayMs: 250,
          endpoint: '/rpc/orion.markets.v1.MarketsService/GetCommodities',
          title: 'Oil vs Gold',
          html: createHtml,
        },
      ],
    ...
    await expect(preview.locator('.orion-widget-shell')).toBeVisible();
    await expect(preview.locator('.orion-widget-generated')).toBeVisible();
    ...
    await expect(widgetPanel.locator('.orion-widget-shell')).toBeVisible();
    await expect(widgetPanel.locator('.orion-widget-generated')).toBeVisible();

    const containment = await widgetPanel.locator('.orion-widget-generated').evaluate((element) => {
    ...
    const storedWidgets = await page.evaluate(() => {
      return JSON.parse(localStorage.getItem('orion-custom-widgets') || '[]') as Array<{ title: string }>;
    });
  ...
  test('supports modify, keeps session history, exposes touch-sized controls, and cleans storage on delete', async ({ page }) => {
    ...
    await installWidgetAgentMocks(page, [
      ...
      {
        endpoint: '/rpc/orion.aviation.v1.AviationService/GetAirportDelays',
    ...
    const storedWidgetMeta = await page.evaluate(() => {
      const widgets = JSON.parse(localStorage.getItem('orion-custom-widgets') || '[]') as Array<{
        id: string;
        title: string;
      }>;
    ...
    await page.evaluate((widgetId: string) => {
      localStorage.setItem('orion-panel-spans', JSON.stringify({ [widgetId]: 2 }));
      localStorage.setItem('orion-panel-col-spans', JSON.stringify({ [widgetId]: 3 }));
    }, storedWidgetMeta!.id);
    ...
    const cleanedStorage = await page.evaluate(() => {
      return {
        widgets: localStorage.getItem('orion-custom-widgets'),
        rowSpans: localStorage.getItem('orion-panel-spans'),
        colSpans: localStorage.getItem('orion-panel-col-spans'),
      };
    });
...
test.describe('AI widget builder — PRO tier', () => {
    ...
    await page.addInitScript(
      ({ wKey, pKey }: { wKey: string; pKey: string }) => {
        if (!sessionStorage.getItem('__widget_pro_e2e_init__')) {
          ...
          sessionStorage.clear();
          localStorage.setItem('orion-variant', 'happy');
          localStorage.setItem('orion-widget-key', wKey);
          localStorage.setItem('orion-pro-key', pKey);
          sessionStorage.setItem('__widget_pro_e2e_init__', '1');
          return;
        }
        if (!localStorage.getItem('orion-widget-key')) localStorage.setItem('orion-widget-key', wKey);
        if (!localStorage.getItem('orion-pro-key')) localStorage.setItem('orion-pro-key', pKey);
      },
  ...
  test('creates a PRO widget: iframe renders with allow-scripts sandbox and PRO badge visible', async ({
    ...
    // PRO preview shows iframe (not basic .orion-widget-generated)
  ...
  test('PRO widget stores HTML in orion-pro-html-{id} key and tier:pro in main array', async ({
    page,
  }) => {
    ...
    const storage = await page.evaluate(() => {
      ...
      const proHtmlStored = localStorage.getItem(`orion-pro-html-${entry.id}`);
