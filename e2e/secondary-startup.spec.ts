...
const SECONDARY_STARTUP_REQUEST =
  /(?:abacus\.orion\.app\/script\.js|fonts\.googleapis\.com\/css2|fonts\.gstatic\.com\/s\/(?:nunito|tajawal)|clerk\.orion\.app|ingest\.us\.sentry\.io|static\.cloudflareinsights\.com|\/_vercel\/insights\/script\.js|www\.youtube\.com\/iframe_api)/;
...
test.describe('secondary startup work', () => {
  ...
  test('defers secondary startup requests until idle startup runs', async ({ page }) => {
    ...
    await expect.poll(() => secondaryRequests.some((url) => url.includes('abacus.orion.app/script.js'))).toBe(true);
