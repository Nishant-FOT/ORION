...
async function expectCurrentMapRenderer(page: Page): Promise<void> {
  await expect(page.locator('.orion-embed-map')).toHaveClass(/(?:^|\s)(deckgl-mode|globe-mode|svg-mode)(?:\s|$)/);
...
async function expectCurrentMapRendererInFrame(frame: FrameLocator, page: Page): Promise<void> {
  ...
  await expect(frame.locator('.orion-embed-map')).toHaveClass(/(?:^|\s)(deckgl-mode|globe-mode|svg-mode)(?:\s|$)/);
...
async function serveThirdPartyHostPage(html: string): Promise<{ url: string; close: () => Promise<void> }> {
  ...
  return {
    url: `http://127.0.0.1:${address.port}/orion-host`,
...
test.describe('public map embed', () => {
  ...
  test('renders the map-only embed route with attribution', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('.orion-embed-attribution')).toHaveText('Live map by ORION');
  ...
  test('loads inside a third-party iframe host page', async ({ page, baseURL }, testInfo) => {
    ...
    await page.route('https://api.orion.app/api/**', async (route) => {
      ...
      <html>
        <body style="margin:0;background:#f7f7f7">
          <main style="max-width:860px;margin:24px auto;font-family:sans-serif">
            <h1>Host page</h1>
            <iframe id="wm" src="${embedUrl}" title="ORION live map" style="width:100%;height:420px;border:0;display:block"></iframe>
          </main>
        </body>
      </html>
    `);
    ...
    try {
      ...
      await expect(frame.locator('.orion-embed-attribution')).toHaveText('Live map by ORION');
      ...
      const mapClass = await frame.locator('.orion-embed-map').getAttribute('class') ?? '';
  ...
  test('does not fetch live conflict markers for the DeckGL embed renderer', async ({ page }) => {
    ...
    await expect(page.locator('.orion-embed-map')).toHaveClass(/(?:^|\s)deckgl-mode(?:\s|$)/);
    await expect(page.locator('body')).toHaveAttribute('data-embed-ready', 'true');
    expect(conflictRequests).toHaveLength(0);
  });
});
