...
async function installCleanLiveMediaPrefs(page: Page, webcamPrefs?: Record<string, unknown>): Promise<void> {
  await page.addInitScript((prefs) => {
    localStorage.removeItem('orion-live-streams-always-on');
    localStorage.removeItem('orion-active-channel');
    if (prefs) {
      localStorage.setItem('orion-webcam-prefs', JSON.stringify(prefs));
    } else {
      localStorage.removeItem('orion-webcam-prefs');
    }
  }, webcamPrefs ?? null);
}
...
async function installAlwaysOnLiveMediaPrefs(page: Page, webcamPrefs?: Record<string, unknown>): Promise<void> {
  await page.addInitScript((prefs) => {
    localStorage.setItem('orion-live-streams-always-on', 'true');
    localStorage.removeItem('orion-active-channel');
    if (prefs) {
      localStorage.setItem('orion-webcam-prefs', JSON.stringify(prefs));
    } else {
...
test.describe('live media intent gating', () => {
  ...
  test('turning always-on off keeps already-playing feeds running', async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem('orion-live-streams-always-on', 'false');
      window.dispatchEvent(new CustomEvent('orion-live-streams-settings-changed', {
