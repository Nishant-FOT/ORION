...
//
// Bound to: api.orion.app/*
// Source of truth for CORS on api.orion.app. Short-circuits OPTIONS
...
//      ~/.claude/skills/orion-architecture-gotchas/reference/
...
const ALLOW_HEADERS = 'Content-Type, Authorization, X-ORION-Key, X-Api-Key, X-Widget-Key, X-Pro-Key, X-ORION-Desktop-Timestamp, X-ORION-Desktop-Signature';
...
//     orion.app fallback echo and get blocked by the browser).
...
export function buildCorsHeaders(origin) {
  ...
  return {
    ...
    // (HttpOnly session cookies, see src/services/orion-session.ts). Browsers
...
export default {
  async fetch(request) {
    ...
    // orion.app fallback origin echo and the browser would block.
