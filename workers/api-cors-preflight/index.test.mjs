...
//   - Disallowed origins fall back to the canonical https://orion.app
...
const CANONICAL_FALLBACK = 'https://orion.app';
const KNOWN_GOOD = 'https://www.orion.app';
const ACAH_EXPECTED = 'Content-Type, Authorization, X-ORION-Key, X-Api-Key, X-Widget-Key, X-Pro-Key, X-ORION-Desktop-Timestamp, X-ORION-Desktop-Signature';
...
test('isAllowedOrigin accepts apex orion.app and subdomains', () => {
  assert.equal(isAllowedOrigin('https://orion.app'), true);
  assert.equal(isAllowedOrigin('https://www.orion.app'), true);
  assert.equal(isAllowedOrigin('https://tech.orion.app'), true);
  assert.equal(isAllowedOrigin('https://commodity.orion.app'), true);
});
...
test('isAllowedOrigin rejects unrelated origins', () => {
  ...
  assert.equal(isAllowedOrigin('https://orion.app.evil.com'), false);
  assert.equal(isAllowedOrigin('https://notorion.app'), false);
...
test('OPTIONS preflight returns 204 with Access-Control-Allow-Credentials: true', async () => {
  const req = makeRequest('OPTIONS', 'https://api.orion.app/api/bootstrap?tier=fast', {
    Origin: KNOWN_GOOD,
...
test('OPTIONS preflight advertises DELETE (regression — api/product-catalog purge)', async () => {
  ...
  const req = makeRequest('OPTIONS', 'https://api.orion.app/api/product-catalog', {
    Origin: KNOWN_GOOD,
...
test('OPTIONS preflight from disallowed origin still sets ACAC but echoes fallback origin', async () => {
  const req = makeRequest('OPTIONS', 'https://api.orion.app/api/bootstrap', {
...
test('non-/api/ paths bypass CORS injection and call fetch directly', async () => {
  ...
  try {
    const req = makeRequest('GET', 'https://api.orion.app/health-check', {
      Origin: KNOWN_GOOD,
    });
    const resp = await worker.fetch(req);
...
test('GET response from origin has CORS headers stamped by the Worker', async () => {
  ...
  try {
    const req = makeRequest('GET', 'https://api.orion.app/api/health', {
      Origin: KNOWN_GOOD,
    });
    const resp = await worker.fetch(req);
    assert.equal(resp.status, 200);
...
test('hasPublicCorsPolicy: rejects WM-app routes (so credentialed flow keeps Worker policy)', () => {
  ...
  assert.equal(hasPublicCorsPolicy('/api/orion-session'), false);
...
test('OPTIONS preflight to /api/mcp from https://claude.ai passes through to Vercel (Worker does NOT short-circuit)', async () => {
  ...
  // preflights with the canonical orion.app fallback origin echo,
  ...
  globalThis.fetch = async (req) => {
    ...
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-ORION-Key',
      },
    });
  };
  try {
    const req = makeRequest('OPTIONS', 'https://api.orion.app/api/mcp', {
      Origin: 'https://claude.ai',
      'Access-Control-Request-Method': 'POST',
    });
    const resp = await worker.fetch(req);
    assert.ok(received instanceof Request, 'request should have been forwarded to fetch()');
    assert.equal(received.url, 'https://api.orion.app/api/mcp');
    assert.equal(resp.status, 204);
...
test('OPTIONS preflight to /api/oauth/register from https://claude.com passes through (OAuth DCR)', async () => {
  ...
  try {
    const req = makeRequest('OPTIONS', 'https://api.orion.app/api/oauth/register', {
...
test('GET to /api/oauth/token from https://claude.ai passes Vercel headers through unchanged', async () => {
  ...
  try {
    const req = makeRequest('POST', 'https://api.orion.app/api/oauth/token', {
      Origin: 'https://claude.ai',
      'Content-Type': 'application/json',
    });
    const resp = await worker.fetch(req);
