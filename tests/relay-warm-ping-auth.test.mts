...
// trusted internal caller via X-ORION-Key = ORION_RELAY_KEY,
// validated by the gateway against its own ORION_RELAY_KEY for these
// paths only.
//
...
function req(pathname: string, key?: string): Request {
  ...
  if (key !== undefined) headers['X-ORION-Key'] = key;
  return new Request(`https://api.orion.app${pathname}`, { headers });
}

describe('relay warm-ping internal auth', () => {
  const original = process.env.ORION_RELAY_KEY;
  beforeEach(() => { process.env.ORION_RELAY_KEY = SECRET; });
  afterEach(() => {
    if (original === undefined) delete process.env.ORION_RELAY_KEY;
    else process.env.ORION_RELAY_KEY = original;
  });
  ...
  it('fails CLOSED when ORION_RELAY_KEY is unset (no bypass)', async () => {
    delete process.env.ORION_RELAY_KEY;
...
describe('relay warm-ping auth wiring (source guardrail)', () => {
  it('keeps the active Service Statuses relay loop on shared warm-ping auth headers', async () => {
    ...
    assert.match(src, /const SERVICE_STATUSES_RPC_URL = 'https:\/\/api\.orion\.app\/api\/infrastructure\/v1\/list-service-statuses'/);
    assert.match(
      src,
