...
describe('frontend CII source of truth', () => {
  ...
  it('routes remaining on-demand CII consumers through cached/server scores first', () => {
    ...
    assert.match(countryIntelSrc, /const scoreCode = normalizeCiiCountryCode\(code\);[\s\S]*calculateCII\(\)\.find\(\(s\) => s\.code === scoreCode\) \?\? getCachedCountryScore\(scoreCode\)/);
  ...
  it('aligns CII badge colors and StrategicRiskPanel display bands to source contracts', () => {
    ...
    const serverRiskSrc = readSrc('server/orion/intelligence/v1/get-risk-scores.ts');
