...
function installBrowserGlobals(): void {
  ...
  Object.defineProperty(globalThis, 'window', {
    ...
    value: {
      ...
      clearInterval: () => {},
      location: {
        href: 'https://orion.app/dashboard',
...
describe('checkout overlay lifecycle', () => {
  it('keeps one SDK handler while refreshing per-session side effects after destroy+reopen', async () => {
    ...
    assert.equal(
      harness.storageWrites.filter((key) => key === 'orion-post-checkout').length,
      1,
  ...
  it('preserves the pending-checkout intent across destroy (pins the null-handler-before-close ordering)', async () => {
    ...
    globalThis.sessionStorage.setItem('orion-pending-checkout', pending);
    ...
    assert.equal(
      globalThis.sessionStorage.getItem('orion-pending-checkout'),
      pending,
