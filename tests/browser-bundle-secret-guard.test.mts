 ...
 * `ORION_API_KEY` (a server-side platform credential) into
 ...
 *      bundle. `ORION_API_KEY` has no prefix → invisible to
 *      `readEnvSecret()` at runtime in web builds.
 *
 *   2. No entry in `RUNTIME_FEATURES.requiredSecrets` references
 *      `ORION_API_KEY`, so `seedSecretsFromEnvironment()` never
 *      iterates over it — the key isn't even attempted.
 *
 *   3. `vite.config.ts` does not pass `ORION_API_KEY` through
...
// of these grants access to orion.app infrastructure. They are
...
const PLATFORM_ONLY_SECRETS = [
  ...
  // api/_api-key.js validation against ORION_VALID_KEYS).
  'ORION_API_KEY',
  ...
  // Leakage lets attackers mint valid ors_ tokens.
...
describe('browser bundle secret guard (#3704)', () => {
  ...
  // always passes — even if `ORION_API_KEY` were added to a
  ...
  //
  //   npm run build
  //   grep -r "ORION_API_KEY" dist/  # must return zero hits
  //
