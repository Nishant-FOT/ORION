...
describe('api/mcp.ts — PRO MCP Server', () => {
  ...
  it('returns HTTP 401 + WWW-Authenticate when no credentials provided', async () => {
    ...
    assert.ok(res.headers.get('www-authenticate')?.includes('Bearer realm="orion"'), 'must include WWW-Authenticate header');
    const body = await res.json();
    assert.equal(body.error?.code, -32001);
  });

  it('returns HTTP 401 + WWW-Authenticate when invalid API key provided', async () => {
    const req = makeReq('POST', initBody(), { 'X-ORION-Key': 'wrong_key' });
    const res = await handler(req);
    assert.equal(res.status, 401);
    const wwwAuth = res.headers.get('www-authenticate') ?? '';
    assert.ok(wwwAuth.includes('Bearer realm="orion"'), 'must include WWW-Authenticate Bearer realm');
  ...
  it('OPTIONS returns 204 with CORS headers', async () => {
    const req = new Request(BASE_URL, { method: 'OPTIONS', headers: { origin: 'https://orion.app' } });
    const res = await handler(req);
  ...
  it('initialize returns protocol version and Mcp-Session-Id header', async () => {
    ...
    assert.equal(body.result?.serverInfo?.name, 'orion');
  ...
  it('malformed body returns JSON-RPC -32600', async () => {
    const req = new Request(BASE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-ORION-Key': VALID_KEY },
      body: '{bad json',
    });
    const res = await handler(req);
    const body = await res.json();
  ...
  it('telemetry: initialize emits mcp.tools_list_emitted with tools_array_bytes, tool_count, client_user_agent', async () => {
    ...
    console.log = (line) => captured.push(line);
    try {
      await mod.default(makeReq('POST', initBody(42), { 'user-agent': 'orion-test/1.0' }));
    } finally {
      console.log = origLog;
    }
    ...
    assert.equal(ev[0].client_user_agent, 'orion-test/1.0');
  ...
  it('get_maritime_activity returns zones and disruptions for valid country code', async () => {
    ...
    // in production — ORION-T8. Items outside the AE bbox (+3° pad)
...
describe('api/mcp.ts — U7 Pro-path', () => {
  ...
  it('edge: Pro tool _execute fetch sends X-ORION-MCP-Internal + X-ORION-MCP-User-Id, no X-ORION-Key', async () => {
    const { deps } = makeProDeps();
    let captured = null;
    ...
    assert.ok(captured.headers.get('x-orion-mcp-internal'), 'X-ORION-MCP-Internal must be set');
    assert.equal(captured.headers.get('x-orion-mcp-user-id'), PRO_USER_ID);
    assert.equal(captured.headers.get('x-orion-key'), null, 'X-ORION-Key must NOT be set for Pro');
    // Signature shape: <ts>.<base64url>
    const sig = captured.headers.get('x-orion-mcp-internal');
  ...
  it('integration: signed header for /api/news/v1/list-feed-digest cannot be replayed against /api/intelligence/v1/deduct-situation', async () => {
    ...
    const signed = await signInternalMcpRequest({
      method: 'GET',
      url: 'https://orion.app/api/news/v1/list-feed-digest?lang=en&variant=full',
      body: null,
      userId: PRO_USER_ID,
      secret: HMAC_SECRET,
    });
    // Re-construct the payload that would be expected for the SAME ts on a different path.
    const replayUrl = new URL('https://orion.app/api/intelligence/v1/deduct-situation');
  ...
  it('F11: parseSignatureHeader rejects ts strings longer than 15 digits', async () => {
    ...
    const req = new Request('https://example.com/x', {
      ...
      headers: {
        'Content-Type': 'application/json',
        'X-ORION-MCP-Internal': `${tsTooLong}.AAAA`,
        'X-ORION-MCP-User-Id': 'u',
      },
      body: '{}',
    });
