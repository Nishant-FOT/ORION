...
describe('api/mcp.ts — resources capability + stability + auth-symmetry', () => {
  beforeEach(async () => {
    process.env.ORION_VALID_KEYS = VALID_KEY;
  ...
  it('resources/list returns exactly four entries with the documented URIs', async () => {
    ...
    const expectedUris = [
      'orion://countries/{iso2}/risk',
      'orion://chokepoints/{slug}/status',
      'orion://seed-meta/freshness',
      'orion://markets/{symbol}/quote',
    ];
  ...
  // -------------------------------------------------------------------------
  it('resources/read orion://countries/de/risk returns country-risk content with cached_at + stale', async () => {
    const res = await handler(envKeyReq(readBody('orion://countries/de/risk')));
    assert.equal(res.status, 200);
    const body = await res.json();
    ...
    assert.equal(c.uri, 'orion://countries/de/risk', 'echo the requested uri verbatim');
  ...
  it('resources/read orion://chokepoints/suez/status returns the transit-summary envelope with cached_at + stale', async () => {
    const res = await handler(envKeyReq(readBody('orion://chokepoints/suez/status')));
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.error, undefined, `unexpected error: ${JSON.stringify(body.error)}`);
  ...
  it('resources/read orion://seed-meta/freshness returns envelope-only (no data field)', async () => {
    const res = await handler(envKeyReq(readBody('orion://seed-meta/freshness')));
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.error, undefined, `unexpected error: ${JSON.stringify(body.error)}`);
    const payload = JSON.parse(body.result.contents[0].text);
  ...
  it('resources/read orion://markets/AAPL/quote returns the matched single-symbol slice with cached_at + stale', async () => {
    const res = await handler(envKeyReq(readBody('orion://markets/AAPL/quote')));
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.error, undefined, `unexpected error: ${JSON.stringify(body.error)}`);
    const payload = JSON.parse(body.result.contents[0].text);
  ...
  it('resources/read with an unknown URI prefix returns -32602', async () => {
    const res = await handler(envKeyReq(readBody('orion://nope/asdf')));
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('resources/read with a malformed iso2 (3 letters) returns -32602 with a specific message', async () => {
    const res = await handler(envKeyReq(readBody('orion://countries/deu/risk')));
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('resources/read with an unknown chokepoint slug returns -32602 listing the known slugs', async () => {
    const res = await handler(envKeyReq(readBody('orion://chokepoints/no-such-slug/status')));
    assert.equal(res.status, 200);
    const body = await res.json();
    assert.equal(body.error?.code, -32602);
  ...
  it('LOAD-BEARING: Pro resources/read on countries/de/risk decrements the daily-quota counter by exactly 1 (identical to tools/call(get_country_risk))', async () => {
    ...
    const resR = await mcpHandler(
      proReq('POST', readBody('orion://countries/de/risk')),
      depsR,
    );
  ...
  it('Pro resources/read on cache-tool-backed URIs (markets, chokepoints, seed-meta) also increments counter by 1 each', async () => {
    const uris = [
      'orion://markets/AAPL/quote',
      'orion://chokepoints/suez/status',
      'orion://seed-meta/freshness',
    ];
  ...
  it('env-key resources/read on countries/de/risk does NOT touch the Pro quota path (env-key tier is its own quota)', async () => {
    // env-key auth path uses X-ORION-Key. The dispatcher's INCR
    ...
    const { deps, pipe } = makeProDeps({ pipelineOpts: { initialCount: 0 } });
    const res = await mcpHandler(envKeyReq(readBody('orion://countries/de/risk')), deps);
    const body = await res.json();
