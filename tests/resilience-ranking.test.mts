...
import { getResilienceRanking } from '../server/orion/resilience/v1/get-resilience-ranking.ts';
import { ApiError } from '../src/generated/server/orion/resilience/v1/service_server.ts';
import {
...
} from '../server/orion/resilience/v1/_shared.ts';
...
const originalValidKeys = process.env.ORION_VALID_KEYS;
const originalApiKey = process.env.ORION_API_KEY;
const originalSeedRefreshKey = process.env.ORION_SEED_REFRESH_KEY;
...
afterEach(() => {
  ...
  if (originalValidKeys == null) delete process.env.ORION_VALID_KEYS;
  else process.env.ORION_VALID_KEYS = originalValidKeys;
  if (originalApiKey == null) delete process.env.ORION_API_KEY;
  else process.env.ORION_API_KEY = originalApiKey;
  if (originalSeedRefreshKey == null) delete process.env.ORION_SEED_REFRESH_KEY;
  else process.env.ORION_SEED_REFRESH_KEY = originalSeedRefreshKey;
...
describe('resilience ranking contracts', () => {
  ...
  it('?refresh=1 is rejected without the dedicated seed refresh key (Pro bearer/static read keys are NOT enough)', async () => {
    ...
    // ORION_SEED_REFRESH_KEY, not the normal premium read keys.
    process.env.ORION_VALID_KEYS = 'normal-read-key';
    process.env.ORION_API_KEY = 'legacy-read-key';
    process.env.ORION_SEED_REFRESH_KEY = 'seed-refresh-secret';
    const { redis } = installRedis({ ...RESILIENCE_FIXTURES });
    redis.set(
      'resilience:static:index:v1',
      JSON.stringify({
    ...
    const authed = new Request('https://example.com/api/resilience/v1/get-resilience-ranking?refresh=1', {
      headers: { 'X-ORION-Key': 'seed-refresh-secret' },
    });
  ...
  it('gateway seed-refresh bypass is scoped to ranking ?refresh=1 only', async () => {
    ...
    const seedRefresh = await handler(
      new Request('https://orion.app/api/resilience/v1/get-resilience-ranking?refresh=1', {
        headers: { 'X-ORION-Key': 'seed-refresh-secret' },
      }),
    );
    ...
    const seedWithoutRefresh = await handler(
      new Request('https://orion.app/api/resilience/v1/get-resilience-ranking', {
        headers: { 'X-ORION-Key': 'seed-refresh-secret' },
      }),
    );
    ...
    const seedWrongPath = await handler(
      new Request('https://orion.app/api/resilience/v1/get-resilience-score?countryCode=US&refresh=1', {
        headers: { 'X-ORION-Key': 'seed-refresh-secret' },
      }),
    );
  ...
  it('ensureResilienceScoreCached returns the missing-cache fallback for cachedFetchJson null sentinel hits', async () => {
    const { redis } = installRedis(RESILIENCE_FIXTURES);
    redis.set(`${RESILIENCE_SCORE_CACHE_PREFIX}NO`, JSON.stringify('__ORION_NEG__'));
