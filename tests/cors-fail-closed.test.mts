...
describe('cors helper', () => {
  it('returns headers for a well-formed request', () => {
    const req = new Request('https://orion.app/x', {
      headers: { Origin: 'https://orion.app' },
    });
    const headers = getCorsHeaders(req);
    assert.equal(headers['Access-Control-Allow-Origin'], 'https://orion.app');
  });
...
// The Vercel project moved from the personal scope (orion-*-elie-<hash>)
// to the "eliewm" team scope. Browsers send Origin on the POST to
// /api/orion-session, so a stale allowlist 403s every preview deployment and the
...
describe('isAllowedOrigin — Vercel preview allowlist (eliewm team scope)', () => {
  ...
  const allowedByJsTwin = (origin: string) =>
    !isDisallowedOriginJs(new Request('https://orion.app/x', { headers: { Origin: origin } }));

  const ALLOWED = [
    ['git-branch alias URL', 'https://orion-git-feature-eliewm.vercel.app'],
    ['hash deployment URL', 'https://orion-abc123def456-eliewm.vercel.app'],
    ['apex production origin', 'https://orion.app'],
    ['production subdomain', 'https://tech.orion.app'],
  ];

  const REJECTED = [
    ['non-orion vercel.app origin', 'https://some-other-app-eliewm.vercel.app'],
    ['foreign team scope', 'https://orion-git-feature-attacker.vercel.app'],
    ['bare orion vercel.app (no scope segment)', 'https://orion.vercel.app'],
    ['suffix-spoofed eliewm origin', 'https://orion-git-feature-eliewm.vercel.app.evil.com'],
    ['dead personal-scope preview (post-migration)', 'https://orion-feature-elie-abc123.vercel.app'],
  ];
  ...
  for (const rel of TWINS) {
    it(`${rel} scopes Vercel previews to the eliewm team`, async () => {
      ...
      assert.ok(
        ...
        `${rel} must allow orion-*-eliewm.vercel.app previews`,
      );
      assert.ok(
        !source.includes('orion-[a-z0-9-]+\\.vercel\\.app'),
...
describe('CORS Worker superset invariant — edge allowlist ⊇ function allowlist', () => {
  ...
  // the preflight echoes the canonical orion.app fallback and the
  ...
  const PROD_ORIGINS = [
    ...
    'https://orion-git-feature-eliewm.vercel.app',
    'https://orion-abc123def456-eliewm.vercel.app',
    'tauri://localhost',
    'asset://localhost',
    ...
    'https://orion-git-feature-attacker.vercel.app',
    'https://orion-feature-elie-abc123.vercel.app',
    'https://evil.com',
  ];
    ...
    it(`Worker allows everything the function allows: ${origin}`, () => {
      if (fnAllows(origin)) {
        assert.equal(
          ...
          true,
          `Worker rejects ${origin} that api/_cors.js accepts — its OPTIONS preflight will echo the orion.app fallback and the browser will block it`,
        );
      }
    });
  }
