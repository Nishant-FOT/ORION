...
const originalKeys = process.env.ORION_VALID_KEYS;
const originalSessionSecret = process.env.ORION_SESSION_SECRET;
...
// Public routes now require a ors_ session token (issue #3541) — header-only
// origin trust is gone. Mint one for tests that previously relied on
// "trusted browser origin = anonymous public read."
process.env.ORION_SESSION_SECRET = originalSessionSecret
...
afterEach(() => {
  if (originalKeys == null) delete process.env.ORION_VALID_KEYS;
  else process.env.ORION_VALID_KEYS = originalKeys;
  installRateLimitRedisFake();
...
describe('premium gateway API key enforcement', () => {
  it('enforces premium credentials while allowing public market session auth', async () => {
    ...
    process.env.ORION_VALID_KEYS = 'real-key-123';

    // Trusted browser origin without credentials — 401 (no API key, no bearer token)
    const browserNoKey = await handler(new Request('https://orion.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: { Origin: 'https://orion.app' },
    }));
    assert.equal(browserNoKey.status, 401);
    assert.deepEqual(await browserNoKey.json(), { error: 'API key required' });

    const resilienceScoreNoKey = await handler(new Request('https://orion.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: { Origin: 'https://orion.app' },
    }));
    assert.equal(resilienceScoreNoKey.status, 401);

    const resilienceRankingNoKey = await handler(new Request('https://orion.app/api/resilience/v1/get-resilience-ranking', {
      headers: { Origin: 'https://orion.app' },
    }));
    ...
    const browserWithKey = await handler(new Request('https://orion.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://orion.app',
        'X-ORION-Key': 'real-key-123',
      },
    }));
    assert.equal(browserWithKey.status, 200);

    const resilienceScoreWithKey = await handler(new Request('https://orion.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://orion.app',
        'X-ORION-Key': 'real-key-123',
      },
    ...
    const resilienceRankingWithKey = await handler(new Request('https://orion.app/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://orion.app',
        'X-ORION-Key': 'real-key-123',
      },
    ...
    // Public endpoints — anonymous browsers authenticate via the ors_ session token
    // (issue #3541; previously this was a trusted-origin bypass).
    const publicAllowed = await handler(new Request('https://orion.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://orion.app', 'X-ORION-Key': SESSION_TOKEN },
    }));
    assert.equal(publicAllowed.status, 200);

    const insiderTransactionsAllowed = await handler(new Request('https://orion.app/api/market/v1/get-insider-transactions?symbol=AAPL', {
      headers: { Origin: 'https://orion.app', 'X-ORION-Key': SESSION_TOKEN },
    }));
    assert.equal(insiderTransactionsAllowed.status, 200);
  });

  it('PR #3557 review: anonymous ors_ session token does NOT unlock premium endpoints', async () => {
    // Regression: an earlier revision returned valid:true for ors_ tokens and
    ...
    // contract: ors_ on a premium route must 401 (no Pro auth) — never 200.
    const handler = createDomainGateway([
      {
        method: 'GET',
        path: '/api/market/v1/analyze-stock',
    ...
    for (const path of ['/api/market/v1/analyze-stock?symbol=AAPL', '/api/resilience/v1/get-resilience-score?countryCode=US']) {
      const res = await handler(new Request(`https://orion.app${path}`, {
        headers: { Origin: 'https://orion.app', 'X-ORION-Key': SESSION_TOKEN },
      }));
      assert.notEqual(res.status, 200, `ors_ MUST NOT unlock ${path} (got ${res.status})`);
    }
  });
  ...
  it('strips client-supplied x-user-id before an anonymous session reaches handlers', async () => {
    ...
    const res = await handler(new Request('https://orion.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: {
        Origin: 'https://orion.app',
        'X-ORION-Key': SESSION_TOKEN,
  ...
  it('rewrites client-supplied x-user-id on wm_ user-API-key auth (#3548)', async () => {
    ...
    try {
      const res = await handler(
        new Request('https://orion.app/api/market/v1/analyze-stock?symbol=AAPL', {
          headers: {
            Origin: 'https://orion.app',
            'X-ORION-Key': 'wm_owner_pro_test',
...
describe('POST-to-GET compatibility hardening', () => {
  ...
  function compatPost(body: string, headers: Record<string, string> = {}) {
    return new Request('https://orion.app/api/market/v1/list-market-quotes', {
      method: 'POST',
      headers: {
        Origin: 'https://orion.app',
        'X-ORION-Key': SESSION_TOKEN,
...
describe('premium gateway bearer token auth', () => {
  ...
  it('valid bearer token resolves userId but entitlement check still applies', async () => {
    ...
    const res = await handler(new Request('https://orion.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://orion.app',
  ...
  it('rejects free bearer token on resilience premium endpoints → 403', async () => {
    ...
    const scoreRes = await handler(new Request('https://orion.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://orion.app',
        Authorization: `Bearer ${token}`,
      },
    ...
    const rankingRes = await handler(new Request('https://orion.app/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://orion.app',
        Authorization: `Bearer ${token}`,
      },
  ...
  it('rewrites spoofed x-user-id from a verified legacy bearer before reaching handlers', async () => {
    ...
    const res = await headerEchoHandler(new Request('https://orion.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://orion.app',
        Authorization: `Bearer ${token}`,
        'x-user-id': 'attacker-controlled-user',
  ...
  it('forwards POST body alongside trusted x-user-id on the legacy bearer path', async () => {
    ...
    const res = await echoHandler(new Request('https://orion.app/api/intelligence/v1/deduct-situation', {
      method: 'POST',
      headers: {
        Origin: 'https://orion.app',
        Authorization: `Bearer ${token}`,
