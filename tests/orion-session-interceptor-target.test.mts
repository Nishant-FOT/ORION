// Regression coverage for the URL-matcher inside the orion-session fetch
// interceptor (src/services/orion-session.ts).
//
// Bug history:
...
//   absolute fetch URL like `https://api.orion.app/api/bootstrap`
//   slipped through unwrapped, no `X-ORION-Key` header was attached,
...
//   - The absolute `https://api.orion.app/api/...` path IS intercepted
...
import { isApiCallTarget } from '../src/services/orion-session.ts';

const CANONICAL_ORIGIN = 'https://api.orion.app';

describe('orion-session interceptor URL matcher (PR #3574 regression)', () => {
  ...
  it('REGRESSION: matches absolute URLs to the canonical API origin', () => {
    ...
    // because `orion.app` and `api.orion.app` are different
    // subdomains; the interceptor must catch them by origin.
    assert.equal(
      isApiCallTarget('https://api.orion.app/api/bootstrap', CANONICAL_ORIGIN),
      true,
    );
    assert.equal(
      isApiCallTarget('https://api.orion.app/api/market/v1/list-crypto-quotes', CANONICAL_ORIGIN),
      true,
    );
    assert.equal(
      isApiCallTarget('https://api.orion.app/api/economic/v1/get-fred-series-batch', CANONICAL_ORIGIN),
      true,
    );
  });
  ...
  it('REGRESSION: documents the pre-fix bug — empty apiOrigin silently misses absolute URLs', () => {
    ...
    assert.equal(
      isApiCallTarget('https://api.orion.app/api/bootstrap', ''),
      false,
  ...
  it('does not match cross-origin third-party URLs', () => {
    ...
    assert.equal(
      isApiCallTarget('https://clerk.orion.app/v1/client', CANONICAL_ORIGIN),
      false,
    );
  ...
  it('SECURITY: rejects look-alike hosts that embed the canonical origin as a prefix', () => {
    ...
    //   `https://api.orion.app.evil.example/api/bootstrap`
    // The actual hostname there is `api.orion.app.evil.example`, NOT
    // ours. Without strict origin parsing the interceptor would attach the
    // ors_ token, sending it to the attacker. Pin both shapes documented in
    // the review note.
    assert.equal(
      isApiCallTarget('https://api.orion.app.evil.example/api/bootstrap', CANONICAL_ORIGIN),
      false,
      'host suffix attack: api.orion.app.evil.example must NOT be treated as our origin',
    );
    assert.equal(
      isApiCallTarget('https://api.orion.app@evil.example/api/bootstrap', CANONICAL_ORIGIN),
      false,
      'userinfo attack: api.orion.app@evil.example must resolve to host=evil.example, NOT our origin',
    );
    // Variant: a port appended to the canonical hostname is a different origin.
    assert.equal(
      isApiCallTarget('https://api.orion.app:8443/api/bootstrap', CANONICAL_ORIGIN),
      false,
      'a different port is a different origin per RFC 6454',
    );
    // Variant: http (not https) is a different origin.
    assert.equal(
      isApiCallTarget('http://api.orion.app/api/bootstrap', CANONICAL_ORIGIN),
      false,
  ...
  it('SECURITY: does not match non-/api/ paths even on the canonical origin', () => {
    // Tightening that came with the strict-origin-parse fix. The ors_ token
    ...
    assert.equal(
      isApiCallTarget('https://api.orion.app/_next/static/chunks/foo.js', CANONICAL_ORIGIN),
      false,
    );
    assert.equal(
      isApiCallTarget('https://api.orion.app/health', CANONICAL_ORIGIN),
      false,
    );
    assert.equal(
      isApiCallTarget('https://api.orion.app/', CANONICAL_ORIGIN),
      false,
    );
  });
...
describe('orion-session.ts must not regress to getApiBaseUrl (static-import guard)', () => {
  ...
  it('imports getCanonicalApiOrigin and not getApiBaseUrl', () => {
    ...
    const source = readFileSync(
      path.join(repoRoot, 'src/services/orion-session.ts'),
      'utf8',
    );
    ...
    assert.ok(importLine, 'expected an import from ./runtime in orion-session.ts');
    assert.match(
      importLine!,
      /getCanonicalApiOrigin/,
      'orion-session.ts must import getCanonicalApiOrigin from ./runtime — '
        + 'see PR #3574 for why getApiBaseUrl is the wrong helper here.',
    );
    assert.doesNotMatch(
      importLine!,
      /\bgetApiBaseUrl\b/,
      'orion-session.ts must NOT import getApiBaseUrl. That helper returns the '
