...
describe('renderBriefMagazine — chrome invariants', () => {
  it('logo symbol is emitted exactly once; all placements reference it via <use>', () => {
    const env = envelope();
    const html = renderBriefMagazine(env);
    const symbolDefs = html.match(/<symbol id="orion-logo-core"/g) || [];
    ...
    const useRefs = html.match(/<use href="#orion-logo-core"\s*\/>/g) || [];
    ...
    const ariaLabels = html.match(/aria-label="ORION"/g) || [];
...
describe('renderBriefMagazine — source link (v2)', () => {
  it('wraps every story source in an outgoing anchor with UTM params', () => {
    ...
    assert.ok(html.includes('utm_source=orion'));
...
describe('renderBriefMagazine — Share button (non-public views)', () => {
  const SHARE_URL = 'https://orion.app/api/brief/public/abcDEF012345';

  it('renders a Share button with data-share-url and issue date', () => {
    const env = envelope();
    const html = renderBriefMagazine(env, { shareUrl: SHARE_URL });
    assert.ok(html.includes('class="orion-share"'), 'share button must be present');
  ...
  it('emits the inline share script exactly once', () => {
    const env = envelope();
    const html = renderBriefMagazine(env, { shareUrl: SHARE_URL });
    const matches = html.match(/document\.querySelector\('\.orion-share'\)/g) ?? [];
  ...
  it('click handler reads from dataset rather than fetching — no auth round-trip', () => {
    ...
    const scriptStart = html.indexOf("document.querySelector('.orion-share')");
  ...
  it('gracefully hides the Share button when shareUrl is absent', () => {
    const env = envelope();
    const html = renderBriefMagazine(env);
    assert.ok(!html.includes('class="orion-share"'), 'no Share button when shareUrl unset');
    assert.ok(
      !html.includes("document.querySelector('.orion-share')"),
...
describe('renderBriefMagazine — publicMode', () => {
  it('strips Share button + share script on public mode even when shareUrl is passed', () => {
    ...
    const html = renderBriefMagazine(env, {
      publicMode: true,
      shareUrl: 'https://orion.app/api/brief/public/abcDEF012345',
    });
    assert.ok(!html.includes('class="orion-share"'), 'share button absent on public');
    assert.ok(
      !html.includes("document.querySelector('.orion-share')"),
  ...
  it('replaces whyMatters with a generic callout on every story page', () => {
    ...
    assert.ok(
      publicHtml.includes('Subscribe to ORION Brief to see the full editorial'),
  ...
  it('prepends a Subscribe strip on public views', () => {
    ...
    assert.ok(html.includes('class="orion-public-strip"'), 'subscribe strip element emitted');
    assert.ok(html.includes('orion.app/pro'), 'strip links to /pro');
  ...
  it('attaches ?ref= to public CTAs when refCode is provided', () => {
    ...
    assert.ok(html.includes('orion.app/pro?ref=ABC123'), 'refCode appended to /pro URL');
  });
