...
// Anonymous browser access requires a ors_ session token (issue #3541).
process.env.ORION_SESSION_SECRET = process.env.ORION_SESSION_SECRET
...
const ORIGINAL_VALID_KEYS = process.env.ORION_VALID_KEYS;
...
afterEach(() => {
  ...
  if (ORIGINAL_VALID_KEYS == null) delete process.env.ORION_VALID_KEYS;
  else process.env.ORION_VALID_KEYS = ORIGINAL_VALID_KEYS;
});
...
describe('gateway telemetry payload — domain extraction', () => {
  it("emits domain='shipping' for /api/v2/shipping/* routes (not 'v2')", async () => {
    ...
    const res = await handler(
      new Request('https://orion.app/api/v2/shipping/route-intelligence', {
  ...
  it("emits domain='market' for the standard /api/<domain>/v1/<rpc> layout", async () => {
    ...
    const recorder = makeRecordingCtx();
    const res = await handler(
      new Request('https://orion.app/api/market/v1/list-market-quotes?symbols=AAPL', {
  ...
  it("PR #3557 round-3: anonymous ors_ token telemetry is anon, NOT enterprise_api_key", async () => {
    // Regression: an earlier revision set usage.enterpriseApiKey for any valid
    // wmKey not starting with 'wm_'. Since 'ors_' doesn't startsWith 'wm_',
    ...
    assert.equal(spy.events.length, 1);
    const ev = spy.events[0]!;
    assert.equal(ev.auth_kind, 'anon', `ors_ tokens must telemeter as anon, got '${ev.auth_kind}'`);
...
describe('gateway telemetry payload — bearer identity propagation', () => {
  ...
  it('records customer_id from a successful legacy premium bearer call', async () => {
    ...
    const recorder = makeRecordingCtx();
    const res = await handler(
      new Request('https://orion.app/api/resilience/v1/get-resilience-score?countryCode=US', {
        headers: {
...
describe('gateway telemetry payload — unmatched route reason labels', () => {
  ...
  it("unknown path → status=404 + reason='unknown_route'", async () => {
    ...
    const recorder = makeRecordingCtx();
    const res = await handler(
      new Request('https://orion.app/api/trade/v1/list-tariffs', {
        headers: { Origin: 'https://orion.app', 'X-ORION-Key': SESSION_TOKEN },
      }),
      recorder.ctx,
    );
  ...
  it("known path with wrong method → status=405 + reason='method_not_allowed'", async () => {
    ...
    const recorder = makeRecordingCtx();
    const res = await handler(
      new Request('https://orion.app/api/market/v1/list-market-quotes', {
        method: 'DELETE',
        headers: { Origin: 'https://orion.app', 'X-ORION-Key': SESSION_TOKEN },
      }),
      recorder.ctx,
