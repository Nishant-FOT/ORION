...
// an anonymous session token via the orion-session fetch wrapper). The enterprise
...
function makeReq(q?: string, method = 'GET'): Request {
  const url = q === undefined
    ? 'https://orion.app/api/symbol-search'
    : `https://orion.app/api/symbol-search?q=${encodeURIComponent(q)}`;
  return new Request(url, { method, headers: { 'X-ORION-Key': TEST_KEY } });
}
...
describe('symbol-search handler', () => {
  it('returns mapped Finnhub results for a query, with our User-Agent', async () => {
    ...
    assert.equal(requestedUA, 'orion-edge/1.0');
