 ...
 *  - Toggle persists to localStorage (`orion-followed-only-filter-${panelId}`).
...
function makeHost() {
  ...
  const host = {
     ...
     * the chip via `target.closest('.orion-followed-only-chip')`. We mirror
    ...
    clickChip() {
      ...
      const isPresent = _innerHtml.includes('class="orion-followed-only-chip');
      const buttonStub = {
        hasAttribute: (name) => (name === 'disabled' ? isDisabled : false),
        closest: (sel) =>
          sel === '.orion-followed-only-chip' && isPresent ? buttonStub : null,
      };
      const ev = {
        type: 'click',
        target: buttonStub,
...
describe('renderFollowedOnlyChip — default + persistence', () => {
  ...
  it('reads persisted "1" → renders active', () => {
    ...
    _localStorage.setItem('orion-followed-only-filter-p1', '1');
    const handle = renderFollowedOnlyChip({ panelId: 'p1' });
  ...
  it('toggle on persists "1" to orion-followed-only-filter-${panelId}', () => {
    setupAnonymousFlagOn();
    _localStorage.setItem(
      FOLLOWED_COUNTRIES_STORAGE_KEY,
      JSON.stringify({ countries: ['US'] }),
    ...
    assert.equal(_localStorage.getItem('orion-followed-only-filter-p1'), '1');
  ...
  it('toggle off removes the key (default-off semantics preserved)', () => {
    ...
    assert.equal(_localStorage.getItem('orion-followed-only-filter-p1'), null);
    assert.match(host.innerHTML, /data-state="inactive"/);
    teardown();
  });
  ...
  it('per-panel scoping — toggling p1 does NOT affect p2', () => {
    ...
    assert.equal(_localStorage.getItem('orion-followed-only-filter-p1'), '1');
    assert.equal(_localStorage.getItem('orion-followed-only-filter-p2'), null);
...
describe('renderFollowedOnlyChip — integration: filter pass against a country-scoped row list', () => {
  ...
  it('stale active + empty watchlist → clears persisted state and does not filter', () => {
    ...
    _localStorage.setItem('orion-followed-only-filter-integ-3', '1');
    ...
    assert.equal(handle.isActive(), false);
    assert.equal(_localStorage.getItem('orion-followed-only-filter-integ-3'), null);
