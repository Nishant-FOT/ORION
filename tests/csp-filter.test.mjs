...
describe('CSP violation filter (shouldSuppressCspViolation)', () => {
  ...
  describe('connect-src HTTPS suppression (policy-aware)', () => {
    it('suppresses HTTPS connect-src when CSP allows https:', () => {
      assert.ok(suppress('enforce', 'connect-src', 'https://api.orion.app/api/oref-alerts', '', true));
    });
    ...
    it('does NOT suppress HTTPS connect-src when CSP does not allow https:', () => {
      assert.ok(!suppress('enforce', 'connect-src', 'https://api.orion.app/api/oref-alerts', '', false));
    });
  ...
  describe('media-src HTTPS suppression (policy-aware) — ORION-HV', () => {
  ...
  describe('default-src HTTP mixed-content suppression — ORION-S0', () => {
    ...
    it('does NOT suppress http default-src block to our own host (real mixed-content regression)', () => {
      assert.ok(!suppress('enforce', 'default-src', 'http://www.orion.app/asset.json', '', false));
      assert.ok(!suppress('enforce', 'default-src', 'http://orion.app/asset.json', '', false));
    });
    ...
    it('does NOT let a orion.app suffix-spoof lookalike bypass the first-party gate', () => {
      // orion.app.evil.com is third-party → http block IS suppressed (it is not us).
      assert.ok(suppress('enforce', 'default-src', 'http://orion.app.evil.com/x', '', false));
    });
  });
  ...
  describe('scheme-only and special values', () => {
    ...
    it('suppresses blob: URI', () => {
      assert.ok(suppress('enforce', 'worker-src', 'blob:https://www.orion.app/abc', '', false));
    });
  ...
  describe('third-party noise', () => {
    ...
    it('suppresses manifest.webmanifest', () => {
      assert.ok(suppress('enforce', 'default-src', 'https://www.orion.app/manifest.webmanifest', '', false));
    });

    it('suppresses third-party stylesheet injection from cdn.jsdelivr.net (style-src-elem)', () => {
      // ORION-J0: extension/bookmarklet injecting antd@4 CSS on
      // finance.orion.app — 270 events / 26 users. We never load
  ...
  describe('first-party infrastructure (mutated user CSP)', () => {
    ...
    // host so we don't drown Sentry in events from those users (ORION-HN).
    ...
    // policy (`img-src 'self' data: blob: https:`) allows them (ORION-JP).
    it('suppresses img-src to apex orion.app (favicon)', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://orion.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('suppresses img-src to www.orion.app (production favicon, ORION-JP)', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://www.orion.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('suppresses img-src to finance.orion.app subdomain', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://finance.orion.app/favico/finance/apple-touch-icon.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('suppresses img-src to tech.orion.app subdomain', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://tech.orion.app/favico/tech/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });
    ...
    it('does NOT suppress img-src to suffix-spoof lookalike `orion.app.evil.com`', () => {
      // Endswith check uses a leading `.` so attacker-controlled lookalikes
      // (`orion.app.evil.com`, `not-orion.app`) are not whitelisted.
      assert.ok(!suppress('enforce', 'img-src', 'https://orion.app.evil.com/pixel.gif', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress img-src to prefix-spoof `not-orion.app`', () => {
      assert.ok(!suppress('enforce', 'img-src', 'https://not-orion.app/pixel.gif', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress connect-src to orion.app (rule is scoped to img-src)', () => {
      ...
      assert.ok(!suppress('enforce', 'connect-src', 'https://api.orion.app/api/health', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress script-src to orion.app (rule is scoped to img-src)', () => {
      ...
      assert.ok(!suppress('enforce', 'script-src', 'https://www.orion.app/assets/main-abc.js', '', false, FIRST_PARTY_CONVEX));
    });
    ...
    it('does NOT suppress http:// img-src to our own apex (mixed-content regression must surface)', () => {
      assert.ok(!suppress('enforce', 'img-src', 'http://orion.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress http:// img-src to a orion.app subdomain', () => {
      assert.ok(!suppress('enforce', 'img-src', 'http://www.orion.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress ws:// img-src to a orion.app subdomain (only https: is whitelisted)', () => {
      ...
      assert.ok(!suppress('enforce', 'img-src', 'ws://www.orion.app/socket', '', false, FIRST_PARTY_CONVEX));
    });
  });
