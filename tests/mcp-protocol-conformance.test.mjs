...
describe('api/mcp.ts — protocol conformance lifecycle (in-process)', () => {
  ...
  it('walks initialize → tools/list → describe_tool → tools/call × N → re-init → still-locked-out', async () => {
    ...
    assert.ok(
      (step1Res.headers.get('www-authenticate') ?? '').includes('Bearer realm="orion"'),
    ...
    assert.equal(step2Body.result?.serverInfo?.name, 'orion', 'step 2: serverInfo.name must be "orion"');
