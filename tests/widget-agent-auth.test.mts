...
describe('widget-agent unified tester key auth', () => {
  ...
  before(async () => {
    ...
    process.env.ORION_VALID_KEYS = 'browser-test-key';
  ...
  it('accepts X-ORION-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://www.orion.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.orion.app',
        'Content-Type': 'application/json',
        'X-ORION-Key': 'browser-test-key',
      },
    ...
    assert.equal(call.arguments[0], 'https://proxy.orion.app/widget-agent');
    ...
    assert.equal(headers.get('X-ORION-Key'), null);
  ...
  it('falls back to legacy tester keys when X-ORION-Key is invalid', async () => {
    const res = await handler(new Request('https://www.orion.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.orion.app',
        'Content-Type': 'application/json',
        'X-ORION-Key': 'wrong-key',
  ...
  it('accepts HttpOnly legacy tester key cookies without JS-readable auth headers', async () => {
    const res = await handler(new Request('https://www.orion.app/api/widget-agent', {
      ...
      headers: {
        Origin: 'https://www.orion.app',
        'Content-Type': 'application/json',
        Cookie: `orion-widget-key=${encodeURIComponent('server-widget-key')}; orion-pro-key=${encodeURIComponent('server-pro-key')}`,
      },
      body: JSON.stringify({ prompt: 'Build a widget', mode: 'create', tier: 'basic' }),
    }));
  ...
  it('rejects invalid X-ORION-Key before relay fetch', async () => {
    const res = await handler(new Request('https://www.orion.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.orion.app',
