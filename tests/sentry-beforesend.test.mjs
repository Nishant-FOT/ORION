...
describe('empty-stack network/timeout errors are NOT suppressed', () => {
  ...
  // suite below for that case (ORION-Q / ORION-15).
...
describe('dynamic-module-import failures (stale chunk after deploy)', () => {
  ...
  // stack (ORION-TN: Map chunk, ORION-S1: hls chunk — both leaked
  // because the old `!hasFirstParty`-only gate let first-party-framed ones
  // through).
  const assetUrlImportErrors = [
    'Failed to fetch dynamically imported module: https://orion.app/assets/panels-abc.js',
    'Failed to fetch dynamically imported module: https://www.orion.app/assets/index-DSkSc57y.js',
    'error loading dynamically imported module: https://www.orion.app/assets/Map-eKJvyIxN.js',
    'error loading dynamically imported module: https://www.orion.app/assets/hls-jw_vZdHi.js',
  ];
  ...
  it('lets through off-origin /assets dynamic-import failures even with first-party stack', () => {
    ...
    assert.ok(beforeSend(event) !== null, 'off-origin asset URL must not be treated as ORION deploy skew');
  });

  it('lets through non-hashed /assets dynamic-import failures even on owned origins', () => {
    const event = makeEvent(
      'Failed to fetch dynamically imported module: https://orion.app/assets/runtime.js',
      'TypeError',
      [firstPartyFrame()],
    );
  ...
  // mismatch `Importing binding name '<x>' is not found.` — ORION-TM)
...
describe('zero-frame async-rejection patterns (timeout / DOMException / OOM / DOM-walker / wrapper-injected timeout)', () => {
  const zeroFrameErrors = [
    ...
    // Firefox setInterval mechanism, no captured frames (ORION-KE)
    ['out of memory', 'Error'],
    // Apple Mail privacy proxy DOM walker (ORION-P2). Frames in
    ...
    // (ORION-PW: /api/setIsSelect from Electron 39.2.7).
    ...
    // site (ORION-KM 10ev/8u). The host-suffixed variant
    ...
    // (ORION-RF). iOS Safari fires `SyntaxError: Unexpected EOF`
    ...
    // Firefox's wording for a failed `fetch()` (ORION-RK) — the
...
describe('existing beforeSend filters', () => {
  ...
  it('suppresses OrbitControls setPointerCapture NotFoundError when frame context matches three.js signature', () => {
    // Verbatim frame context slice from ORION-NC: minified three.js OrbitControls
  ...
  it('suppresses Clerk SDK "Failed to fetch (clerk.orion.app)" even with a clerk first-party frame', () => {
    // ORION-SA/SB: the bundled Clerk SDK fetches its Frontend API
    // (clerk.orion.app, a CNAME to Clerk's auth infra) for token
    // refresh and retries transient failures itself. A leaked
    // `Failed to fetch (clerk.orion.app)` is a Clerk-SDK-internal
    ...
    const event = makeEvent('Failed to fetch (clerk.orion.app)', 'TypeError', [
  ...
  it('suppresses bare "Failed to fetch" when an extension monkeypatched window.fetch (ORION-SG)', () => {
    // Real ORION-SG stack: our runtime fetch interceptor + country-geometry
  ...
  it('does NOT suppress bare "Failed to fetch" with a first-party frame and a NON-fetch extension frame', () => {
    // Precision guard for ORION-SG: an extension frame whose function is
  ...
  it('does NOT suppress "Failed to fetch (<hostname>)" when no maplibre frame is present', () => {
    ...
    const event = makeEvent('Failed to fetch (api.orion.app)', 'TypeError', [
      { filename: '/assets/panels-wF5GXf0N.js', lineno: 100, function: 'MyApiCall' },
    ]);
  ...
  it('does NOT suppress MapLibre AJAXError for a non-allowlisted host (mixed stack)', () => {
    // Mirrors ORION-NE/NF real-world stack: maplibre + first-party fetch wrapper.
    const event = makeEvent('Failed to fetch (pmtiles.orion.app)', 'TypeError', [
      { filename: '/assets/maplibre-A8Ca0ysS.js', lineno: 4, function: 'ajaxFetch' },
      { filename: '/assets/panels-wF5GXf0N.js', lineno: 24, function: 'window.fetch' },
    ]);
  ...
  it('suppresses "Failed to fetch (<host>)" when stack is extension-only (covered by generic extension rule)', () => {
    // ORION-P5: AdBlock-class extensions wrap window.fetch and their
    ...
    const event = makeEvent('Failed to fetch (abacus.orion.app)', 'TypeError', [
  ...
  it('does NOT suppress "Failed to fetch (<host>)" when stack has both first-party and extension frames', () => {
    ...
    // api.orion.app outage isn't silenced for users who happen to run
  ...
  it('suppresses iOS Safari WKWebView "Cannot inject key into script value" regardless of first-party frame', () => {
    ...
    // so scoping on excType is safe (ORION-NM).
  ...
  it('suppresses Convex re-auth race on fetchToken (stack has tryToReauthenticate)', () => {
    ...
    // internal, not actionable in our code (ORION-NJ).
    const event = makeEvent(
  ...
  // ORION-MK: Fireglass (Symantec/Broadcom CloudSOC) console-hook recursion.
  ...
  // ORION-MH: Chrome Mobile WebView 105+ duplex requirement, Dodo SDK path.
  ...
  // ORION-MP: Chrome extension intercepting maplibre fetch — suppress only when no first-party frames.
  ...
  // ORION-MQ: Sentry SDK DOM breadcrumb null.contains crash — suppress only when no first-party frames.
  ...
  // ORION-MV: Convex WS onmessage JSON.parse truncation — suppress only when stack has no first-party frames.
  ...
  // ORION-NR: deck.gl/maplibre internal null-access on Layer.isHidden
  ...
  // ORION-NQ: Safari short-var ReferenceError ("Can't find variable: ss")
...
// ─── ORION-SQ: ProgressEvent rejection ignoreErrors entry ──────────
//
...
describe('ignoreErrors — ProgressEvent promise rejection (ORION-SQ)', () => {
...
// ─── ORION-SP: SyntaxError through the deck.gl/maplibre init path ───
//
...
describe('SyntaxError via deck.gl/maplibre init path (ORION-SP)', () => {
  // Mirrors the real ORION-SP stack: deck-stack + maplibre vendor frames
...
// ─── ORION-TG: mainWorldSdk extension-global ReferenceError ─────────
//
...
describe('ignoreErrors — mainWorldSdk extension global (ORION-TG)', () => {
