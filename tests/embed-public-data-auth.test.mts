...
describe('embed public data auth', () => {
  ...
  it('lets anonymous iframe requests reach embed RPC handlers while nearby RPCs stay gated', async () => {
    ...
    for (const path of EMBED_PUBLIC_RPC_PATHS) {
      const res = await gateway(new Request(`https://orion.app${path}`, {
        headers: { Origin: 'https://orion.app' },
      }));
      assert.equal(res.status, 200, `${path} should not require orion-session in an embed iframe`);
    }

    const gated = await gateway(new Request('https://orion.app/api/conflict/v1/list-ucdp-events', {
      headers: { Origin: 'https://orion.app' },
    }));
  ...
  it('scopes anonymous bootstrap access to the weather embed key only', async () => {
    const publicReq = new Request('https://orion.app/api/bootstrap?keys=weatherAlerts', {
      headers: { Origin: 'https://orion.app' },
    });
    ...
    const rejected = [
      'https://orion.app/api/bootstrap',
      'https://orion.app/api/bootstrap?tier=fast',
      'https://orion.app/api/bootstrap?keys=weatherAlerts,marketQuotes',
      'https://orion.app/api/bootstrap?keys=marketQuotes',
      'https://orion.app/api/bootstrap?keys=weatherAlerts&debug=1',
      'https://orion.app/api/bootstrap?keys=weatherAlerts&keys=marketQuotes',
    ];

    for (const url of rejected) {
      const req = new Request(url, { headers: { Origin: 'https://orion.app' } });
