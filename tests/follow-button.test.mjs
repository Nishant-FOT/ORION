 ...
 *    resolves the inner `.orion-follow-btn`. We don't parse the html;
...
function makeHost() {
  ...
  const host = {
     ...
     * `target.closest('.orion-follow-btn')`. We set `target` to a stub that
    ...
    clickButton() {
      ...
      const buttonStub = {
        ...
        closest: (sel) =>
          sel === '.orion-follow-btn' && _innerHtml.includes('class="orion-follow-btn')
            ? buttonStub
            : null,
      };
      const ev = {
...
describe('renderFollowButton — basic visual states', () => {
  it('anonymous, not followed → emits outlined-star html with data-state="unfollowed"', () => {
    ...
    assert.match(handle.html, /class="orion-follow-btn orion-follow-btn--md/);
  ...
  it('signed-in entitlement loading → emits spinner html with disabled and data-state="loading"', () => {
    ...
    assert.match(handle.html, /orion-follow-btn-spinner/);
  });
  ...
  it('size="sm" applies the sm modifier class', () => {
    ...
    assert.match(handle.html, /orion-follow-btn--sm/);
  });
