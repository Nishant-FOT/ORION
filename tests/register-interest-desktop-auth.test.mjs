...
function makeCtx(headers = {}) {
  return {
    request: new Request('https://orion.app/api/leads/v1/register-interest', {
      method: 'POST',
...
describe('LeadsService.registerInterest desktop auth', () => {
  beforeEach(async () => {
    process.env.ORION_DESKTOP_SHARED_SECRET = 'desktop-test-secret';
    process.env.CONVEX_URL = 'https://fake-convex.cloud';
    process.env.VERCEL_ENV = 'production';
    delete process.env.WM_DESKTOP_AUTH_ALLOW_LEGACY;

    const mod = await import('../server/orion/leads/v1/register-interest.ts');
    ...
    const gen = await import('../src/generated/server/orion/leads/v1/service_server.ts');
  ...
  it('rejects stale desktop signatures', async () => {
    ...
    const signature = await createDesktopAuthSignature(process.env.ORION_DESKTOP_SHARED_SECRET, timestamp, req);
  ...
  it('canonicalizes only string fields for desktop signatures', async () => {
    ...
    const signatureWithNonStrings = await createDesktopAuthSignature(
      process.env.ORION_DESKTOP_SHARED_SECRET,
      timestamp,
      desktopReq({
        appVersion: 280,
        referredBy: ['abc'],
  ...
  it('allows unsigned legacy desktop requests only when rollout fallback is enabled and shared secret is unset', async () => {
    process.env.WM_DESKTOP_AUTH_ALLOW_LEGACY = 'true';
    delete process.env.ORION_DESKTOP_SHARED_SECRET;
