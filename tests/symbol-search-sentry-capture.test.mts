...
// ─── ORION-RE: symbol-search Sentry-capture policy ──────────────────
//
...
const TEST_KEY = 'orion-test-enterprise-key';
...
process.env.ORION_VALID_KEYS = TEST_KEY;
...
function makeReq(q = 'nvidia'): Request {
  return new Request(
    `https://orion.app/api/symbol-search?q=${encodeURIComponent(q)}`,
    { headers: { 'X-ORION-Key': TEST_KEY } },
  );
}
...
describe('symbol-search Sentry-capture policy (ORION-RE)', () => {
  ...
  for (const finnhubStatus of [502, 503, 504]) {
    it(`skips capture for upstream gateway transient ${finnhubStatus}`, async () => {
      const { envelopeHits, status } = await runWithFinnhubStatus(finnhubStatus);
      assert.equal(envelopeHits, 0, `gateway transient ${finnhubStatus} must NOT page Sentry (ORION-RE)`);
