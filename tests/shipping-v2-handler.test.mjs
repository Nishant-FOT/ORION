...
function makeCtx(headers = {}) {
  const req = new Request('https://orion.app/api/v2/shipping/route-intelligence', {
    method: 'GET',
    headers,
  });
...
function proCtx() {
  return makeCtx({ 'X-ORION-Key': 'pro-test-key' });
}
...
describe('ShippingV2Service handlers', () => {
  beforeEach(async () => {
    process.env.ORION_VALID_KEYS = 'pro-test-key';
    process.env.UPSTASH_REDIS_REST_URL = 'https://fake-upstash.example';
    process.env.UPSTASH_REDIS_REST_TOKEN = 'fake-token';

    const riMod = await import('../server/orion/shipping/v2/route-intelligence.ts');
    const rwMod = await import('../server/orion/shipping/v2/register-webhook.ts');
    const lwMod = await import('../server/orion/shipping/v2/list-webhooks.ts');
    const dwMod = await import('../server/orion/shipping/v2/deliver-webhook.ts');
    webhookShared = await import('../server/orion/shipping/v2/webhook-shared.ts');
    ...
    const gen = await import('../src/generated/server/orion/shipping/v2/service_server.ts');
  ...
  describe('registerWebhook', () => {
    ...
    it('rejects callers without an API key with 401 (tenant-isolation gate)', async () => {
      // Without this gate, Clerk-authenticated pro callers with no X-ORION-Key
  ...
  describe('deliverWebhook', () => {
    ...
    it('sends only after a public delivery-time DNS answer and includes delivery headers', async () => {
      ...
      assert.equal(seen.headers['user-agent'], 'ORION-ShippingV2-Webhooks/1.0');
      assert.equal(seen.headers['x-orion-delivery-id'], 'whd_test_delivery');
      assert.equal(seen.headers['x-orion-event'], 'chokepoint.disruption');
      assert.match(seen.headers['x-orion-signature'], /^sha256=[0-9a-f]{64}$/);
