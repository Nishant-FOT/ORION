...
describe('score cache payload validation', () => {
  it('accepts any valid formula tag independent of seeder env unless a live runtime formula is supplied', () => {
    ...
    try {
      ...
      assert.equal(parseCachedScorePayload(JSON.stringify('__ORION_NEG__')), null);
...
describe('ensures ranking aggregate is present every cron, with truthful meta', () => {
  ...
  it('fails fast when the dedicated seed refresh key is missing', () => {
    assert.match(
      src,
      /function requireSeedRefreshKey\(\)[\s\S]*?if \(WM_REFRESH_KEY\) return;[\s\S]*?throw new Error\('ORION_SEED_REFRESH_KEY is required for resilience ranking refresh'\);/,
  ...
  it('operator force-refresh docs include the required seed refresh key', async () => {
    ...
    for (const file of files) {
      ...
      for (const match of scoreWarmCommands) {
        ...
        assert.match(
          commandContext,
          /ORION_SEED_REFRESH_KEY=<seed-refresh-key>/,
          `${file} must include ORION_SEED_REFRESH_KEY for seed-resilience-scores`,
        );
      }
      assert.doesNotMatch(
        src,
        /ORION_API_KEY=<key> node scripts\/seed-resilience-scores\.mjs/,
...
describe('handler warm pipeline is chunked', () => {
  ...
  it('warmMissingResilienceScores splits SETs into batches', async () => {
    ...
    const dir = dirname(fileURLToPath(import.meta.url));
    const src = readFileSync(
      join(dir, '..', 'server', 'orion', 'resilience', 'v1', '_shared.ts'),
      'utf8',
    );
    assert.match(
      src,
