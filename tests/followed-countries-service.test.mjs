...
describe('followed-countries service — constants & key', () => {
  ...
  it('uses storage key orion-followed-countries-v1', () => {
    assert.equal(FOLLOWED_COUNTRIES_STORAGE_KEY, 'orion-followed-countries-v1');
  });

  it('exports event name orion-followed-countries-changed', () => {
    assert.equal(WM_FOLLOWED_COUNTRIES_CHANGED, 'orion-followed-countries-changed');
  });
});
