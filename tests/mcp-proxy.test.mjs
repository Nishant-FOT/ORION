...
// finding — ors_ session tokens are anonymous and freely mintable via
// /api/orion-session, so accepting them turned the auth gate into a two-step
// bypass). The positive-path tests need an enterprise key, not a session
// token. ORION_SESSION_SECRET is set so the session module loads without throw;
// SESSION_TOKEN is kept around so the explicit "ors_ tokens are rejected"
// regression test below can prove the bypass is closed.
process.env.ORION_SESSION_SECRET ||= 'test-secret-must-be-at-least-32-chars-long-xxx';
...
function buildHeaders(origin, { authed = true, extra = {} } = {}) {
  ...
  if (authed) h['X-ORION-Key'] = ENTERPRISE_KEY;
  return h;
}
...
function makeGetRequest(params = {}, origin = 'https://orion.app', opts = {}) {
  const url = new URL('https://orion.app/api/mcp-proxy');
...
function makePostRequest(body = {}, origin = 'https://orion.app', opts = {}) {
  return new Request('https://orion.app/api/mcp-proxy', {
    method: 'POST',
...
function makeOptionsRequest(origin = 'https://orion.app') {
  return new Request('https://orion.app/api/mcp-proxy', {
    method: 'OPTIONS',
    headers: { origin },
  });
...
describe('api/mcp-proxy', () => {
  ...
  describe('Auth gate', () => {
    it('returns 401 when no X-ORION-Key is provided', async () => {
      const res = await handler(makeGetRequest({ serverUrl: 'https://mcp.example.com/mcp' }, 'https://orion.app', { authed: false }));
    ...
    it('returns 401 for POST without key', async () => {
      const res = await handler(makePostRequest({ serverUrl: 'https://mcp.example.com/mcp', toolName: 'search' }, 'https://orion.app', { authed: false }));
      assert.equal(res.status, 401);
    });
    ...
    // ors_ session tokens are anonymous and freely mintable by any caller
    // via POST /api/orion-session. Without forceKey:true, they would pass the
    // auth gate — turning the gate into a two-step bypass (mint + call).
    // PR #3768 review finding; closes the residual #3723 surface.
    // ors_ session tokens are anonymous and freely mintable via
    // /api/orion-session. The auth gate must reject them — otherwise the
    // bypass is "mint, then proxy". isCallerPremium does this by
    // requiring keyCheck.required === true (ors_ short-circuits at
    // required:false). PR #3768 review regression.
    it('rejects a ors_ session token even though it is technically valid', async () => {
      const url = new URL('https://orion.app/api/mcp-proxy');
      url.searchParams.set('serverUrl', 'https://mcp.example.com/mcp');
      ...
      assert.equal(res.status, 401, 'ors_ session token must NOT unlock /api/mcp-proxy');
    ...
    it('rejects wm_ user keys when Convex validation cannot run / returns null', async () => {
      ...
      const req = new Request(url.toString(), {
        method: 'GET',
        headers: { origin: 'https://orion.app', 'X-ORION-Key': 'wm_user_abc123' },
      });
      const res = await handler(req);
      assert.equal(res.status, 401);
    });
  ...
  describe('CORS and method handling', () => {
    ...
    it('returns 405 for DELETE', async () => {
      const res = await handler(new Request('https://orion.app/api/mcp-proxy', {
        method: 'DELETE',
        headers: { origin: 'https://orion.app', 'X-ORION-Key': ENTERPRISE_KEY },
      }));
  ...
  describe('GET /api/mcp-proxy (list tools)', () => {
    ...
    it('ignores invalid JSON in headers param', async () => {
      ...
      const req = new Request(url.toString(), { method: 'GET', headers: { origin: 'https://orion.app', 'X-ORION-Key': ENTERPRISE_KEY } });
      const res = await handler(req);
      assert.equal(res.status, 200);
    });
