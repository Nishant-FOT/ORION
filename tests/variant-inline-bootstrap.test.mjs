...
const variantBootstrapScript = inlineScripts.find(
  (script) => script.includes('orion-variant') && script.includes('document.documentElement.dataset.variant'),
);
  ...
  it('detects every public variant host before the app bundle loads', () => {
    for (const variant of ['happy', 'tech', 'finance', 'commodity', 'energy']) {
      assert.ok(
        ...
        `index.html inline bootstrap must set data-variant for ${variant}.orion.app`,
      );
    }
  });
