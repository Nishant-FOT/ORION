...
describe('getRelayHeaders — no auth', () => {
  ...
  it('aviation User-Agent override works', () => {
    const h = getRelayHeaders({ 'User-Agent': 'ORION-Server/1.0' });
    assert.equal(h['User-Agent'], 'ORION-Server/1.0');
  });
});
...
describe('getRelayHeaders — with auth', () => {
  ...
  it('respects custom RELAY_AUTH_HEADER', () => {
    withEnv({ RELAY_AUTH_HEADER: 'x-orion-relay' }, () => {
      const h = getRelayHeaders();
      assert.equal(h['x-orion-relay'], 'test-secret-abc');
...
describe('relay.ts — consumer import verification', () => {
  const consumers = [
    { file: 'server/orion/aviation/v1/_shared.ts', importPath: '../../../_shared/relay' },
    { file: 'server/orion/intelligence/v1/_relay.ts', importPath: '../../../_shared/relay' },
    { file: 'server/orion/research/v1/list-tech-events.ts', importPath: '../../../_shared/relay' },
    { file: 'server/orion/maritime/v1/get-vessel-snapshot.ts', importPath: '../../../_shared/relay' },
    { file: 'server/orion/market/v1/_shared.ts', importPath: '../../../_shared/relay' },
    { file: 'server/orion/news/v1/list-feed-digest.ts', importPath: '../../../_shared/relay' },
    { file: 'server/orion/military/v1/list-military-flights.ts', importPath: '../../../_shared/relay' },
  ];
  ...
  it('no local getRelayBaseUrl/getRelayHeaders/getRelayRequestHeaders definitions in server/ (except _shared/relay.ts)', () => {
    const dupeFiles = [
      'server/orion/aviation/v1/_shared.ts',
      'server/orion/intelligence/v1/_relay.ts',
      'server/orion/research/v1/list-tech-events.ts',
      'server/orion/maritime/v1/get-vessel-snapshot.ts',
      'server/orion/market/v1/_shared.ts',
      'server/orion/news/v1/list-feed-digest.ts',
      'server/orion/military/v1/list-military-flights.ts',
    ];
  ...
  it('military list-military-flights.ts uses getRelayBaseUrl() not raw WS_RELAY_URL + /opensky', () => {
    const src = readFileSync(resolve('server/orion/military/v1/list-military-flights.ts'), 'utf-8');
    assert.ok(
