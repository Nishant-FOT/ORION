...
describe('widget-agent relay — security', () => {
  ...
  it('SSRF guard — allowlist is checked before any fetch call in tool loop', () => {
    ...
    // The fetch call to api.orion.app must come AFTER the check
    const fetchCallIdx = relay.indexOf("'https://api.orion.app'", allowlistCheck);
    assert.ok(
      fetchCallIdx > allowlistCheck,
      'fetch() to api.orion.app must appear after allowlist check',
    );
  });
  ...
  it('CORS reuses getCorsOrigin (not a narrow hardcoded origin list)', () => {
    ...
    assert.ok(
      !corsBlock.includes("['https://orion.app'"),
...
describe('widget-store — constants and logic', () => {
  ...
  it('storage key is orion-custom-widgets', () => {
    assert.ok(
      store.includes("'orion-custom-widgets'"),
      "Storage key must be 'orion-custom-widgets'",
    );
  });

  it('auth gate migrates orion-widget-key to an HttpOnly session instead of storing it', () => {
    assert.ok(
      store.includes("'orion-widget-key'"),
      "Feature gate must know the legacy 'orion-widget-key' name for migration",
    );
    assert.ok(
      store.includes('migrateLegacyKeysToHttpOnlySession') &&
        browserKeySession.includes('establishOrionKeySession'),
      'Widget key writes must go through the server session endpoint',
    );
    assert.ok(
      !/localStorage\.setItem\(['"]orion-widget-key['"]/.test(store),
      'orion-widget-key must not be written to localStorage',
    );
    assert.ok(
      !/document\.cookie\s*=.*orion-widget-key.*encodeURIComponent\(.*key/s.test(store),
      'orion-widget-key must not be written to a JS-readable cookie',
    );
  });
  ...
  it('deleteWidget cleans orion-panel-spans (aggregate map)', () => {
    assert.ok(
  ...
  it('deleteWidget cleans orion-panel-col-spans (aggregate map)', () => {
    assert.ok(
...
describe('proxy routing — widgetAgentUrl', () => {
  ...
  it('widgetAgentUrl targets proxy.orion.app (not toRuntimeUrl)', () => {
    // The URL may be in a constant above the function; search the whole file
    assert.ok(
      proxy.includes('proxy.orion.app'),
      'Must target proxy.orion.app directly (sidecar destroys SSE via arrayBuffer)',
    );
  ...
  it('vite.config.ts proxies /widget-agent to proxy.orion.app', () => {
    assert.ok(
      vite.includes('/widget-agent'),
      'vite.config.ts must have proxy entry for /widget-agent',
    );
    assert.ok(
      vite.includes('proxy.orion.app'),
      'Vite proxy target must be proxy.orion.app',
    );
  });
...
describe('CustomWidgetPanel — header buttons and events', () => {
  ...
  it('renderWidget uses shared wrapped widget HTML helper', () => {
    ...
    assert.ok(
      sanitizer.includes('orion-widget-generated'),
...
describe('PRO widget — store and sanitizer', () => {
  const store = src('src/services/widget-store.ts');
  const san = src('src/utils/widget-sanitizer.ts');
  const sandbox = src('public/orion-widget-sandbox.html');
  ...
  it('PRO auth migrates orion-pro-key to an HttpOnly session instead of storing it', () => {
    const browserKeySession = src('src/services/browser-key-session.ts');
    assert.ok(
      store.includes("'orion-pro-key'"),
      "isProWidgetEnabled must know the legacy 'orion-pro-key' name for migration",
    );
    assert.ok(
    ...
    assert.ok(
      !/localStorage\.setItem\(['"]orion-pro-key['"]/.test(store),
      'orion-pro-key must not be written to localStorage',
    );
    assert.ok(
      !/document\.cookie\s*=.*orion-pro-key.*encodeURIComponent\(.*key/s.test(store),
      'orion-pro-key must not be written to a JS-readable cookie',
    );
  });

  it('user identity migrates legacy orion-pro-key instead of reading localStorage directly', () => {
    ...
    assert.ok(
      ...
      'user-identity must route legacy orion-pro-key through the HttpOnly migration helper',
    );
    assert.ok(
      !/localStorage\.getItem\(['"]orion-pro-key['"]/.test(identity),
      'user-identity must not read orion-pro-key directly from localStorage',
    );
  });

  it('PRO HTML stored in separate orion-pro-html-{id} key', () => {
    assert.ok(
      store.includes('orion-pro-html-'),
      "PRO HTML must be stored in 'orion-pro-html-{id}' separate localStorage key",
    );
  });
  ...
  it('loadWidgets hydrates PRO HTML from separate key', () => {
    ...
    assert.ok(
      loadBody.includes('proHtml') || loadBody.includes('orion-pro-html'),
      'loadWidgets must read PRO HTML from separate key',
    );
  });

  it("loadWidgets drops PRO entry when orion-pro-html-{id} is missing", () => {
    const loadIdx = store.indexOf('function loadWidgets');
    const loadBody = store.slice(loadIdx, loadIdx + 600);
    assert.ok(
  ...
  it('deleteWidget removes orion-pro-html-{id} key', () => {
    ...
    assert.ok(
      deleteBody.includes('orion-pro-html') || deleteBody.includes('proHtmlKey'),
      'deleteWidget must also remove the orion-pro-html-{id} key',
    );
  });
  ...
  it('wrapProWidgetHtml uses sandbox page src (not srcdoc) for CSP isolation', () => {
    ...
    assert.ok(
      fnBody.includes('orion-widget-sandbox.html'),
  ...
  it('PRO widget iframe uses nonce handshake before posting HTML', () => {
    assert.ok(
      san.includes('data-orion-token') && san.includes('orion-widget-ready'),
  ...
  it('PRO widget postMessage targetOrigins match the opaque sandbox model', () => {
    const parentDelivery = san.match(
      /iframe\.contentWindow\?\.postMessage\(\s*\{[\s\S]*?type:\s*['"]orion-html['"][\s\S]*?\},\s*(['"])\*\1,\s*\)/,
    );
    assert.ok(
      parentDelivery,
    ...
    const readyDelivery = sandbox.match(
      /window\.parent\.postMessage\(\s*\{[\s\S]*?type:\s*['"]orion-widget-ready['"][\s\S]*?\},\s*parentOrigin,\s*\)/,
    );
    assert.ok(
      readyDelivery,
    ...
    assert.ok(
      !/window\.parent\.postMessage\(\s*\{[\s\S]*?type:\s*['"]orion-widget-ready['"][\s\S]*?\},\s*(['"])\*\1/.test(sandbox),
  ...
  it('widget sandbox allows approved Vercel previews and rejects lookalike origins', () => {
    assert.ok(
      sandbox.includes("url.hostname === 'orion.app'")
        && sandbox.includes("url.hostname.endsWith('.orion.app')"),
      'sandbox must parse hostname and allow the orion.app apex/subdomains only',
    );
    assert.ok(
      !sandbox.includes("endsWith('orion.app')") && !sandbox.includes('endsWith("orion.app")'),
      'sandbox must not use raw suffix checks that allow evilorion.app',
    );
    ...
    // step with isAllowedVercelPreview in public/orion-widget-sandbox.html.
    const matchesAllowedTeam = (hostname) =>
      slugs.some((team) =>
        new RegExp('^orion-[a-z0-9-]+-' + team + '\\.vercel\\.app$').test(hostname),
      );
    assert.equal(matchesAllowedTeam('orion-git-feature-eliewm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('orion-abc123-eliewm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('orion-feature-attacker.vercel.app'), false);
    assert.equal(matchesAllowedTeam('orion-git-feature-eliewm.vercel.app.evil.com'), false);
    assert.equal(matchesAllowedTeam('orion-feature-xeliewm.vercel.app'), false);
    assert.equal(matchesAllowedTeam('evilorion.app'), false);
    // The retired personal scope (orion-*-elie-<hash>) must no longer match.
    assert.equal(matchesAllowedTeam('orion-feature-elie-abc123.vercel.app'), false);
    ...
    assert.equal(matchesWithTeammate('orion-feature-kieran.vercel.app'), true);
    assert.equal(matchesWithTeammate('orion-feature-kieranfake.vercel.app'), false);
  });
  ...
  it('widget sandbox behavior accepts Vercel previews and blocks spoofed parents', () => {
    ...
    function runSandbox(referrer) {
      ...
      const context = {
        ...
        window: {
          location: { hash: '#id=orion-1&token=test-token' },
          parent,
    ...
    const allowed = runSandbox('https://orion-git-feature-eliewm.vercel.app/dashboard');
    assert.equal(allowed.readyMessages.length, 1);
    assert.equal(allowed.readyMessages[0].targetOrigin, 'https://orion-git-feature-eliewm.vercel.app');
    assert.deepEqual(JSON.parse(JSON.stringify(allowed.readyMessages[0].payload)), {
      type: 'orion-widget-ready',
      id: 'orion-1',
      token: 'test-token',
    });
    allowed.message({
      data: { type: 'orion-html', id: 'orion-1', token: 'test-token', html: '<p>ok</p>' },
      origin: 'https://orion-git-feature-eliewm.vercel.app',
      source: allowed.parent,
    });
    assert.deepEqual(allowed.writes, ['<p>ok</p>']);

    const spoofed = runSandbox('https://orion-git-feature-eliewm.vercel.app.evil.com/');
    assert.deepEqual(spoofed.readyMessages, []);
    spoofed.message({
      data: { type: 'orion-html', id: 'orion-1', token: 'test-token', html: '<p>bad</p>' },
      origin: 'https://orion-git-feature-eliewm.vercel.app.evil.com',
  ...
  it('PRO widget re-deliveries are rate-limited to bound document.write storms', () => {
    ...
    // window.location.hash and re-posts orion-widget-ready could trigger an
...
describe('PRO widget — i18n keys and CSS', () => {
  ...
  it('.orion-widget-pro iframe CSS sets 400px height', () => {
    assert.ok(
      css.includes('.orion-widget-pro'),
      'CSS must target .orion-widget-pro for PRO iframe container',
    );
    const proIdx = css.indexOf('.orion-widget-pro');
...
// "PRO key rejected. Update orion-pro-key…" 403 in the modal — the user has no
...
describe('WidgetChatModal — preflight 403 message branches on auth mode', () => {
  ...
  it('en.json defines widgets.preflightProSubscriptionRequired (just-upgraded / outage)', () => {
    ...
    assert.ok(
      !/orion-pro-key/i.test(en.widgets.preflightProSubscriptionRequired),
      'preflightProSubscriptionRequired must not mention orion-pro-key — Clerk users have no tester key',
    );
  });
  ...
  it('en.json defines widgets.preflightProRequired (free-user upgrade ask, no "just upgraded" language)', () => {
    ...
    assert.ok(
      !/orion-pro-key/i.test(en.widgets.preflightProRequired),
      'preflightProRequired must not mention orion-pro-key',
    );
    assert.ok(
