// Tests for the auto-refresh layers added to orion-session.ts:
//
...
//     - A 401 from the API triggers ensureOrionSession() and a single replay.
//     - Premium-RPC paths short-circuit BEFORE the ors_ branch — no retry.
//     - When the caller already supplied Authorization, the ors_ branch
//       is skipped — no retry.
...
// ---------------------------------------------------------------------------
// Stub browser globals BEFORE the orion-session module is imported. The module
...
// transitively via runtime.ts → orion-session.ts) reads `location.hostname` at
...
let mod: typeof import('../src/services/orion-session.ts');
let wrappedFetch: typeof fetch;

before(async () => {
  mod = await import('../src/services/orion-session.ts');
  mod.installOrionSessionFetchInterceptor();
...
beforeEach(() => {
  ...
  // ensureOrionSession's storage path) would short-circuit the next test's
  // mint attempt.
...
function setStoredSessionExp(_token: string, expMs: number): void {
  memoryStorage.set('orion-session-exp', JSON.stringify({ exp: expMs }));
}
...
// Force the in-memory `cached` state by calling the module's API. ensureOrionSession
...
// ensureOrionSession's storage path to populate `cached`.
async function primeCachedFromStorage(): Promise<void> {
  await mod.ensureOrionSession();
}
...
describe('orion-session periodic refresh (Layer 1)', () => {
  it('skips the periodic mint when document is hidden', async () => {
    ...
    currentFetchHandler = (input) => {
      ...
      if (url.includes('/api/orion-session')) {
        mintCalls += 1;
  ...
  it('skips the periodic mint when the cached token is still fresh', async () => {
    setStoredSessionExp('ors_fresh', FAR_FUTURE);
  ...
  it('visibilitychange handler does NOT mint when the cached token is fresh', async () => {
    setStoredSessionExp('ors_fresh_visible', FAR_FUTURE);
...
describe('orion-session refresh-on-401 (Layer 2)', () => {
  it('retries an API 401 with a freshly-minted token', async () => {
    ...
    const resp = await wrappedFetch('https://api.orion.app/api/bootstrap');
  ...
  it('does NOT retry when the path is in PREMIUM_RPC_PATHS', async () => {
    setStoredSessionExp('ors_anything', FAR_FUTURE);
    await primeCachedFromStorage();
    ...
    const resp = await wrappedFetch('https://api.orion.app/api/market/v1/analyze-stock');
    assert.equal(resp.status, 401);
    assert.equal(attempts, 1, 'premium path must NOT trigger a retry inside this interceptor');
    assert.equal(mintCalls, 0, 'premium path must NOT mint a ors_ token (the dedicated injector handles it)');
  });
  ...
  it('does NOT retry when the caller supplied Authorization', async () => {
    ...
    assert.equal(resp.status, 401);
    assert.equal(attempts, 1, 'caller-supplied Authorization must NOT be retried by the ors_ interceptor');
    assert.equal(mintCalls, 0, 'caller-supplied Authorization must NOT trigger a ors_ mint');
  ...
  it('returns the second 401 if the retry also fails (no infinite loop)', async () => {
    ...
    assert.deepEqual(warnings, [
      '[orion-session] API request still returned 401 after refreshing HttpOnly session cookie',
    ]);
  });
});
