 ...
 *  - orion-pro-key 401 → retries with orion-widget-key before Clerk
 ...
 *  - orion-pro-key / orion-widget-key order is deterministic and deduped
 */
...
const TARGET = 'https://api.orion.app/api/sanctions/v1/list-sanctions-pressure';
// A real PUBLIC path used to verify the path-gating bypass: hits below
// fetch the same way but should NOT see Bearer attached.
const PUBLIC_TARGET = 'https://api.orion.app/api/economic/v1/get-fred-series-batch';
const PUBLIC_INSIDER_TRANSACTIONS_TARGET =
  'https://api.orion.app/api/market/v1/get-insider-transactions?symbol=AAPL';
...
describe('premiumFetch', () => {
  ...
  it('passthrough when Authorization header already set', async () => {
    ...
    assert.equal(sentHeaders().get('X-ORION-Key'), null);
  });

  it('passthrough when X-ORION-Key header already set', async () => {
    setup();
    await premiumFetch(TARGET, { headers: { 'X-ORION-Key': 'caller-key' } });
    assert.equal(fetchMock.mock.calls.length, 1);
    assert.equal(sentHeaders().get('X-ORION-Key'), 'caller-key');
  });
  ...
  it('tester key: valid key accepted — exactly one fetch, key forwarded', async () => {
    ...
    assert.equal(sentHeaders().get('X-ORION-Key'), 'valid-gateway-key');
  });
  ...
  it('tester key: 401 falls through to Clerk JWT (two fetches)', async () => {
    ...
    assert.equal(sentHeaders(0).get('X-ORION-Key'), 'widget-only-key');
    ...
    assert.equal(sentHeaders(1).get('X-ORION-Key'), null);
  });

  it('orion-pro-key 401 retries with orion-widget-key before Clerk', async () => {
    let n = 0;
    setup({
    ...
    assert.equal(sentHeaders(0).get('X-ORION-Key'), 'relay-only-pro-key');
    assert.equal(sentHeaders(0).get('Authorization'), null);
    assert.equal(sentHeaders(1).get('X-ORION-Key'), 'valid-widget-key');
  ...
  // Bearer JWT to non-premium calls suppresses the orion-session
  // interceptor's ors_ attach (premiumFetch sets Authorization → the
  ...
  // falls through and the interceptor attaches ors_) saw the data.
  //
  // The fix: only attach Bearer when the path is in PREMIUM_RPC_PATHS.
  // Public paths fall through so the orion-session interceptor handles
  // ors_ attach.
  ...
  it('non-premium path: tester key still attached (works on any path)', async () => {
    ...
    assert.equal(sentHeaders(0).get('X-ORION-Key'), 'valid-key');
    assert.equal(sentHeaders(0).get('Authorization'), null);
  });
