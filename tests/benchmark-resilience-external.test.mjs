...
describe('benchmark CLI exit policy', () => {
  it('does not fail non-strict cron mode for cold-start skips', () => {
    assert.equal(benchmarkCliExitCode({ skipped: true, reason: 'no-orion-scores' }, false), 0);
    assert.equal(benchmarkCliExitCode({ skipped: true, reason: 'no-orion-scores' }, true), 1);
  });
