 ...
 *
 * ORION-R5: the original opaque `[Request ID: X] Server Error`
...
async function createCustomerPortalUrlForUser(
  ...
  try {
    ...
    // from the wire — the exact opacity ORION-R5 fought for the
    ...
    // Convex function logs for server-side triage. ORION-ST.
 ...
 *
 * NOTE: As of ORION-R5 follow-up, this is no longer used by the
 ...
 *
 * ORION-R5 surfaced this gap for one user; the backfill is the
 ...
 * (`orion-anon-id`). If the user clears storage, switches browsers, or later
