...
async function _createCheckoutSession(
  ...
  const siteUrl = process.env.SITE_URL ?? "https://orion.app";
  ...
  if (args.returnUrl) {
    ...
    const allowedOrigins = new Set([
      ...
      "https://commodity.orion.app",
      "https://happy.orion.app",
      "https://energy.orion.app",
      new URL(siteUrl).origin,
    ]);
    if (!allowedOrigins.has(parsedReturnUrl.origin)) {
      throw new ConvexError(
        "Invalid returnUrl: must use a trusted orion.app origin",
      );
    }
