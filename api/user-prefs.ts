...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      // event per user (5ev/5u over a week — ORION-QK), which a
      ...
      if (kind === 'SERVICE_UNAVAILABLE') {
        ...
        // dashboard or page on-call (ORION-QA).
  ...
  try {
    ...
    if (kind === 'SERVICE_UNAVAILABLE') {
      ...
      // event stays queryable but doesn't page on-call (ORION-QA).
 ...
 * totals and alerting (per ORION-PX 2026-04-30 triage).
 *
...
function handleConflictResponse(
  ...
  // users via user_id distribution (see ORION-PX 2026-04-30:
...
export function buildSentryContext(
  ...
  const errorShape = opts.errorShapeOverride
    ...
    // ORION-PG: the JSON-cased variant was previously falling
    ...
    ?? (/UNAUTHENTICATED|"code":\s*"Unauthenticated"/.test(msg) ? 'convex_auth_drift'
      ...
      // and genuine 503s when triaging (ORION-PG).
      ...
      // transients apart from genuine Convex platform 5xx (ORION-PG).
