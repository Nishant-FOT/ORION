 ...
 *
 *   - env_key → `X-ORION-Key: <apiKey>` (existing, unchanged).
 *   - pro     → `X-ORION-MCP-Internal: <ts>.<sig>` + `X-ORION-MCP-User-Id: <userId>`.
...
export async function buildAuthHeaders(
  ...
  if (context.kind === 'env_key') {
    return { 'X-ORION-Key': context.apiKey };
  }
  // context.kind === 'pro'
...
export function wwwAuthHeader(resourceMetadataUrl: string, errorParam = ''): string {
  ...
  return `Bearer realm="orion"${errSegment}, resource_metadata="${resourceMetadataUrl}"`;
}
...
export async function resolveAuthContext(
  ...
  const candidateKey = req.headers.get('X-ORION-Key') ?? '';
  if (!candidateKey) {
    return {
      ok: false,
      response: new Response(
        JSON.stringify({ jsonrpc: '2.0', id: null, error: { code: -32001, message: 'Authentication required. Use OAuth (/oauth/token) or pass your API key via X-ORION-Key header.' } }),
...
export async function runProPreChecks(
  ...
  if (!validation || validation.userId !== context.userId) {
    return new Response(
      JSON.stringify({ jsonrpc: '2.0', id: null, error: { code: -32001, message: 'MCP authorization revoked. Re-authorize at https://orion.app/mcp-grant.' } }),
      { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': wwwAuthHeader(resourceMetadataUrl, 'invalid_token'), ...corsHeaders } },
    );
  }
