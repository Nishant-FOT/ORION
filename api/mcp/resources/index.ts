...
// (e.g. `orion://countries/{iso2}/risk`) verbatim. Strict 2025-06-18
...
export const RESOURCE_REGISTRY: McpResourceDef[] = [
  {
    ...
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('orion://countries/')) return null;
      const m = /^orion:\/\/countries\/([a-z]{2})\/risk$/.exec(uri);
      const iso2 = m?.[1];
      if (!iso2) {
        return {
          ok: false,
          reason: 'Expected orion://countries/{iso2}/risk where {iso2} is lowercase ISO 3166-1 alpha-2.',
        };
      }
  ...
  {
    uri: 'orion://chokepoints/{slug}/status',
    ...
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('orion://chokepoints/')) return null;
      const m = /^orion:\/\/chokepoints\/([a-z][a-z0-9-]*)\/status$/.exec(uri);
      const slug = m?.[1];
      if (!slug) {
        return {
          ok: false,
          reason: 'Expected orion://chokepoints/{slug}/status where {slug} is a hand-curated kebab-case identifier.',
        };
      }
  ...
  },
  {
    ...
    paramExtractor: (uri: string) => {
      if (uri !== 'orion://seed-meta/freshness') {
        if (uri.startsWith('orion://seed-meta/')) {
          return { ok: false, reason: 'Expected orion://seed-meta/freshness (no further path segments).' };
        }
        return null;
      }
  ...
  {
    uri: 'orion://markets/{symbol}/quote',
    name: 'Market Quote',
    ...
    tool: 'get_market_data',
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('orion://markets/')) return null;
      ...
      const m = /^orion:\/\/markets\/([A-Z][A-Z0-9.=-]{0,15})\/quote$/.exec(uri);
      const symbol = m?.[1];
      if (!symbol) {
        return {
          ok: false,
          reason: 'Expected orion://markets/{symbol}/quote where {symbol} is an uppercase ticker (e.g. "AAPL", "GC=F", "BTC-USD").',
        };
      }
