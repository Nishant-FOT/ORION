...
function makeReq({ origin, referer, secFetchSite, key, cookie } = {}) {
  ...
  return new Request('https://api.orion.app/api/test', { headers });
}

// ── #3541 regression: header-only signals must NEVER pass ──────────────────

test('#3541: forged Referer alone is rejected', async () => {
  const r = await validateApiKey(makeReq({ referer: 'https://orion.app/' }));
...
test('#3541: forged Origin: https://orion.app alone is rejected (no key, no session)', async () => {
  const r = await validateApiKey(makeReq({ origin: 'https://orion.app' }));
  assert.equal(r.valid, false);
});
...
test('valid ors_ session token from any origin is accepted (forceKey=false)', async () => {
...
test('valid HttpOnly orion-session cookie is accepted without JS-readable auth header', async () => {
  const { token } = await issueSessionToken();
  const r = await validateApiKey(makeReq({ cookie: `orion-session=${encodeURIComponent(token)}` }));
  assert.equal(r.valid, true);
  assert.equal(r.required, false);
  assert.equal(r.kind, 'session');
});

test('HttpOnly orion-pro-key cookie is accepted as enterprise key without a JS-readable header', async () => {
  const r = await validateApiKey(makeReq({ cookie: `orion-pro-key=${encodeURIComponent(ENTERPRISE_KEY)}` }));
  assert.equal(r.valid, true);
  assert.equal(r.required, true);
  assert.equal(r.kind, 'enterprise');
});

test('dual orion-pro-key cookies use the first value sent by the browser', async () => {
  const r = await validateApiKey(makeReq({
    cookie: `orion-pro-key=${encodeURIComponent(ENTERPRISE_KEY)}; orion-pro-key=old-js-readable-key`,
  }));
  assert.equal(r.valid, true);
  assert.equal(r.required, true);
  assert.equal(r.kind, 'enterprise');
...
test('PR #3557 review: ors_ session token is REJECTED when forceKey=true (premium endpoints)', async () => {
  // ors_ tokens are anonymous and freely mintable via /api/orion-session — they
...
test('PR #3557 review: ors_ result must NOT carry kind=enterprise (gateway entitlement-bypass anti-regression)', async () => {
  // Gateway skips entitlement check ONLY for kind:'enterprise'. If a future
  // refactor mislabels ors_ as enterprise, anonymous tokens silently unlock
...
test('valid ors_ session token works even when Origin is also forged (not redundant — no privilege escalation)', async () => {
  const { token } = await issueSessionToken();
  const r = await validateApiKey(makeReq({ origin: 'https://evil.example.com', key: token }));
  assert.equal(r.valid, true);
});

test('tampered ors_ token is rejected', async () => {
  const { token } = await issueSessionToken();
...
test('garbage ors_ shape is rejected', async () => {
  const r = await validateApiKey(makeReq({ key: 'ors_garbage' }));
  assert.equal(r.valid, false);
});

// ── Enterprise key (ORION_VALID_KEYS) ────────────────────────────────
...
test('REGRESSION: wm_-prefixed key in ORION_VALID_KEYS is honored as enterprise', async () => {
  // Pre-#3541 the static enterprise allowlist accepted any key shape, so
  // some operator-issued keys (e.g. Railway ORION_RELAY_KEY) carry the
  ...
  // present in ORION_VALID_KEYS.
  const previous = process.env.ORION_VALID_KEYS;
  process.env.ORION_VALID_KEYS = `${ENTERPRISE_KEY},wm_legacy_operator_key_in_static_list`;
  try {
    ...
    process.env.ORION_VALID_KEYS = previous;
  }
});
...
test('desktop Tauri origin with ors_ session token is rejected (desktop must use enterprise key)', async () => {
  const { token } = await issueSessionToken();
...
//   - ors_ token + forceKey=true → REJECTED (PR #3557 review fix)
//   - ors_ token + forceKey=false → accepted
