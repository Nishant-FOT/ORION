 ...
 *        https://api.orion.app/api/seed-contract-probe
 *
...
interface BoundaryCheck {
   ...
   *  intended code-path served the response. */
...
export async function checkPublicBoundary(
  ...
  // ONLY no-Pro path now is a ors_-prefixed HMAC-signed session token. Mint one
  // here ourselves — we share the ORION_SESSION_SECRET environment with the
  // /api/orion-session endpoint, so an in-process issue is equivalent to round-
  // tripping through it (and avoids the extra network hop).
  // If ORION_SESSION_SECRET isn't configured, fall back to the bare request — the
  ...
  const headers: Record<string, string> = {
    ...
    'User-Agent': 'ORION-SeedContractProbe/1.0',
  };
  if (sessionToken) headers['X-ORION-Key'] = sessionToken;
