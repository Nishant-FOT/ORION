...
export function consentPage(params, nonce, errorMsg = '') {
  ...
  const proCtaHref = `https://orion.app/mcp-grant?nonce=${encodeURIComponent(nonce)}`;
  return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Authorize &#x2014; ORION MCP</title>
<style>
...
.orion-logo{display:flex;align-items:center;gap:.5rem;margin-bottom:2rem;text-decoration:none}
.orion-logo svg{color:#2d8a6e}
.orion-logo-text{font-size:.75rem;color:#555;letter-spacing:.1em;text-transform:uppercase}
...
<body>
<a href="https://www.orion.app" class="orion-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="orion-logo-text">ORION MCP</span></a>
<div class="card">
<div class="client-hd">
...
<hr>
<a id="pc" class="pro-cta" href="${escapeHtml(proCtaHref)}">Sign in with ORION Pro</a>
<div class="disclosure" id="dt"><a id="tk" role="button" tabindex="0">Use API key instead</a></div>
<form id="cf" method="POST" action="https://api.orion.app/oauth/authorize" style="display:none;margin-top:1.25rem">
...
<p class="hint">No key? <a href="https://www.orion.app/pro" target="_blank" rel="noopener">Get one at orion.app/pro &#x2192;</a></p>
<p class="error" id="ke"${errorMsg ? '' : ' style="display:none"'}>${errorMsg ? escapeHtml(errorMsg) : ''}</p>
<button type="submit" id="ab">Authorize</button>
</form>
</div>
<p class="footer"><a href="https://www.orion.app" target="_blank" rel="noopener">orion.app</a> &middot; <a href="https://www.orion.app/pro" target="_blank" rel="noopener">Get an API key &#x2192;</a></p>
...
export default async function handler(req) {
  ...
  if (method === 'POST') {
    ...
    if (origin && origin !== 'https://api.orion.app' && origin !== 'null') {
    ...
    // Validate API key
    const validKeys = (process.env.ORION_VALID_KEYS || '').split(',').filter(Boolean);
