 ...
 *   2. Browser tester key (X-ORION-Key) — validated against
 *      ORION_VALID_KEYS so one browser-held key can unlock premium
...
const RELAY_BASE = 'https://proxy.orion.app';
const WIDGET_AGENT_KEY = process.env.WIDGET_AGENT_KEY ?? '';
const PRO_WIDGET_KEY = process.env.PRO_WIDGET_KEY ?? '';
const ORION_VALID_KEY_SET = new Set(
  (process.env.ORION_VALID_KEYS ?? '')
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean),
);

function hasValidORIONKey(key: string): boolean {
  return Boolean(key) && ORION_VALID_KEY_SET.has(key);
}
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-ORION-Key, X-Api-Key, X-Widget-Key, X-Pro-Key',
      },
    });
  }
  ...
  const headerORIONKey =
    req.headers.get('X-ORION-Key') ??
    req.headers.get('X-Api-Key') ??
    '';
  const worldMonitorKey =
    headerORIONKey ||
    getCookie(req, 'orion-pro-key') ||
    getCookie(req, 'orion-widget-key');
  if (hasValidORIONKey(worldMonitorKey)) {
    isPro = true;
  } else {
    const authHeader = req.headers.get('Authorization');
    if (authHeader?.startsWith('Bearer ')) {
      // Web users authenticate with a bearer session.
      const session = await validateBearerToken(authHeader.slice(7));
      if (!session.valid) {
        return json({ error: 'Invalid or expired session' }, 401, corsHeaders);
      }
      const allowed = session.role === 'pro';
      if (!allowed) {
      ...
      // Legacy tester key path (orion-widget-key / orion-pro-key)
      const widgetKey = req.headers.get('X-Widget-Key') || getCookie(req, 'orion-widget-key');
      const proKey = req.headers.get('X-Pro-Key') || getCookie(req, 'orion-pro-key');
  ...
  const relayHeaders: Record<string, string> = {
    ...
    'User-Agent': 'orion-widget-edge/1.0',
