...
test('validateSessionToken rejects a token signed with a different secret', async () => {
  ...
  process.env.ORION_SESSION_SECRET = 'different-secret-also-32-chars-or-longer-yyyy';
  try {
...
test('validateSessionToken rejects garbage input', async () => {
  ...
  assert.equal(await validateSessionToken('ors_'), false);
  assert.equal(await validateSessionToken('ors_no-dot'), false);
  assert.equal(await validateSessionToken('ors_a.'), false);
  assert.equal(await validateSessionToken('ors_.b'), false);
...
test('isSessionTokenShape only matches ors_ prefix', () => {
  assert.equal(isSessionTokenShape('ors_abc'), true);
...
test('issueSessionToken throws when ORION_SESSION_SECRET is missing/short (fail closed)', async () => {
  const stash = process.env.ORION_SESSION_SECRET;
  process.env.ORION_SESSION_SECRET = 'too-short';
  try {
    await assert.rejects(() => issueSessionToken(), /ORION_SESSION_SECRET/);
  } finally {
    process.env.ORION_SESSION_SECRET = stash;
  }
