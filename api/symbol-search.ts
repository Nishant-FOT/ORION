...
// `orion-edge/1.0` convention used by api/notify.ts and
// api/notification-channels.ts.
const FETCH_USER_AGENT = 'orion-edge/1.0';
...
export default async function handler(
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      // (ORION-RE) on real users typing things like "$" or "< "
      ...
      // only pages at warning on an unactionable transient (ORION-RE). A
