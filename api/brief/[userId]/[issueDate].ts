...
function renderErrorPage(heading: string, body: string): string {
  return (
    ...
    + `<title>${heading} · ORION</title>`
    + `<style>${ERROR_PAGE_STYLES}</style>`
    + '</head><body><div>'
    + `<h1>${heading}</h1>`
    + `<p>${body}</p>`
    + '<p><a href="https://orion.app">Return to ORION</a></p>'
    + '</div></body></html>'
  );
}
...
const FORBIDDEN_PAGE = renderErrorPage(
  ...
  "The brief link you followed is incomplete or has been tampered with. Open the most recent notification from ORION to read today's brief.",
);
  ...
  try {
    const res = await fetch(`${convexSiteUrl}/relay/followed-countries`, {
      ...
      headers: {
        ...
        'User-Agent': 'orion-brief-render/1.0',
      },
