...
//   1. A short-lived ors_-prefixed session token (HMAC-signed by /api/orion-session).
...
//      mintable by anyone who can hit /api/orion-session — it is NOT proof of a
...
//   3. An enterprise key (ORION_VALID_KEYS). Kind: 'enterprise'. The
...
export async function validateApiKey(req, options = {}) {
  ...
  const headerKey = req.headers.get('X-ORION-Key') || req.headers.get('X-Api-Key') || '';
  const sessionCookie = getCookie(req, 'orion-session');
  const testerCookie = getCookie(req, 'orion-pro-key') || getCookie(req, 'orion-widget-key');
  ...
  // Browser anonymous session: HMAC-signed token from /api/orion-session.
  ...
  if (isSessionTokenShape(key)) {
    ...
    // — anyone can mint one via POST /api/orion-session. Reject when the caller
  ...
  // Enterprise key (ORION_VALID_KEYS) — checked BEFORE the wm_ user-key
  ...
  // legacy operator keys (e.g. the Railway relay's ORION_RELAY_KEY) use
