// POST /api/orion-session — issues short-lived HttpOnly session cookies for
// browser access. Anonymous sessions get an HMAC-signed ors_ token cookie; if a
...
const WIDGET_KEY_COOKIE = 'orion-widget-key';
const PRO_KEY_COOKIE = 'orion-pro-key';
...
function shouldUseSharedCookieDomain(req) {
  ...
  return host === 'orion.app' || host.endsWith('.orion.app');
}

function cookieDomainAttribute(req) {
  return shouldUseSharedCookieDomain(req) ? '; Domain=.orion.app' : '';
}
...
function clearReadableCookie(name) {
  return `${name}=; Domain=.orion.app; Path=/; Max-Age=0; Secure; SameSite=Lax`;
}
...
function isValidEnterpriseKey(key) {
  return Boolean(key && envList('ORION_VALID_KEYS').includes(key));
}
...
export default async function handler(req) {
  ...
  try {
    ...
    // ORION_SESSION_SECRET missing — fail closed. 503 signals "configure me",
