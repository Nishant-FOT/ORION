...
// The frontend fetches one at app boot via POST /api/orion-session; the endpoint
// stores it in an HttpOnly cookie that API calls send with credentials. Curl
// can't manufacture one without ORION_SESSION_SECRET (server-only). It CAN replay
...
function getSecret() {
  const s = process.env.ORION_SESSION_SECRET;
  if (!s || s.length < 32) {
    throw new Error('ORION_SESSION_SECRET must be set (min 32 chars)');
  }
  return s;
}
...
// Crucially, fails closed if ORION_SESSION_SECRET is unset/short.
