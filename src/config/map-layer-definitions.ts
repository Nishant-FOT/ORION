...
export const LAYER_REGISTRY: Record<keyof MapLayers, LayerDefinition> = {
  ...
  liveTankers:              def('liveTankers',              '&#128674;', 'liveTankers',              'Live Tanker Positions', ['flat']),
  energySupplyNetwork:      def('energySupplyNetwork',      '&#9889;',   'energySupplyNetwork',      'Energy Supply Network', ['flat']),
  chokepointMonitoring:     def('chokepointMonitoring',     '&#9875;',   'chokepointMonitoring',     'Chokepoint Monitoring', ['flat']),
};
...
export const LAYER_EXPLANATIONS: Partial<Record<keyof MapLayers, LayerExplanation>> = {
  conflicts: {
    ...
    source: 'ORION conflict-zone registry, UCDP/ACLED conflict context, and documented boundary metadata such as the Korean DMZ.',
  ...
  ciiChoropleth: {
    ...
    source: 'ORION CII scoring service using conflict, unrest, advisories, cyber, AIS, aviation, natural-event, and news signals.',
  ...
  natural: {
    ...
    evidence: ['docs/data-sources.mdx', 'docs/architecture.mdx', 'server/orion/natural/v1/list-natural-events.ts'],
  },
  flights: {
    key: 'flights',
    coverage: 'curated',
  ...
  ais: {
    ...
    source: 'AISStream relay snapshots, ORION maritime service, and chokepoint disruption classifiers.',
  ...
  waterways: {
    ...
    source: 'ORION strategic-waterways registry with supply-chain chokepoint status overlays from AIS, NGA warnings, and PortWatch-derived feeds.',
    ...
    evidence: ['docs/architecture.mdx', 'docs/data-sources.mdx', 'src/config/geo.ts', 'server/orion/supply-chain/v1/get-chokepoint-status.ts'],
  },
  tradeRoutes: {
    key: 'tradeRoutes',
    coverage: 'curated',
    ...
    source: 'ORION trade-route registry plus supply-chain chokepoint status and transit summaries.',
  ...
  hotspots: {
    ...
    source: 'ORION hotspot registry, RSS/GDELT news intelligence, hotspot escalation scoring, military activity, and CII context.',
...
const VARIANT_LAYER_ORDER: Record<MapVariant, Array<keyof MapLayers>> = {
  full: [
    ...
    'ais', 'tradeRoutes', 'protests',
    'ucdpEvents', 'displacement', 'climate', 'weather',
    'outages', 'natural', 'fires',
    'waterways', 'economic', 'minerals', 'gpsJamming',
    'satellites', 'sanctions', 'pipelines',
    'storageFacilities', 'fuelShortages', 'commodityPorts',
  ],
  tech: [
    'startupHubs', 'techHQs', 'accelerators', 'cloudRegions',
    'datacenters', 'cables', 'outages',
    'techEvents', 'natural', 'fires',
  ],
  finance: [
 ...
 * Used by both the layer picker UI and the CMD+K dispatcher to
 * hide / silently-skip toggles that would be a no-op.
 *
 * Rules:
