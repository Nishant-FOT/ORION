import { establishOrionKeySession } from '@/services/orion-session';

export type BrowserSessionKeyName = 'orion-widget-key' | 'orion-pro-key';
...
function clearLegacyReadableCookie(name: BrowserSessionKeyName): void {
  try {
    document.cookie = `${name}=; domain=.orion.app; path=/; max-age=0; SameSite=Lax; Secure`;
...
export async function migrateLegacyKeysToHttpOnlySession(keys: {
  ...
  const ok = await establishOrionKeySession({
    ...(widgetKey ? { widgetKey } : {}),
    ...(proKey ? { proKey } : {}),
  });
  if (!ok) return false;

  if (widgetKey) clearLegacyKeyStorage('orion-widget-key');
  if (proKey) clearLegacyKeyStorage('orion-pro-key');
  return true;
}
