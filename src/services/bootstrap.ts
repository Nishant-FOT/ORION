...
export async function fetchBootstrapData(): Promise<void> {
  ...
  // - 5.0 s gives slow connections and cold CDN caches enough headroom to avoid
  //   the cascade where the hydration cache stays empty and every panel falls
  //   through to individual RPCs that also time out.
  ...
  const slowTimeout = setTimeout(() => slowCtrl.abort(), desktop ? 8_000 : 5_000);
