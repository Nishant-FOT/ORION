...
export const disasterAdapter: DomainAdapter = {
  ...
  collectSignals(ctx: AppContext): SignalEvidence[] {
    ...
    const cache = ctx.intelligenceCache as {
      earthquakes?: Array<{ occurredAt?: number; location?: { latitude?: number; longitude?: number }; magnitude: number; place: string }>;
      protests?: { events?: Array<{ time?: { getTime?: () => number }; country?: string }> };
      outages?: Array<{ pubDate?: { getTime?: () => number }; country?: string; lat?: number; lon?: number; severity: string; title: string }>;
    };
