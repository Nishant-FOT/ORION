...
export const militaryAdapter: DomainAdapter = {
  ...
  collectSignals(ctx: AppContext): SignalEvidence[] {
    ...
    const cache = ctx.intelligenceCache as {
      military?: {
        flights?: Array<{ lastSeen?: { getTime?: () => number }; aircraftType: string; lat: number; lon: number; operatorCountry: string; operator: string; callsign: string }>;
        vessels?: Array<{ lastAisUpdate?: { getTime?: () => number }; isDark?: boolean; aisGapMinutes?: number; vesselType: string; lat: number; lon: number; operatorCountry: string; operator: string; name: string }>;
      };
    };
