...
import type { Earthquake } from '@/generated/client/orion/seismology/v1/service_client';
...
// Keep these aligned with server/orion/intelligence/v1/_risk-config.ts.
...
export function ingestAdvisoriesForCII(advisories: SecurityAdvisory[]): void {
  ...
  for (const [code, fallbackLevel] of Object.entries(ADVISORY_LEVELS_FALLBACK)) {
    if (!countryDataMap.has(code)) countryDataMap.set(code, initCountryData());
    const data = countryDataMap.get(code)!;
    if (!data.advisoryMaxLevel) {
      data.advisoryMaxLevel = fallbackLevel;
      data.advisoryCount = Math.max(data.advisoryCount, 1);
    }
  }
}
...
function calcConflictScore(data: CountryData, countryCode: string): number {
  ...
  if (events.length > 0) {
    ...
    const battleCount = events.filter(e => e.eventType === 'battle').reduce((sum, e) => sum + ((now - e.time.getTime()) <= SEVEN_DAYS ? 1.0 : 0.4), 0);
    const explosionCount = events.filter(e => e.eventType === 'explosion' || e.eventType === 'remote_violence').reduce((sum, e) => sum + ((now - e.time.getTime()) <= SEVEN_DAYS ? 1.0 : 0.4), 0);
    const civilianCount = events.filter(e => e.eventType === 'violence_against_civilians').reduce((sum, e) => sum + ((now - e.time.getTime()) <= SEVEN_DAYS ? 1.0 : 0.4), 0);
    const totalFatalities = events.reduce((sum, e) => {
      const weight = (now - e.time.getTime()) <= SEVEN_DAYS ? 1.0 : 0.4;
      return sum + e.fatalities * weight;
    }, 0);
...
export function calculateCII(): CountryScore[] {
  ...
  for (const code of countryCodes) {
    ...
    const { score, components } = calculateCountryScoreSnapshot(code, data);
