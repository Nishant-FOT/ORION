...
} from '@/generated/client/orion/infrastructure/v1/service_client';
...
const STATUS_REVERSE: Record<string, CableHealthStatus> = {
  ...
  CABLE_HEALTH_STATUS_OK: 'healthy',
  CABLE_HEALTH_STATUS_UNSPECIFIED: 'unknown',
};

function toRecord(cableId: string, proto: ProtoCableHealthRecord): CableHealthRecord {
  return {
    cableId,
    cableName: cableId,
    status: STATUS_REVERSE[proto.status] || 'unknown',
    lastChecked: proto.lastUpdated ? new Date(proto.lastUpdated).toISOString() : new Date().toISOString(),
  };
}
...
export async function fetchCableHealth(): Promise<CableHealthResponse> {
  ...
  for (const [id, proto] of Object.entries(resp.cables)) {
    cables[id] = toRecord(id, proto);
  }

  const result: CableHealthResponse = {
    overallStatus: 'unknown',
    lastUpdated: resp.generatedAt ? new Date(resp.generatedAt).toISOString() : new Date().toISOString(),
    cables,
  };
