...
const POST_CHECKOUT_FLAG_KEY = 'orion-post-checkout';
const APP_CHECKOUT_BASE_URL = 'https://orion.app/dashboard';
...
export async function startCheckout(
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      // for our own JSON error envelopes. ORION-RN.
...
function reportCheckoutError(
  ...
  const payload = {
    ...
    tags: {
      ...
      // definitive for Cloudflare emission. ORION-RN.
...
function renderCheckoutErrorSurface(
  ...
  if (fallbackToPricingPage) {
    window.location.assign('https://orion.app/pro');
    return;
  }
