 ...
 *
 * Injects a Clerk Bearer token (or ORION_API_KEY as fallback) directly
 ...
 *
 *   1. premiumFetch sets Authorization → orion-session interceptor sees it
 *      and steps aside (no `X-ORION-Key: ors_…` is attached).
 ...
 *   3. api/_api-key.js `validateApiKey()` reads ONLY X-ORION-Key.
 ...
 * attaches ors_) saw the data normally — the inverse of the expected
 * "Pro sees more" behaviour.
 *
 * Fix: don't attach Bearer for non-premium paths. Fall through to the
 * unauth path so the orion-session interceptor handles the ors_ attach.
 * API-key holders (step 1) and tester-key holders (step 2) are unaffected
 * — those keys travel via X-ORION-Key which works on any path.
 */
...
function reportServerError(res: Response, input: RequestInfo | URL): void {
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://orion.app').pathname;
    ...
    // (ORION-RG). Genuine origin 5xx (500-511) stay at `error`.
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-ORION-Key')) {
    const res = await globalThis.fetch(input, withCredentials(init));
    reportServerError(res, input);
    return res;
  }

  // 1. ORION_API_KEY from env (desktop / test environments).
  try {
    ...
    if (wmKey) {
      existing.set('X-ORION-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-ORION-Key', testerKey);
  ...
  //    endpoints, attaching Bearer would suppress the orion-session
  //    interceptor's ors_ attach and produce a 401 (see the file-level
  //    comment for the full chain). Falling through to step 4 instead lets
  //    the interceptor attach ors_ and the gateway accept it.
  ...
  // For NON-premium paths this lands on the orion-session interceptor (which
  // attaches ors_) → gateway accepts → 200. For premium paths reached here
