...
const DEFAULT_WEB_API_URL = 'https://api.orion.app';
...
function isORIONWebHost(hostname: string): boolean {
  return hostname === 'orion.app'
    || hostname === 'www.orion.app'
    || hostname.endsWith('.orion.app');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://orion.app';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.orion.app');
  } catch {
    return false;
  }
}
...
export function installRuntimeFetchPatch(): void {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const debug = localStorage.getItem('orion-debug-log') === '1';
    ...
    if (!localApiToken || tokenExpired) {
      try {
        const { tryInvokeTauri } = await import('@/services/tauri-bridge');
        localApiToken = (await tryInvokeTauri<string>('get_local_api_token')) ?? null;
    ...
    if (allowCloudFallback && !isKeyFreeApiTarget(target)) {
      try {
        ...
        const wmKeyState = getSecretState('ORION_API_KEY');
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        const wmKeyValue = getRuntimeConfigSnapshot().secrets['ORION_API_KEY']?.value;
        if (wmKeyValue) {
          cloudHeaders.set('X-ORION-Key', wmKeyValue);
        }
      }
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.orion.app/api/...) — just inject auth.
