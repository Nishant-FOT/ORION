    ...
    cableIndexPromise = import('@/config/geo-map')
      .then(({ UNDERSEA_CABLES }) => {
        cableIndex = UNDERSEA_CABLES.map((cable) => {
          const mid = midpoint(cable.points as [number, number][]);
  ...
  switch (type) {
    case 'pipeline':
      return PIPELINES.map(pipeline => {
        const mid = midpoint(pipeline.points as [number, number][]);
        if (!mid) return null;
  ...
  for (const type of types) {
    ...
    if (type === 'nuclear') {
      ...
      const byOperator = (nuclearFacilities ?? [])
        ...
        .map(f => ({ id: f.id, name: f.name, type, lat: f.lat, lon: f.lon, distanceKm: haversineDistanceKm(lat, lon, f.lat, f.lon) }));
      if (byOperator.length > 0) {
        countryAssets = byOperator.sort((a, b) => (a.distanceKm ?? 0) - (b.distanceKm ?? 0));
      }
    }
