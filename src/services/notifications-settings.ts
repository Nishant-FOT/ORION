...
export function renderNotificationsSettings(host: NotificationsSettingsHost): NotificationsSettingsResult {
  ...
  let html = '';
  html += `<div class="orion-pref-group-content orion-notif-tab-content">`;
  if (host.isSignedIn) {
    ...
    html += `<button type="button" class="panel-locked-cta" id="usNotifSignInBtn">Sign In to Enable Notifications</button>`;
  }
  html += `</div>`;
    ...
    attach(container: HTMLElement): () => void {
      ...
      function reloadNotifSection(): void {
        ...
        getChannelsData().then((data) => {
          ...
          if (loadingEl) {
            const isAuth = err instanceof Error && (err.message.includes('Not authenticated') || err.message.includes('null'));
            loadingEl.innerHTML = isAuth
              ? `<div class="ai-flow-toggle-desc">Sign in to configure notifications.</div><button type="button" class="panel-locked-cta" id="usNotifSignInBtn">Sign In</button>`
      ...
      container.addEventListener('click', (e) => {
        ...
        const startTelegramPairing = (rowEl: HTMLElement) => {
          ...
          createPairingToken().then(({ token, expiresAt }) => {
            if (signal.aborted) return;
            const botUsername = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TELEGRAM_BOT_USERNAME as string | undefined) ?? 'ORIONBot';
      ...
      const onMessage = (e: MessageEvent): void => {
        const trustedOrigin = e.origin === window.location.origin ||
          e.origin === 'https://orion.app' ||
          e.origin === 'https://www.orion.app' ||
          e.origin.endsWith('.orion.app');
