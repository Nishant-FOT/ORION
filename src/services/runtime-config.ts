...
const TOGGLES_STORAGE_KEY = 'orion-runtime-feature-toggles';
...
if (typeof window !== 'undefined') {
  window.addEventListener('storage', (e) => {
    if (e.key === 'orion-secrets-updated') {
...
export async function setSecretValue(key: RuntimeSecretKey, value: string): Promise<void> {
  ...
  try {
    localStorage.setItem('orion-secrets-updated', String(Date.now()));
