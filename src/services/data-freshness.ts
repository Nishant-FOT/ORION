...
const SOURCE_METADATA: Record<DataSourceId, { name: string; requiredForRisk: boolean; panelId?: string }> = {
  ...
  'thermal-escalation': { name: 'Thermal Escalation', requiredForRisk: false, panelId: 'map' },
  'cross-source-signals': { name: 'Cross-Source Signals', requiredForRisk: false, panelId: 'intel' },
};
...
const INTELLIGENCE_GAP_MESSAGES: Record<DataSourceId, string> = {
  ...
  'thermal-escalation': 'Thermal escalation data unavailable—satellite thermal monitoring offline',
  'cross-source-signals': 'Cross-source intelligence signals unavailable—multi-source correlation degraded',
};
