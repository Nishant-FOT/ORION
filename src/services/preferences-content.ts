...
export function renderPreferences(host: PreferencesHost): PreferencesResult {
  ...
  // ── Display group ──
  html += `<details class="orion-pref-group" open>`;
  html += `<summary>${t('preferences.display')}</summary>`;
  html += `<div class="orion-pref-group-content">`;
  ...
  // ── Intelligence group ──
  html += `<details class="orion-pref-group">`;
  ...
  if (!host.isDesktopApp && host.isSignedIn && isCloudSyncEnabled()) {
    ...
    html += `<div class="orion-pref-group-content">`;
    html += `<div class="orion-sync-status-row">
      <div class="orion-sync-status-info">
        <span class="orion-sync-status-dot" id="usSyncDot" style="background:${SYNC_STATE_COLORS[syncState] ?? SYNC_STATE_COLORS.error}"></span>
        <span class="orion-sync-status-label" id="usSyncLabel">${SYNC_STATE_LABELS[syncState] ?? 'Unknown'}</span>
        <span class="orion-sync-status-time" id="usSyncTime">Last synced: ${escapeHtml(lastSyncStr)}</span>
      </div>
      <button type="button" class="btn btn-secondary orion-sync-now-btn" id="usSyncNowBtn">Sync now</button>
    </div>`;
    html += `</div></details>`;
  }
