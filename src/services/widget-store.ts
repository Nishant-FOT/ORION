...
const STORAGE_KEY = 'orion-custom-widgets';
...
// Legacy orion-widget-key / orion-pro-key values used to live in localStorage and
// JS-readable cookies. New writes go to /api/orion-session, which sets short-lived
...
function migrateLegacyKeyStorage(): void {
  ...
  const widgetKey = readLegacySessionKey('orion-widget-key');
  const proKey = readLegacySessionKey('orion-pro-key');
...
export function setWidgetKey(key: string): void {
  ...
  if (!trimmed) {
    clearLegacyKeyStorage('orion-widget-key');
    return;
  }
...
export function setProKey(key: string): void {
  ...
  if (!trimmed) {
    clearLegacyKeyStorage('orion-pro-key');
    return;
  }
