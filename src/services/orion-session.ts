...
// short-lived HMAC-signed token via POST /api/orion-session. The token is stored
// by the server in an HttpOnly cookie; JavaScript only tracks the expiry.
//
// Two pieces:
//   1. ensureOrionSession() — asks the server to mint/refresh the HttpOnly cookie.
//   2. installOrionSessionFetchInterceptor() — patch globalThis.fetch ONCE so
...
async function fetchNewSession(body?: { widgetKey?: string; proKey?: string }): Promise<StoredSession | null> {
  try {
    ...
    const resp = await fetchImpl(toApiUrl('/api/orion-session'), {
      method: 'POST',
...
export async function ensureOrionSession(): Promise<boolean> {
...
export function getWmSessionToken(): string | null {
  ...
  // be fresh by calling ensureOrionSession().
  return null;
}

export async function establishOrionKeySession(keys: { widgetKey?: string; proKey?: string }): Promise<boolean> {
...
// Production code never imports this — it's exclusively for `tests/orion-session-*`.
//
// `interceptorInstalled` is also reset so a test that calls this followed by
// `installOrionSessionFetchInterceptor()` actually re-runs the install path
...
//
// Decide whether a fetch URL should go through the ors_-injection branch.
// Exported (and named with no implementation detail in its signature) so the
// regression test in tests/orion-session-interceptor-target.test.mts can lock the
...
//      under-matches → ors_ never attached → 401 on every browser request.
//
...
//      string as a prefix (e.g. `https://api.orion.app.evil.example/`)
//      OR as the userinfo portion (`https://api.orion.app@evil/`)
//      slip through, sending the ors_ token to a foreign host. Bug class:
...
// If a caller already set Authorization / X-ORION-Key / X-Api-Key, we
// don't override — Clerk Bearer JWT and explicit user keys still take
// precedence over the anonymous session token.
export function installOrionSessionFetchInterceptor(): void {
  ...
  // request to https://api.orion.app/api/* — the interceptor only
  // catches relative '/api/' paths, the ors_ token never gets attached, and
  ...
  window.fetch = async function wmSessionFetch(input, init) {
    ...
    // ORION_API_KEY, or tester key based on what the user has). Stepping
  ...
  //      tabs all hitting /api/orion-session in parallel).
  ...
  if (typeof setInterval === 'function') {
    setInterval(() => {
      ...
      ensureOrionSession().catch(() => { /* best-effort */ });
    }, PERIODIC_REFRESH_MS);
  }
