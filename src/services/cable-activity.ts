...
import { MaritimeServiceClient, type NavigationalWarning } from '@/generated/client/orion/maritime/v1/service_client';
...
function findNearestCable(lat: number, lon: number): UnderseaCable | null {
  ...
  for (const cable of cablesData) {
    for (const point of cable.points) {
      const cableLon = point[0];
      const cableLat = point[1];
      if (cableLon == null || cableLat == null) continue;
  ...
  for (const warning of cableWarnings) {
    ...
    if (!seenIds.has(advisoryId)) {
      ...
      advisories.push({
        id: advisoryId,
        cableId: matchedCable?.id || 'unknown',
        cableName: matchedCable?.name || 'Unknown Cable',
        type: 'advisory',
        title,
        ...
        reported: issueDate.toISOString(),
        source: 'NGA',
        lat,
        lon,
        impact: isOperation
