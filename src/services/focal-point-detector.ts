...
class FocalPointDetector {
  ...
  analyze(clusters: ClusteredEvent[], signalSummary: SignalSummary): FocalPointSummary {
    ...
    this.lastSummary = {
      ...
      signalCountryCount: signalSummary.topCountries.length,
    };
      ...
      for (const entity of context.entities) {
        ...
        if (existing) {
          ...
          if (existing.topHeadlines.length < 3 && titleHasEntity) {
            existing.topHeadlines.push({ title: cluster.primaryTitle, source: cluster.primarySource, link: cluster.primaryLink });
          }
        } else {
          mentions.set(entity.entityId, {
            entityId: entity.entityId,
            entityType: entityEntry.type as EntityMention['entityType'],
            displayName: entityEntry.name,
            mentionCount: 1,
            avgConfidence: entity.confidence,
            clusterIds: [clusterId],
            // Only include headline if entity appears in title
            topHeadlines: titleHasEntity ? [{ title: cluster.primaryTitle, source: cluster.primarySource, link: cluster.primaryLink }] : [],
          });
        }
      }
    }
