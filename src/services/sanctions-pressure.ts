...
} from '@/generated/client/orion/sanctions/v1/service_client';
...
export async function fetchSanctionsPressure(): Promise<SanctionsPressureResult> {
  ...
  // Try bootstrap endpoint first (works without server-side auth),
  // then fall back to the RPC via breaker.
  try {
