...
const TABS_STORAGE_KEY = `orion-tabs-v1:${SITE_VARIANT}`;
 ...
 * Dynamic panels (custom widgets, MCP panels, desktop runtime-config) survive
 * with their current config from the previous tab.
 */
export function buildDefaultTabPanels(
  ...
  const variantDefaults = VARIANT_DEFAULTS[SITE_VARIANT] ?? VARIANT_DEFAULTS.full ?? [];

  const panelSettings: Record<string, PanelConfig> = {};
  for (const key of variantDefaults) {
    if (currentPanelSettings[key]) {
      panelSettings[key] = currentPanelSettings[key];
    } else {
      panelSettings[key] = { enabled: true } as PanelConfig;
    }
  }

  for (const key of Object.keys(currentPanelSettings)) {
    if (isDynamicPanel(key) && !panelSettings[key]) {
      panelSettings[key] = currentPanelSettings[key] ?? { enabled: true } as PanelConfig;
    }
  }

  const panelOrder = [...variantDefaults.filter((key) => !!panelSettings[key])];
