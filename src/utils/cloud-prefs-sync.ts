...
const KEY_SYNC_VERSION = 'orion-cloud-sync-version';
const KEY_LAST_SYNC_AT = 'orion-last-sync-at';
const KEY_SYNC_STATE = 'orion-cloud-sync-state';
const KEY_LAST_SIGNED_IN_AS = 'orion-last-signed-in-as';
...
const KEY_LOCAL_SCHEMA_VERSION = 'orion-cloud-prefs-local-schema-version';
...
function showUndoToast(prevBlobJson: string): void {
  document.querySelector('.orion-sync-restore-toast')?.remove();

  const toast = document.createElement('div');
  toast.className = 'orion-sync-restore-toast update-toast';
