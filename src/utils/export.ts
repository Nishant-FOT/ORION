...
interface IntelExportData {
  protests?: { events: Array<{ title: string; country: string; eventType: string; severity: string; time: Date }> };
  earthquakes?: Array<{ place: string; magnitude: number; depthKm: number; occurredAt: number; sourceUrl: string }>;
  outages?: Array<{ title: string; country: string; severity: string; pubDate: Date; link: string }>;
  flightDelays?: Array<{ name: string; iata: string; city: string; country: string; delayType: string; severity: string; avgDelayMinutes: number; source: string }>;
  military?: {
    flights?: Array<{ callsign: string; hexCode: string; aircraftType: string; operator: string; operatorCountry: string; lat: number; lon: number }>;
    vessels?: Array<{ name: string; mmsi: string; operatorCountry: string; vesselType: string; lat: number; lon: number }>;
  };
  iranEvents?: Array<{ title: string; category: string; locationName: string; severity: string; timestamp: string }>;
  orefAlerts?: { alertCount: number; historyCount24h: number };
  advisories?: Array<{ title: string; source: string; level: string | null; country: string | null; pubDate: Date; link: string }>;
  radiation?: { observations: Array<{ location: string; country: string; value: number; unit: string; observedAt: Date }> };
  imageryScenes?: Array<{ id: string; satellite: string; datetime: string; resolutionM: number; mode: string }>;
  sanctions?: { totalCount: number; sdnCount: number; newEntryCount: number };
  thermalEscalation?: { summary: { clusterCount: number; elevatedCount: number } };
  usniFleet?: { vessels?: unknown[] };
...
export function exportToJSON(data: ExportData, filename = 'orion-export'): void {
  const jsonStr = JSON.stringify(sanitizeData(data), null, 2);
  downloadFile(jsonStr, `${filename}.json`, 'application/json');
}

export function exportToCSV(data: ExportData, filename = 'orion-export'): void {
  const clean = sanitizeData(data);
  const lines: string[] = [];

  lines.push(`# ORION Export — ${new Date(clean.timestamp).toISOString()}`);
  ...
  const intel = clean.intelligence as IntelExportData | undefined;
  if (intel) {
  ...
  if (clean.monitors && clean.monitors.length > 0) {
    ...
    clean.monitors.forEach(m => {
      lines.push(csvRow([m.id, m.keywords.join(';'), m.color]));
    });
    lines.push('');
  }
...
export const COUNTRY_EVIDENCE_PROVENANCE_DISCLAIMER =
  'This ORION evidence bundle packages user-visible context and source metadata for analyst handoff. It is not a legal evidentiary record; verify source availability, timestamps, and claims before reuse.';
...
export function renderCountryEvidenceMarkdown(bundle: CountryEvidenceBundle): string {
  const lines: string[] = [];
  lines.push(`# ORION Evidence Bundle: ${bundle.country} (${bundle.code})`);
  lines.push('');
...
export class ExportPanel {
  ...
  private export(format: ExportFormat): void {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `orion-${timestamp}`;
