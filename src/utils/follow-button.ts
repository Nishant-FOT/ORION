...
function renderHtml(state: ButtonViewState, props: FollowButtonProps): string {
  ...
  const sizeCls = `orion-follow-btn--${props.size ?? 'md'}`;
  ...
  if (state.loading) {
    return (
      `<button type="button" class="orion-follow-btn ${sizeCls} orion-follow-btn--loading"` +
      ` data-country="${safeCode}" data-state="loading"` +
      ` aria-label="Loading follow state for ${safeName}"` +
      ` title="Syncing your follows…" disabled>` +
      `<span class="orion-follow-btn-spinner" aria-hidden="true"></span>` +
      `</button>`
    );
  }
  ...
  if (state.followed) {
    return (
      `<button type="button" class="orion-follow-btn ${sizeCls} orion-follow-btn--followed"` +
      ` data-country="${safeCode}" data-state="followed"` +
      ` aria-pressed="true"` +
      ` aria-label="Unfollow ${safeName}"` +
      ` title="Unfollow ${safeName}">` +
      // Filled star
      `<svg class="orion-follow-btn-icon" width="16" height="16" viewBox="0 0 24 24"` +
  ...
  return (
    `<button type="button" class="orion-follow-btn ${sizeCls} orion-follow-btn--unfollowed${
      state.atCap ? ' orion-follow-btn--at-cap' : ''
    }"` +
      ...
      const clickHandler = (ev: Event) => {
        ...
        const btn =
          target && typeof (target as Element).closest === 'function'
            ? (target as Element).closest<HTMLElement>('.orion-follow-btn')
            : null;
        if (!btn) return;
