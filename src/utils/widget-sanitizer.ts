...
export function wrapWidgetHtml(html: string, extraClass = ''): string {
  const shellClass = ['orion-widget-shell', extraClass].filter(Boolean).join(' ');
  return `
    <div class="${shellClass}">
      <div class="orion-widget-body">
        <div class="orion-widget-generated">${sanitizeWidgetHtml(stripLeadingPanelHeader(html))}</div>
      </div>
    </div>
  `;
}
...
// clears this floor; a malicious widget script that re-posts orion-widget-
...
function mountProWidget(iframe: HTMLIFrameElement): void {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!event.data || event.data.type !== 'orion-widget-ready') return;
    ...
    iframe.contentWindow?.postMessage(
      { type: 'orion-html', id: mounted.id, token: mounted.token, html: storedHtml },
      '*',
    );
    ...
    for (const mut of mutations) {
      for (const node of mut.addedNodes) {
        ...
        if (node instanceof HTMLIFrameElement && node.dataset.wmId) {
          ...
          node.querySelectorAll<HTMLIFrameElement>('iframe[data-orion-id]').forEach(mountProWidget);
        }
      }
      for (const node of mut.removedNodes) {
        ...
        if (node instanceof HTMLIFrameElement && node.dataset.wmId) {
          ...
          node.querySelectorAll<HTMLIFrameElement>('iframe[data-orion-id]').forEach(unmountProWidget);
        }
      }
    }
  });
...
export function wrapProWidgetHtml(bodyContent: string): string {
  const id = `orion-${Math.random().toString(36).slice(2)}`;
  const token = createWidgetToken();
  widgetBodyStore.set(id, stripLeadingPanelHeader(bodyContent));
  const src = `/orion-widget-sandbox.html#id=${encodeURIComponent(id)}&token=${encodeURIComponent(token)}`;
  return `<div class="orion-widget-shell orion-widget-pro"><iframe src="${src}" data-orion-id="${id}" data-orion-token="${token}" sandbox="allow-scripts" style="width:100%;height:400px;border:none;display:block;" title="Interactive widget"></iframe></div>`;
}
