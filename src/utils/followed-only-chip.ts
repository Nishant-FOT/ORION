 ...
 *  - `localStorage["orion-followed-only-filter-${panelId}"] = '1' | '0'`
...
const STORAGE_KEY_PREFIX = 'orion-followed-only-filter-';
...
function renderHtml(state: ViewState): string {
  ...
  const cls = [
    'orion-followed-only-chip',
    state.active ? 'orion-followed-only-chip--active' : '',
    state.disabled ? 'orion-followed-only-chip--disabled' : '',
  ]
    .filter(Boolean)
    .join(' ');
  ...
  return (
    ...
    `<svg class="orion-followed-only-chip-icon" width="12" height="12" viewBox="0 0 24 24"` +
    ` fill="${state.active ? 'currentColor' : 'none'}"` +
    ` stroke="currentColor" stroke-width="2"` +
    ` stroke-linejoin="round" aria-hidden="true">` +
    `<path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>` +
    `</svg>` +
    `<span class="orion-followed-only-chip-label">${safeLabel}</span>` +
    `</button>`
  );
}
      ...
      const clickHandler = (ev: Event): void => {
        ...
        const btn =
          target && typeof (target as Element).closest === 'function'
            ? (target as Element).closest<HTMLElement>('.orion-followed-only-chip')
            : null;
        if (!btn) return;
