// Styles removed — will be rebuilt with new UI
...
async function diagFetch(path: string, init?: RequestInit): Promise<Response> {
  if (!_diagToken) {
    try {
      _diagToken = (await tryInvokeTauri<string>('get_local_api_token')) ?? null;
...
function renderOverview(area: HTMLElement): void {
  ...
  const wmState = getSecretState('ORION_API_KEY');
  ...
  setTrustedHtml(area, trustedHtml(`
    ...
    <div class="settings-ov-license">
      <section class="orion-section">
        <h2 class="orion-section-title">${t('modals.settingsWindow.worldMonitor.apiKey.title')}</h2>
        <p class="orion-section-desc">${t('modals.settingsWindow.worldMonitor.apiKey.description')}</p>
        <div class="orion-key-row">
          <div class="orion-input-wrap">
            <input type="password" class="orion-input" data-orion-key-input
              placeholder="${t('modals.settingsWindow.worldMonitor.apiKey.placeholder')}"
              autocomplete="off" spellcheck="false"
              ${wmState.present ? `value="${MASKED_SENTINEL}"` : ''} />
            <button type="button" class="orion-toggle-vis" data-orion-toggle title="Show/hide">&#x1f441;</button>
          </div>
          <span class="orion-badge ${wmStatusClass}">${wmStatusText}</span>
        </div>
      </section>

      <div class="orion-divider"><span>${t('modals.settingsWindow.worldMonitor.dividerOr')}</span></div>

      <section class="orion-section">
        <h2 class="orion-section-title">${t('modals.settingsWindow.worldMonitor.register.title')}</h2>
        <p class="orion-section-desc">${t('modals.settingsWindow.worldMonitor.register.description')}</p>
        <div class="orion-register-row">
          <button type="button" class="orion-submit-btn" data-orion-open-pro>
...
function initOverviewListeners(area: HTMLElement): void {
  area.querySelector('[data-orion-toggle]')?.addEventListener('click', () => {
    const input = area.querySelector<HTMLInputElement>('[data-orion-key-input]');
    if (input) input.type = input.type === 'password' ? 'text' : 'password';
  });

  area.querySelector<HTMLInputElement>('[data-orion-key-input]')?.addEventListener('input', (e) => {
  ...
  area.querySelector('[data-orion-open-pro]')?.addEventListener('click', () => {
    const url = 'https://orion.app/pro';
...
function initDiagnostics(): void {
  ...
  if (fetchDebugToggle) {
    fetchDebugToggle.checked = localStorage.getItem('orion-debug-log') === '1';
    fetchDebugToggle.addEventListener('change', () => {
      localStorage.setItem('orion-debug-log', fetchDebugToggle.checked ? '1' : '0');
    });
  }
...
async function initSettingsWindow(): Promise<void> {
  ...
  document.getElementById('okBtn')?.addEventListener('click', () => {
    void (async () => {
      try {
        const wmKeyInput = document.querySelector<HTMLInputElement>('[data-orion-key-input]');
        ...
        if (hasWmKeyChange && wmKeyValue) {
          await setSecretValue('ORION_API_KEY', wmKeyValue);
        }
...
localStorage.setItem('orion-settings-open', '1');
window.addEventListener('beforeunload', () => localStorage.removeItem('orion-settings-open'));
