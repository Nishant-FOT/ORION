...
// @ts-expect-error — harness-only import, module may not exist in all builds
...
const [cableLon, cableLat] = firstPathPoint(UNDERSEA_CABLES as unknown as Array<{ points: [number, number][] }>, [38.0, 20.0]);
const [pipelineLon, pipelineLat] = firstPathPoint(PIPELINES as unknown as Array<{ points: [number, number][] }>, [45.0, 30.0]);
  ...
  const cableAdvisories: CableAdvisory[] = [
    {
      ...
      source: 'e2e-harness',
      reported: new Date('2026-02-01T08:00:00.000Z').toISOString(),
      lat: 12.2,
      lon: 45.2,
