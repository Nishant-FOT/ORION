...
export interface SwUpdateHandlerOptions {
  ...
  /** Enable debug logging. Defaults to localStorage.getItem('orion-debug-sw') === '1'. */
  debug?: boolean;
...
// ---------------------------------------------------------------------------
// Debug logging (opt-in via localStorage.setItem('orion-debug-sw', '1'))
// Persists a rolling 30-entry log in sessionStorage so it survives page reloads.
// Copy with: JSON.parse(sessionStorage.getItem('orion-sw-debug-log'))
// ---------------------------------------------------------------------------

export const SW_DEBUG_LOG_KEY = 'orion-sw-debug-log';
...
export function installSwUpdateHandler(options: SwUpdateHandlerOptions = {}): void {
  ...
  const debugEnabled = options.debug ?? (() => {
    try { return localStorage.getItem('orion-debug-sw') === '1'; } catch { return false; }
  })();
