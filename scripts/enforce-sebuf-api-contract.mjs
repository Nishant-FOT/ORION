 ...
 *      generated service_server under src/generated/server/orion/<snake_domain>/v<N>/.
...
const GEN_SERVER_DIR = join(ROOT, 'src/generated/server/orion');
...
//      e.g. /api/v2/shipping/*).
// Both map to src/generated/server/orion/<snake_domain>/v<N>/.
  ...
  if (gatewayMatch) {
    ...
    if (!existsSync(expectedServer)) {
      violation(
        ...
        `Expected ${relative(ROOT, expectedServer)}. Either regenerate (cd proto && buf generate), restore the proto under proto/orion/${snakeDomain}/v${version}/service.proto, or delete this orphaned gateway.`,
      );
    }
    continue;
  }
