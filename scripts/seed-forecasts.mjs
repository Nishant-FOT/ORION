...
async function warmPingChokepoints() {
  const baseUrl = process.env.ORION_API_BASE_URL;
  if (!baseUrl) { console.log('  [Chokepoints] Warm-ping skipped (no ORION_API_BASE_URL)'); return; }
  try {
    const resp = await fetch(`${baseUrl}/api/supply-chain/v1/get-chokepoint-status`, {
      headers: { 'User-Agent': CHROME_UA, Origin: 'https://orion.app' },
      ...
      const resp = await withRetry(async () => {
        ...
        const response = await forecastFetch(provider.apiUrl, {
          ...
          headers: {
            ...
            ...(provider.name === 'openrouter' ? { 'HTTP-Referer': 'https://orion.app', 'X-Title': 'ORION' } : {}),
          },
          body: JSON.stringify({
