...
const RELAY_URL = process.env.RELAY_URL || 'https://proxy.orion.app';
...
async function fetchFeed(feed) {
  ...
  async function tryFetch(url, label) {
    try {
      ...
      if (!resp.ok) {
        console.warn(`  ${feed.name} (${label}): HTTP ${resp.status}`);
        return [];
      }
      ...
      console.warn(`  ${feed.name} (${label}): ${e.message}`);
      return [];
    }
  }
  ...
  if (proxyUrl) {
    const proxyResult = await tryFetch(proxyUrl, 'proxy');
    if (proxyResult.length > 0) return proxyResult;
  }
  return tryFetch(feed.url, 'direct');
}
