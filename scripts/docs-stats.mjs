...
function computeStats() {
  ...
  const protoServices = protoFiles
    ...
    .reduce((a, b) => a + b, 0);
  const protoDomainFolders = dirsIn('proto/orion').length;
  ...
  // ---- Server domain handlers (server/orion/*/) ----
  const serverDomains = dirsIn('server/orion').length;
...
function claims(s) {
  return [
    ...
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /typed API: (\d+)\s+services/, value: s.protoServices },
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /typed API: \d+\s+services, (\d+)\s+proto files/, value: s.protoFiles },
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /\*\*(\d+)\s+proto files\*\* defining/, value: s.protoFiles },
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /\*\*(\d+)\s+typed service domains\*\*/, value: s.protoServices },
    // Heading labels the table below it, which is enumerated from server/orion/* dirs → pin to serverDomains (not protoServices; the two equal 34 today but a domain with two `service` blocks would diverge them).
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /##\s+(\d+)\s+Service Domains/, value: s.serverDomains },
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /Protocol Buffers \((\d+)\s+files\)/, value: s.protoFiles },
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /orion\)\. (\d+)\s+services, \d+\s+proto files, and a global/, value: s.protoServices },
    { file: 'blog-site/src/content/blog/build-on-orion-developer-api-open-source.md', re: /orion\)\. \d+\s+services, (\d+)\s+proto files, and a global/, value: s.protoFiles },
    { file: 'blog-site/src/content/blog/what-is-orion-real-time-global-intelligence.md', re: /typed APIs \((\d+)\s+proto files, \d+\s+services\)/, value: s.protoFiles },
    { file: 'blog-site/src/content/blog/what-is-orion-real-time-global-intelligence.md', re: /typed APIs \(\d+\s+proto files, (\d+)\s+services\)/, value: s.protoServices },
    ...
    { file: 'blog-site/src/content/blog/orion-vs-traditional-intelligence-tools.md', re: /using the (\d+)\s+typed API services/, value: s.protoServices },
  ];
}
