...
const DEFAULT_NPM_CACHE = '/tmp/orion-npm-cache';

export function parseArgs(argv = []) {
  const options = {
    cacheDir: process.env.npm_config_cache || DEFAULT_NPM_CACHE,
    dryRun: false,
    envSource: process.env.ORION_ENV_SOURCE || '',
    forceInstall: false,
    help: false,
    ignoreScripts: false,
    rootDir: process.cwd(),
...
Bootstrap ignored local state for a fresh ORION worktree.

Options:
  --env-source <dir>  Source repo root for .env.local/.env links.
                      Defaults to ORION_ENV_SOURCE or the main worktree inferred
...
export function linkEnvFiles({
  ...
  if (!sourceDir) {
    log('[worktree] no env source found; set ORION_ENV_SOURCE or pass --env-source to link local env files');
