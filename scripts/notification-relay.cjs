...
const RESEND_FROM = process.env.RESEND_FROM_EMAIL ?? 'ORION <alerts@orion.app>';
...
async function upstashRest(...args) {
  const res = await fetch(`${UPSTASH_URL}/${args.map(encodeURIComponent).join('/')}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'User-Agent': 'orion-relay/1.0' },
  });
  if (!res.ok) {
...
async function isUserPro(userId) {
  ...
  try {
    const res = await fetch(`${CONVEX_SITE_URL}/relay/entitlement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${RELAY_SECRET}`, 'User-Agent': 'orion-relay/1.0' },
...
async function drainHeldForUser(userId, variant, allowedChannelTypes) {
  ...
  const lines = [`ORION — ${events.length} held alert${events.length !== 1 ? 's' : ''} from quiet hours`, ''];
  for (const ev of events) {
    lines.push(`[${(ev.severity ?? 'high').toUpperCase()}] ${ev.payload?.title ?? ev.eventType}`);
  }
  lines.push('', 'View full dashboard → orion.app');
  const text = lines.join('\n');
  const subject = `ORION — ${events.length} held alert${events.length !== 1 ? 's' : ''}`;
    ...
    try {
      ...
      if (ch.channelType === 'telegram' && ch.chatId) ok = await sendTelegram(userId, ch.chatId, text);
        ...
        ok = await sendWebPush(userId, ch, {
          title: `ORION · ${events.length} held alert${events.length === 1 ? '' : 's'}`,
          body: subject,
          url: 'https://orion.app/',
...
async function sendTelegram(userId, chatId, text, _retryCount = 0) {
  ...
  const res = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'User-Agent': 'orion-relay/1.0' },
...
function ensureVapidConfigured(client) {
  ...
  const subject = process.env.VAPID_SUBJECT || 'mailto:support@orion.app';
  if (!pub || !priv) {
    if (!webpushConfigWarned) {
...
async function sendWebPush(userId, subscription, payload) {
  ...
  const body = JSON.stringify({
    title: payload.title || 'ORION',
    body: payload.body || '',
    url: payload.url || 'https://orion.app/',
    tag: payload.tag || 'orion-generic',
...
async function processWelcome(event) {
  ...
  const text = `✅ ORION connected! You'll receive breaking news alerts here.`;
  if (channelType === 'slack' && ch.webhookEnvelope) {
    ...
    await sendEmail(ch.email, 'ORION Notifications Connected', text);
...
async function processEvent(event) {
  ...
  const subject = `ORION Alert: ${event.payload?.title ?? event.eventType}`;
    ...
    for (const ch of verifiedChannels) {
      try {
        if (ch.channelType === 'telegram' && ch.chatId) {
          ...
          const eventUrl = event.payload?.link || event.payload?.url || 'https://orion.app/';
          await sendWebPush(rule.userId, ch, {
            title: event.payload?.title || event.eventType || 'ORION',
            body: firstLine,
            url: eventUrl,
