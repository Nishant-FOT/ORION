...
function extractUserContext(prefs) {
  ...
  const watchlist = prefs['orion-market-watchlist-v1'];
  ...
  const frameworks = prefs['orion-analysis-frameworks'];
  const panelFrameworks = prefs['orion-panel-frameworks'];
  ...
  const panels = prefs['orion-panels'];
  if (Array.isArray(panels)) {
    ctx.enabledPanels = panels.filter(p => typeof p === 'string').slice(0, 30);
  }

  const disabled = prefs['orion-disabled-feeds'];
