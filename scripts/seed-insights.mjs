...
// Set ORION_RELAY_KEY on the Railway service (must match a value in
// Vercel's ORION_VALID_KEYS). Origin alone is no longer reliable
...
async function warmDigestCache() {
  const apiBase = process.env.API_BASE_URL || 'https://api.orion.app';
  const headers = {
    'User-Agent': CHROME_UA,
    Origin: 'https://orion.app',
  };
  if (RELAY_API_KEY) headers['X-ORION-Key'] = RELAY_API_KEY;
  try {
    ...
    if (resp.ok) console.log('  Digest cache warmed via RPC');
      ...
      const keyNote = RELAY_API_KEY ? '' : ' (ORION_RELAY_KEY not set — Origin-only auth)';
