...
//
// Usage:
//   ORION_API_KEY=wm_xxx API_BASE=https://api.orion.app \
//     node scripts/audit-resilience-cohorts.mjs
//   ORION_API_KEY=wm_xxx API_BASE=... \
...
// (see src/shared/premium-paths.ts). A valid ORION_API_KEY is
...
if (!FIXTURE_PATH) {
  if (!API_BASE) {
    console.error('[audit-resilience-cohorts] API_BASE env var required (e.g. https://api.orion.app), or FIXTURE=path.json for offline mode');
    process.exit(2);
  }
  if (!process.env.ORION_API_KEY) {
    console.error('[audit-resilience-cohorts] ORION_API_KEY env var required; resilience RPC paths are in PREMIUM_RPC_PATHS.');
    process.exit(2);
  }
}
...
function apiHeaders() {
  ...
  if (process.env.ORION_API_KEY) {
    h['X-ORION-Key'] = process.env.ORION_API_KEY;
  }
  return h;
}
